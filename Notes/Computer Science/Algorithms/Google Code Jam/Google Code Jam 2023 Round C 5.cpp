...
/*
Intuitively one might suppose that what is relevant is uh firstly like the latest largest values can be potentially used to easily troll the uh opposition so if there is ever a relevant uh decrease like a uh a larger letter occurs before the uh end I mean it almost seems like uh oh I mean clearly if it is increasing then we can not force but if there is ever any decrease it is like depends on the precise uh P value uh uh uh because one might suppose that if P=2 e.g. then ZAZ is OK whereas a P=3 value here can be used to uh kill it or whatever and in fact BLAHBLAHBLAHZAZ as well e.g. so it uh suffices to see what that P value was I think if P>=3 and there is like some nontrivially split block is like uh but then the way the blocks break is like non trivial.
...
Sat, Apr 15 at 12:15 PM

Keep it 100|100 100|100 super trill <3
"
*/
