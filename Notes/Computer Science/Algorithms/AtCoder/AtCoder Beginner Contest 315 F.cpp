...
Ugh kind of annoying but I will leak 1 secret on the road to CodeForces Top 2400 Gold is that most of these rounds will feature at least 1 kind of interesting Dynamic Programming task just to test if competitors really deeply understand Dynamic Programming or whatever and so 1 meta trick if you do this too much and are in the hunt for Top 2400 is to just try out see if one can hack around a Dynamic Program rather than a sharper Mathematics forces solution like here instead of Googling trawling around the canon.

Test + Additional Input:


Analysis:
It ought to be noted immediately onsight that this is partially related to my Cracks Maze Puzzle task and also that immediately like C < ~30 or something and so O[30 * N] is a fast enough algorithm or maybe under a unimodality assumption which may be rigourously provable that O[log[30] * N] would suffice here ugh if one could reduce that then to a canonical type of shortcuts algorithm.
*/
