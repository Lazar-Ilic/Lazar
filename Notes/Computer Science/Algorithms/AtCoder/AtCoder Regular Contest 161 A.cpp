...
Analysis:
Think uh Proof Of Correctness here is simple smoothing uh...
*/
	...
	sortify(av);
	bv=V(a);
	for(b=0;b<a/2;b++)bv[2*b+1]=av[a-1-b];
	for(b=0;b<=a/2;b++)bv[2*b]=av[a/2-b];
	//outv(av);
	//outv(bv);
	for(b=0;b<a/2;b++){
		if(bv[2*b]>=bv[2*b+1] or bv[2*b+2]>=bv[2*b+1]){
			out<<"No"<<nl;
			return;
		}
	}
