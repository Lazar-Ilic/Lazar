...
*/
	...
	sortify(avp);
	e=sz(avp);
	for(d=avp[sz(avp)-1].first;d>=0;d--){
		// Add in the relevant first initial dudes uh at this level if uh relevant and do a simultaneous Breadth First Search outwards per usual.
		bv=V(0);
		for(auto dude:av){
			// Process through neighbours if added in this round.
		...
		while(e-1>=0 and avp[e-1].first==d){
			if(avb[avp[e-1].second]==0){
				bv.add(avp[e-1].second);
				avb[avp[e-1].second]=1;
			}
			e--;
		}
		av=bv;
	...
	z=0;
	for(d=0;d<a;d++)if(avb[d])z++;
	out<<z<<nl;
	for(d=0;d<a;d++)if(avb[d])out<<d+1<<" ";
	out<<nl;
}
