...
Oh uh OKOKOKOK did not really realise to simply do it via permutations uh I guess that like sometimes for some like n~20 cases it really is strongly superior here to be using like O[n^3 * 2^n] rather than O[n!] if that is validly runtime here... uh OKOKOK anyways just OK I misread the source code and need to now go ahead and note that up top uh...
...
*/
	...
	in>>a>>b;
	for(c=0;c<20;c++)for(d=0;d<20;d++)graph2[c][d]=1000;
	for(c=0;c<20;c++){graph2[a][c]=1;graph2[c][a]=1;}
	avs=VS(a);
	avv=VV(a,V(0));
	...
	for(c=0;c<a;c++){
		for(d=c+1;d<a;d++){
			e=0;
			for(f=0;f<b;f++)if(avs[c][f]!=avs[d][f])e++;
			if(e==1){
				avv[c].add(d);
				avv[d].add(c);
				graph2[c][d]=1;
