...
*/
	...
	in>>as;
	avv=VV(sz(as)+1,V(sz(as)+1,0));
	avv[0][0]=1;
	z=1;
	for(auto dude:as){
		if(dude=='(')for(b=1;b<=sz(as);b++)avv[z][b]=avv[z-1][b-1];
		else if(dude==')')for(b=0;b<sz(as);b++)avv[z][b]=avv[z-1][b+1];
		else{
			avv[z][0]=avv[z-1][1];
			avv[z][sz(as)]=avv[z-1][sz(as)-1];
			for(b=1;b<sz(as);b++)avv[z][b]=(avv[z-1][b-1]+avv[z-1][b+1])%p;
		}
		z++;
	}
	out<<avv[sz(as)][0]<<nl;
