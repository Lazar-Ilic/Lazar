...
Analysis:
Well to be sure if we were to insert these dudes in using a disjoint set union then the uh conditions change in to like basically uh each condition bans 2 disjoint sets from being unionised in an operation so we can like create a USETP or SETP of pairs of DSs which are banned and then test if each dude is in the BANNED list.
*/
	...
	in>>c;
	av=V(c);
	for(d=0;d<c;d++)in>>av[d];
	av.add(a);
	in>>d;
	bv=V(d);
	for(e=0;e<d;e++)in>>bv[e];
	bv.add(b);
	MPI ampi;
	for(auto dude:avp)ampi[{*lowerbound(av.begin(),av.end(),dude.first),*lowerbound(bv.begin(),bv.end(),dude.second)}]++;
	m=0;
	n=INT_MAX;
	for(auto dude:ampi){
		// Can sometimes optimise this to use uh else if flow by setting m and n to be like the 1st dude if there is a logically ensured 1st dude.
		if(dude.second<n)n=dude.second;
		if(dude.second>m)m=dude.second;
	}
	if(sz(ampi)!=(c+1)*(d+1))n=0;
	out<<n<<" "<<m<<nl;
}
