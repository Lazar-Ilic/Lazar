...
/*
Analysis:
Not surprised if Wrong Answer.

Post Contest Analysis:
Could probably be optimised much further by constant factor.
...
*/
	...
	avvl=VVL(sz(as),VL(2));
	if(as[0]=='a'){
		avvl[0][0]=min(a,c+b+c);
		avvl[0][1]=min(a+c,c+b);
	}
	else{
		avvl[0][0]=min(b,c+a+c);
		avvl[0][1]=min(c+a,b+c);
	}
	for(d=1;d<sz(as);d++){
		if(as[d]=='a'){
			avvl[d][0]=min(min(avvl[d-1][0]+a,avvl[d-1][0]+c+b+c),min(avvl[d-1][1]+c+a,avvl[d-1][1]+b+c));
			avvl[d][1]=min(min(avvl[d-1][0]+a+c,avvl[d-1][0]+c+b),min(avvl[d-1][1]+b,avvl[d-1][1]+c+a+c));
		}
		else{
			avvl[d][0]=min(min(avvl[d-1][0]+b,avvl[d-1][0]+c+a+c),min(avvl[d-1][1]+a+c,avvl[d-1][1]+c+b));
			avvl[d][1]=min(min(avvl[d-1][0]+c+a,avvl[d-1][0]+b+c),min(avvl[d-1][1]+a,avvl[d-1][1]+c+b+c));
		}
	}
	out<<min(avvl[sz(as)-1][0],avvl[sz(as)-1][1])<<nl;
}
