...
Z az(){
	...
	for(b=0;b<a;b++){
		avv[a/2][b]=a*a-1-avv[a/2-1][b];
		av[a*a-1-avv[a/2-1][b]]=1;
	}
	for(b=1;b<=a*a;b++){
		if(av[b]==0 and b%2==1){
			bv.add(b);
	...
	for(b=a/2+1;b<a;b++){
		avv[a/2][b]=a*a-avv[a/2-1][b];
		av[a*a-avv[a/2-1][b]]=1;
	}
	for(b=0;b<=a/2;b++){
		avv[a/2+1][b]=a*a-avv[a/2][b];
		av[a*a-avv[a/2][b]]=1;
	}

	// Process remainder into bv.
	for(b=1;b<=a*a;b++){
		if(av[b]==0 and b%2==1){
			bv.add(b);
		}
