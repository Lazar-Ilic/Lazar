...
*/
	...
	y=1;
	for(b=1;b<a and y;b++)if(av[b]!=av[0])y=0;
	if(y){
		out<<"Yes"<<nl;
		out<<0<<nl;
		return;
	...
	for(b=1;b<=10000;b++){
		// Need to sort av in order uh and then uh index up from a down to 1 I think greedy will uh work here... proof? Say they start out uh nearly flat like uh 0 0 0 a becomes uh 0 0 0 10 becomes 2 3 4 11 becomes 6 6 6 12 becomes uh 8 9 10 13 to 12 12 12 14 to 14 15 15 16 to 17 17 18 18 to 21 20 20 19 to uh OK and this will fail where uh 21 23 23 23 uh is the average here uh integer uh? Average is integer uh but uh 2 more loops be like... 14 15 15 16 need uh like sums to uh 1 2 3 4 like uh 20 so like 6 5 5 4 is like uh 42 41 32 31 frankly is like somehow need to uh consider uh maybe processing here uh 2 loops out or maybe even somehow like uh full on n loops out is not viable as [n!]^[n-1] is certainly not viable here.
		y=1;
		for(c=1;c<a and y;c++)if(av[c]!=av[0])y=0;
		if(y){
			out<<"Yes"<<nl;
			out<<b<<nl;
			for(c=0;c<b;c++)outv(avv[c]);
			return;
		}
	}
}
