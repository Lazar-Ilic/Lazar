...
*/
	...
	in>>as;
	for(b=0;b<a;b++)bv[b]=3*(av[b])+(as[b]=='E')+2*(as[b]=='X');
	for(b=0;b<3;b++){
		for(c=0;c<3;c++){
			for(d=0;d<3;d++){
				avb=VB(3,0);
				avb[b]=1;avb[c]=1;avb[d]=1;
				if(avb[0]){
					// Go ahead and compute and then multiply by relevant MEX Minimum Excluded Natural here. Can reduce runtime here by some non trivial constant factor by instead uh essentially forming 9 vectors of indices and then merging through uh rather than looping through this is like O[27 * N] and it can be made I think more like O[9 * N] or O[3 * N] or O[2 * N] maybe somehow easyish.
					if(avb[1]==0)bl=1;
					else if(avb[2]==0)bl=2;
					else bl=3;
					cl=0;
					dl=0;
					el=0;
