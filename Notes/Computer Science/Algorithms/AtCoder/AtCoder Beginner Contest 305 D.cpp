...
*/
	...
	bv=V(a,0);
	for(b=2;b<a;b++){
		if(b%2==0)bv[b]=bv[b-1]+av[b]-av[b-1];
		else bv[b]=bv[b-1];
	}
	in>>b;
	for(c=0;c<b;c++){
		in>>d>>e;
		if(d==0)d++;
		f=bv[lowerbound(all(av),e)-av.begin()]-bv[lowerbound(all(av),d)-av.begin()];
		if(bv[lowerbound(all(av),d)-av.begin()]-bv[lowerbound(all(av),d)-av.begin()-1]>0)f+=*(lowerbound(all(av),d))-d;
		if(bv[lowerbound(all(av),e)-av.begin()]-bv[lowerbound(all(av),e)-av.begin()-1]>0)f-=*(lowerbound(all(av),e))-e;
		out<<f<<nl;
	}
}
