...
*/
	...
	bv=V(b); // For Answers To Output
	avv=VV(b,V(3));
	for(c=0;c<b;c++){
		in>>avv[c][0]>>avv[c][1];avv[c][0]--;avv[c][1]--;
		avv[c][2]=c;
	}
	sortify(avv);
	c=b-1; // To Check Visibility From
	cv=V(0); // Stack To Simulate And Search
	for(d=a-1;d>=0;d--){
		// Search From This Element To The Right In Stack How Many Are Using Negative And LowerBound
		while(c>=0 and avv[c][0]==d){
			bv[avv[c][2]]=lowerbound(all(cv),-avv[c][1])-cv.begin();
			c--;
		}
		// Add Element In To Stack And Update Accordingly
		while(sz(cv)>0 and av[-cv[sz(cv)-1]]<av[d])cv.pop_back();
		cv.push_back(-d);
	}
	for(auto dude:bv)out<<dude<<nl;
}
