...
/*
Analysis:
Once we remove all of the dudes with degree >=3 and their connection points and account for them we will be left with like uh... some 2-stars connected in to disconnected components and like uh... 1 dude has 2 edges and uh... 2 of those dudes has 5... so (edges+1)/3 is the formula for those dudes uh... find that remainder chunk left over.
...
*/
	...
	in>>a;
	VSET avset(a);
	av=V(0); // Sizes to be sorted and output later.
	for(b=0;b<a-1;b++){
		in>>c>>d;
		c--;d--;
		avset[c].insert(d);
	...
	for(b=0;b<a;b++){
		if(sz(avset[b])>=3){
			av.add(sz(avset[b]));
			avp=VP(0);
			for(auto neighbour:avset[b]){
				for(auto neighbour2:avset[neighbour]){
					avp.add({neighbour,neighbour2});
				}
			}
			for(auto dude:avp){
				avset[dude.first].erase(dude.second);
				avset[dude.second].erase(dude.first);
			}
		}
	}
	for(b=0;b<a;b++){
