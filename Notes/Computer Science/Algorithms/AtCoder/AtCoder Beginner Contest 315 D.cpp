...
*/
	...
	// Can do with linked lists instead.
	for(c=0;c<a;c++)aset.insert(c);
	for(d=0;d<b;d++)bset.insert(d);
	z=1;
	while(z){
		z=0;
		cv=V(0);
		...
		if(sz(bset)>1){
			for(auto dude:aset){
				y=0;
				for(x=0;x<26 and y<=1;x++)if(avv[dude][x]>0){y++;w=x;}
				if(y==1){
					z=1;
					cv.add(dude);
					ev.add(w);
					//for(auto dude1:bset)bvv[dude1][w]--;
				}
			}
		}
		if(sz(aset)>1){
			for(auto dude:bset){
				y=0;
				for(x=0;x<26 and y<=1;x++)if(bvv[dude][x]>0){y++;w=x;}
				if(y==1){
					z=1;
					dv.add(dude);
					fv.add(w);
					//for(auto dude1:aset)avv[dude1][w]--;
				}
			}
		}
		for(auto www:ev)for(auto dude1:bset)bvv[dude1][www]--;
		for(auto www:fv)for(auto dude1:aset)avv[dude1][www]--;
		for(auto dude:cv)aset.erase(dude);
		for(auto dude:dv)bset.erase(dude);
	}
	out<<sz(aset)*sz(bset)<<nl;
}
