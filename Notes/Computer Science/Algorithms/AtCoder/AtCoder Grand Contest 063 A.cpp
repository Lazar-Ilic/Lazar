...
Obviously should have just onsighted this 1 1st due to penalty concerns rather than tanking on D.

Test + Additional Input:


Analysis:
Oh OKOKOK so after a little bit of thought on D E F I return to A actually and realise that uh we want to cover up our opponents' letters actually to block those from being the MEX so they just go in order blocking letters and the earliest unblocked letter should yield the desired answer here.
*/
	...
	d=0;
	for(b=0;b<a;b++){
		if(b%2==0 and d<sz(bv)-1)d++;
		else if(b%2==1 and c<sz(av)-1)c++;
		if(av[c]<bv[d])out<<"Alice"<<nl;
		else out<<"Bob"<<nl;
	}
}
