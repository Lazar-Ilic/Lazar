...
Analysis:
Dijkstra style Breadth First Search works here. Layers and then distances from the point set get processed in merge order if not yet visited etc. etc. yikes I uh made a logical error from misreading this task in round and did not realise that the different costs condition means it is a little bit trickier than this ugh. Still probably some like modified uh priority queue maybe works but it might be a little trickier ugh. Somehow uh uh uh could add in each edge in to the priority queue somehow based around the uh day and uh minimum time to visit and then uh uh uh somehow uh invert you know the uh Delta value basically in to the minimum day after that with respect to this Delta vector cv and then uh I think that would work but I frankly am a little bit tired and lethargic here to actually go ahead and implement it up and have more important MIT Open CourseWares to go solve the homework tasks to etc. etc.
*/
	...
	for(c=0;c<b;c++){
		in>>d>>e>>f;
		d--;e--;
		avvp[d].add({f,e}); // Edge Weight, Neighbour
		avvp[e].add({f,d}); // Edge Weight, Neighbour
	}
	dv=V(a,0); // Visited yet? Uh day+1 for output later.
	ev=V(a,0); // Minimum cost to hit upon day of visiting.
	in>>c;
	av=V(c); // List of dudes in uh this level uh.
	for(d=0;d<c;d++){
		in>>av[d];
		dv[av[d]]=1;
	}
	in>>c;
	cv=V(c); // Costs for each day uh.
	for(d=0;d<c;d++)in>>cv[d];
	for(d=0;d<c;d++){
		bv=V(0); // Next level for next day. Any new dudes from today uh.
		//avp=VP(0);
		PQP apqp;
		for(auto dude:av){
			for(auto edges:avvp[dude]){
				if(edges.first<=cv[c] and !dv[edges.second]){
					apqp.add(edges);
				}
			}
		}
		//sortify(avp);
		// Eh not sure how necessary uh umm... seems not to be here. Or SETP.
		// Oh yeah the point here was to use a priority queue uh duh...
		while(!apqp.empty()){
			ap=apqp.top();
			apqp.pop();
			if(!dv[ap.second]){
				dv[ap.second]=d+2;
				bv.add(ap.second);
				for(auto edges:avvp[ap.second]){
					if(!dv[edges.second] and ){
						apqp.add({,edges.second});
					}
				}
			}
		}
