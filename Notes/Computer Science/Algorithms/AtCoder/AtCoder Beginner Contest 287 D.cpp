...
Z az(){
	...
	bv=V(sz(bs)+1);
	bv[0]=1;
	for(b=1;b<=sz(bs) and av[b-1]==1;b++){
		if(as[b-1]==bs[b-1] or as[b-1]=='?' or bs[b-1]=='?')av[b]=1;
	}
	for(b=1;b<=sz(bs) and bv[b-1]==1;b++){
		if(as[sz(as)-b]==bs[sz(bs)-b] or as[sz(as)-b]=='?' or bs[sz(bs)-b]=='?')bv[b]=1;
	}
	for(b=0;b<=sz(bs);b++){
		if(av[b]==1 and bv[sz(bs)-b]==1){
			out<<"Yes"<<nl;
		}
		else{
			out<<"No"<<nl;
