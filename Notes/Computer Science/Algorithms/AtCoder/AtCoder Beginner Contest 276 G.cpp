...
Z az(){
	...
	in>>a>>b;
	// Oh it is as easy as adding up caseworks on how many precise clones of 3 we have and then each step size is forced to be like a 1 or a 2 on top of that which we can precompute all of the Fibonacci dudes and Fibonacci dude prefix sums for that and then I think simply sum with like O[n] and then O[1] choose calls frankly for stars and barsing that precise count of 3s so separate those dudes we have like it is really quite simple separate the 3s so if we have c 3s just like [c+a-1] choose [a-1] and then boom down the relevant prefix sum or prefix sum of prefix sums meta.
}
