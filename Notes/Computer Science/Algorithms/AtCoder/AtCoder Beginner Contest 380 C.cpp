...
*/
	...
	bs="";
	cs="";
	d=0; // Number Of 0 Block
	e=0; // Number Of 1 Block
	f=0; // Size Of Relevant 0 Block We Skip
	z=1;
	for(c=0;c<sz(as);c++){
		if(c==0 or as[c]!=as[c-1]){
			if(as[c]=='0')d++;
			else e++;
		}
		if((e==b-1 and as[c]=='0') or (e==b and as[c]=='1'))cs+=as[c];
		else if(e==b and as[c]=='0' and z){
			z=0;
			reverse(all(cs));
			bs+=cs;
			bs+=as[c];
