...
Analysis:
So there exists a kind of naive I was tanked out here on Task G and felt kind of lazy but there exists a uh simpleish O[9! * [9 choose 5]] type of solution which can be sharpened up pruned dramatically which has to do with uh for each uh permutation uh basically checking uh possible break points uh and the way we prune is by ensuring that e.g. each team is in increasing sorted order and that furthermore the minima in each time are themselves sorted from Left to Right and this super duper super prunes off cases early on if we code it up in the Right way to not actually check out uh that [9 choose 5] term can be dramatically cut down I think in to more like O[9! * ~10] or something like that in expected run time.
*/
