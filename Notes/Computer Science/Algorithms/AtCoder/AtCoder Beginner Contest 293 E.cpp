...
L function1(L al,L nl,L ml){
	if(nl==0)return 1%ml;
	if(nl==1)return (1+al)%ml;
	if(nl%2==1)return ((1+al)*function1((al*al)%ml,(nl-1)/2,ml))%ml;
	else return (1+(al*(1+al))%ml*function1((al*al)%ml,nl/2-1,ml))%ml;
}
...
Z az(){
	...
	/*
	https://ideone.com/gWBvJl
	*/

	in>>cl>>al>>bl;
	out<<function1(cl,al-1,bl)<<nl;
}
