...
UI128 euclid(UI128 a,UI128 b,UI128 &x,UI128 &y){//Simon Lindholm
	if(!b)return x=1,y=0,a;
	UI128 d=euclid(b,a%b,y,x);
	return y-=a/b*x,d;
}
UI128 crt(UI128 a,UI128 m,UI128 b,UI128 n){ // Returns x = a [modulo m] and x = b [modulo n]
	if(n>m)swap(a,b),swap(m,n);
	UI128 x,y,g=euclid(m,n,x,y);
	assert((a-b)%g==0); // else no solution
	x=(b-a)%n*x%n/g*m+a;
	return x<0?x+m*n/g:x;
}
...
Analysis:
Contradiction here would emerge if and only if there was 1 pair with a contradiction which would mean that some p divided both right hand sides and the left hand side was not congruent modulo that p or umm... could be a power of p yadda yadda yadda. Certainly this implies that there is some divisor of the difference in right hand side dudes uh which must uh divide c actually uh then I think uh... or uh not quite but uh something else uh... but that difference yadda...
*/
