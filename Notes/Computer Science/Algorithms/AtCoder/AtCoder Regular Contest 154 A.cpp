...
Z az(){
	...
	in>>a>>as>>bs;
	e=0; // as smaller value
	f=0;
	g=0; // Represents first deviation value which is bigger.
	h=0;
	i=0; // 2 Pointers indices I guess.
	while(as[h]==bs[h])h++;
	if(h<a and bs[h]<as[h]){
		cs=as;
		as=bs;
		bs=cs;
	}
	// OK so as is forced to be the lesser one.
	h++;
	while(h<a){
		if(bs[h]<as[h]){
			// Swap.
