...
Z az(){
	...
	in>>a>>b>>c>>d>>e;
	avv=VV(c+1,V(4,-1)); // Topmost, Bottommost, Leftmost, Rightmost
	for(f=0;f<a;f++){
		for(g=0;g<b;g++){
			in>>h;
			if(avv[h][0]==-1)avv[h][0]=f;
			avv[h][1]=f;
			if(avv[h][2]==-1 or g<avv[h][2])avv[h][2]=g;
			if(avv[h][3]==-1 or g>avv[h][3])avv[h][3]=g;
		}
	}
	y=c;
	for(h=1;h<=c;h++){
	...
	for(f=0;f<=a-d;f++){
		for(g=0;g<=b-e;g++){
			z=0;
			for(h=1;h<=c;h++){
				if(avv[h][0]!=-1 and f<=avv[h][0] and f+d-1>=avv[h][1] and g<=avv[h][2] and g+e-1>=avv[h][3])z++;
				//out<<h<<" "<<avv[h][0]<<" "<<avv[h][1]<<" "<<avv[h][2]<<" "<<avv[h][3]<<nl<<f<<" "<<f+d-1<<" "<<g<<" "<<g+e-1<<nl;
			}
			out<<y-z<<" ";
		}
		out<<nl;
