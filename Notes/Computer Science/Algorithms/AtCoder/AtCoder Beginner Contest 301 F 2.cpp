...
/*
Markov chain umm relevant values are like how many dudes are 0 in i.e. all lower case thus far and how many dudes are uh like OK if not ? then it is a forced transition uh in any case need how many are 0 so far how many are uh 1 how many are 2 and how many are uh 3 and then forced uh 26 or whatever. Modulo p. Ungh 1st and 2nd characters are uh equal...
...
*/
	...
	for(b=0;b<a;b++){
		if(as[b]>='A' and as[b]<='Z'){
			avvl[b+1][0]=0;
			avvl[b+1][1]=avvl[b][0];
			avvl[b+1][2]=(avvl[b][1]+avvl[b][2])%p;
			avvl[b+1][3]=0;
		}
			...
			avvl[b+1][1]=avvl[b][1];
			avvl[b+1][2]=0;
			avvl[b+1][3]=(avvl[b][2]+avvl[b][3])%p;
		}
		else{
			// ?
			avvl[b+1][0]=(26*avvl[b][0])%p;
			avvl[b+1][1]=(26*avvl[b][1]+26*avvl[b][0])%p;
			avvl[b+1][2]=(26*avvl[b][1]+26*avvl[b][2])%p;
			avvl[b+1][3]=(26*avvl[b][2]+26*avvl[b][3])%p;
		}
	}
	out<<(avvl[a][0]+avvl[a][1]+avvl[a][2]+avvl[a][3])%p<<nl;
}
