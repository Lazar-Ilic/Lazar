...
*/
	...
	for(z=0;z<a;z++){
		...
		if(al<=6){
			...
			// Better to have a 1 if possible uh uh uh take the uh first deviation would be uh uh uh so if it has 3 1s that is OK else uh simply uh uh uh like 101000 would be like uh... 100110 basically uh... if possible here else it goes in to another simple little control flow like uh... 100010 or whatever so if the 2nd 1 occurs in the uh final or 2nd final position is clearly like uh... 11100000 whatever... can replace the relevant bl invocations in memory directly to print statements ungh.
			av=V(0);
			for(b=62;b>=0 and sz(av)<=2;b--){
				if(al ba (1L << b))av.add(b);
			}
			//out<<al<<nl;
			//outv(av);out<<nl<<nl;
			if(sz(av)==3){
				bl=0;
				for(b=0;b<=2;b++)bl+=(1L << av[b]);
				out<<bl<<nl;
			}
			else if(sz(av)==2 and av[1]>=2){
				bl=0;
				bl+=(1L << av[0]);
				bl+=(1L << (av[1]-1));
				bl+=(1L << (av[1]-2));
				...
				out<<(1L << (av[0]-1))+(1L << (av[0]-2))+(1L << (av[0]-3))<<nl;
			}
		}
	}
}
