...
*/
	...
	for(b=13;b<=23;b++){
		// Merge from 11 uh up basically uh.
		c=0;
		e=13-2;
		while(avv[b-e+1][0]*avv[e][c]<=avv[e][e-1])c++;
		for(d=0;d<e;d++)if(d!=c)avv[b].add(avv[e][d]);
		for(d=0;d<b-e+1;d++)avv[b].add(avv[b-e+1][d]*avv[e][c]);
	}
	for(b=24;b<=45;b++){
		c=0;
		e=24-2;
