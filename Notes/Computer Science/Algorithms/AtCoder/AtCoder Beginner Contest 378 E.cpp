...
*/
	...
	for(c=0;c<a;c++){
		...
		d%=b;
		z=(z+d)%b; // Prefix Sum
		al+=bitcounts.query(z)*z-bitsums.query(z)+(bitcounts.query(b)-bitcounts.query(z+1))*(z+b)-(bitsums.query(b)-bitsums.query(z+1)); // Can Be Optimised Further
		bitcounts.update(z,1);
		bitsums.update(z,z);
		//out<<z<<" "<<d<<" "<<al<<nl;
	}
	out<<al<<nl;
