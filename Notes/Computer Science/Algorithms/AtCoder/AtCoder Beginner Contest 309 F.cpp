...
Analysis:
Uh errrm elem I think the strategy which might work here but I am a little bit L A Z Y Cabi Cabi Cabi Cabi today to type up is something like uh... firstly we can insert a 6x factor so take all the dudes and their permutations and then like insert them in order uh in rounds for == cases uh but in order and basically track the relevant minimum thus far under each threshold value on uh so the X is ensured via the order in which we are inserting and processing but like a uh... range updated uh... basically a fast range updated segment tree can work here uh... dunno if there is some easy O[n] or uh O[6 * n] here uh... need to update in the relevant uh range uh... or just do a point update and do a fast Range Minimum Query perhaps uh... querying all dudes to the Left can work too uh... so just a segment tree might ACcepted here uh...
*/
