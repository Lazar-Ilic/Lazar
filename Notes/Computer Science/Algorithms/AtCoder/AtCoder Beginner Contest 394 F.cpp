...
Uh think the solution here is something like uh just pin a root and then discover the degree and subtree size for each node as usual. Then like it suffices to compute the maximum uh subtree which is like uh basically all the primary dudes on the subtree must have a uh uh uh degree of >=4 or something. But the key then is like that we want to maximise these nodes uh or maybe uh... think about how many dudes will be left as kind of leaves in the subtree eh something about something and adding in 3 leaves at the end or something eh egh so like maybe bottoms up tree dynamic programme conditioning on uh basically if we can extend further we must in fact do so so otherwise just count the node if it can not be extended further as if and only if it has 3 leaves as like a potential leaf in the big subtree of master nodes or whatever.
*/
