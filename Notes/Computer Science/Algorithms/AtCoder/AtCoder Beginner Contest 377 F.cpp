...
*/
	...
	for(c=0;c<b;c++){
		...
		asetv.insert({1,0,d});
		asetv.insert({0,1,e});
		asetv.insert({1,1,d+e});
		asetv.insert({1,-1,d-e});
	}
	MPI ampi;
	MII convert;
	for(a=2;a<=12000;a++)convert[a*(a-1)/2]=a;
	bl=0;
	for(auto dude:asetv){
		// Count Number On All Lines
		if(dude[0]+dude[1]==1)bl+=al;
		else if(dude[0]+dude[1]==2)bl+=al-abs(al+1-dude[2]);
		else bl+=al-abs(dude[2]);
	}
	for(auto dude1:asetv)for(auto dude2:asetv)if(dude1<dude2 and !(dude1[0]==dude2[0] and dude1[1]==dude2[1])){
		// Compute Intersection Point And If Inside Add ++ To MPI ampi
		I lolx=-1,loly=-1;
		if(((L)dude1[1]*dude2[0]-(L)dude1[0]*dude2[1])!=0 and ((L)dude1[1]*dude2[2]-(L)dude1[2]*dude2[1])%((L)dude1[1]*dude2[0]-(L)dude1[0]*dude2[1])==0 and ((L)dude1[2]*dude2[0]-(L)dude1[0]*dude2[2])%((L)dude1[1]*dude2[0]-(L)dude1[0]*dude2[1])==0){
			lolx=((L)dude1[1]*dude2[2]-(L)dude1[2]*dude2[1])/((L)dude1[1]*dude2[0]-(L)dude1[0]*dude2[1]);
			loly=((L)dude1[2]*dude2[0]-(L)dude1[0]*dude2[2])/((L)dude1[1]*dude2[0]-(L)dude1[0]*dude2[1]);
		}
		if(lolx>=1 and lolx<=al and loly>=1 and loly<=al){
			if(ampi.count({lolx,loly}))ampi[{lolx,loly}]++;
			else ampi[{lolx,loly}]=1;
		}
	}
	for(auto dude:ampi)bl-=convert[dude.second]-1;
	//for(auto dude:ampi)out<<dude.first.first<<" "<<dude.first.second<<" "<<dude.second<<nl;
	out<<al*al-bl<<nl;
}
