...
Z az(){
	...
	OKOKOK could maybe do this more simply with 5 simple convolutions to generate the number of each dudes dudes I think um. Like flip flop the bits so that 1 1 counts up the number of 0s and then invert by a a- flip-flop there to get the true value...
	*/
	...
	bvl=VL(a);
	bvvl=VVL(5,VL(a));
	cvvl=VVL(5,VL(0)); // To store convolutions.
	for(c=0;c<a;c++){
		in>>avl[c];
		avl[c+a]=avl[c];
	}
	...
	for(c=0;c<5;c++){
		for(d=0;d<2*a;d++)avvl[c][d]=1-(avl[d]%2);
		for(d=0;d<a;d++)bvvl[c][d]=1-(bvl[d]%2);
		for(d=0;d<2*a;d++)avl[d]/=2;
		for(d=0;d<a;d++)bvl[d]/=2;
	}
	for(c=0;c<5;c++){
		cvvl[c]=convolutionp(avvl[c],bvvl[c]);
		for(d=0;d<sz(cvvl[c]);d++)cvvl[c][d]=a-cvvl[c][d];
		//for(d=0;d<sz(cvvl[c]);d++)out<<cvvl[c][d]<<" ";
		//out<<nl;
		//for(auto dude:cvvl[c])dude=a-dude; // Count of 00s flip-flopped.
	}
	/*
	for(c=0;c<5;c++){
		for(d=0;d<sz(cvvl[c]);d++)out<<cvvl[c][d]<<" ";
	...
	*/
	...
	al=0;
	for(c=a-1;c<a-1+a;c++){
		avl[c]=(cvvl[0][c])+(cvvl[1][c])*2+(cvvl[2][c])*4+(cvvl[3][c])*8+(cvvl[4][c])*16;
		if(avl[c]>al)al=avl[c];
		//out<<avl[c]<<" ";
	}
	out<<al<<nl;
}
