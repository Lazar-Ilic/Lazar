...
*/
	...
	al=103;
	bl=1;
	avl=VL(2*a); // Values modulo p.
	bvl=VL(2*a+1,0); // Partial prefix sums modulo p.
	cvl=VL(2*a); // Reverse...
	dvl=VL(2*a+1,0);
	evl=VL(2*a+1); // Relevant shifter powers.
	for(b=0;b<2*a;b++){
		avl[b]=L(as[b])*bl%p;
		evl[b]=bl;
		bl=(bl*al)%p;
	...
	bl=1;
	for(b=0;b<2*a;b++){
		cvl[b]=L(as[2*a-1-b])*bl%p;
		bl=(bl*al)%p;
	}
	for(b=1;b<=2*a;b++){
		bvl[b]=(bvl[b-1]+avl[b-1])%p;
		dvl[b]=(dvl[b-1]+cvl[b-1])%p;
	}
	// Checking through indices at which we start the reverse dude.
	for(b=0;b<=a;b++){
		//out<<((dvl[2*a-b]-dvl[2*a-b-a])*(evl[b]))%p<< " "<<((bvl[2*a]-bvl[b+a])+(bvl[b]-bvl[0])*(evl[2*a-2*b]))%p<<nl;
		if((((dvl[2*a-b]-dvl[2*a-b-a])*(evl[b]))%p+p)%p==(((bvl[2*a]-bvl[b+a])+(bvl[b]-bvl[0])*(evl[a]))+p)%p){
			bs="";
			for(c=b+a-1;c>=b;c--)bs+=as[c];
			out<<bs<<nl;
			out<<b<<nl;
			return;
		}
