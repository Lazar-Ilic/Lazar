...
/*
Analysis:
This is a super duper chill variant on th canonical Disjoint Set Union offline timed with rollbacks data structure and the canonical Guide To Competitive Programming or Competitive Programmers' Handbook or United States Of America Computing Olympiad Guide websites all have OK codes to ACe this... I ought to be faster and sharper in round and notch this one off prior to the 60 minutes mark on the clock and time for the new Project Leonhard Euler round...

Post Contest Analysis:
Well I might come back to wrap this up but the point is one must be careful around this adsu.rollback call... at that point in time one must carefully similarly modify the uh multiplicities vector and that can be very seriously altered by the rollback calls as each call could make 3 modifications in the underlying if separating some dude in to 2 new old dudes. OKOKOK there exists better and simpler according to the tutorials.
...
*/
	...
	for(c=0;c<b;c++){
		...
		if(d==1){
			in>>e>>f;
			e--;f--;
			multiplicities[adsu.size(e)]--;
			multiplicities[adsu.size(f)]--;
			adsu.join(e,f);
			multiplicities[adsu.size(e)]++;
			timessincelastclear[e].add(c);
			timessincelastclear[f].add(c);
		}
		else{
			in>>e;
			e--;
			for(f=sz(timessincelastclear[e])-1;f>=0;f--)adsu.rollback(timessincelastclear[e][f]+1);
			timessincelastclear[e]=V(0);
		}
		out<<multiplicities[1]<<nl;
	}
}
