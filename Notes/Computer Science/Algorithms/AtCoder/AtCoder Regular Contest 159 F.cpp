...
/*
Think there is some kind of a Left Right partial sum uh Dynamic Programming solution where for each uh as we process through we need to like process when the Left point and uh Right point I think some like 2 pointers through all of the uh different values of dudes somehow like note how ABCA breaks down via not AA but AB and CA break down and like AABA does not break down is like uh AAA can certainly be broken down so it is like I think as long as >=1/2 the dudes are no majority >1/2 we are uh in the clear by like Pigeonhole Principle there exists some config in the break down which has like uh 1 of the semi-majority or whatever and so like the only blocked ones are those uh with a uh majority and then like transitions are implicit in that uh state based Dynamic Programming could crack this task prolly in like 5 hours not 1 so uh 5.0x slower latency productivity hit compared to very top Competitive Programmers here is A OK in my books.
*/
