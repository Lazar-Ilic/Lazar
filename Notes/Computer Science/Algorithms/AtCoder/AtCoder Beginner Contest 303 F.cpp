...
/*
Analysis:
3500ms lowkey here suggests another like uh O[n * log[n] * log[A]] type of approach here searching for the threshold turn value uh... say we wanted to kill the monster by a certain time T then what we need to do is compute the optimal allocation towards killing it by that time T is like uh... is this a uh Convex Hull Trick? Consider the uh uh uh the uh convex hull of these possible potential points is like a uh curve of n points in Z2 is then like uh... how do we order them up prior to the uh time T? Greedy? Think about when precisely a swap is incentivised... push completed dudes as far Right as possible and then sort the incomplete dudes in uh order I guess uh... optimal permutation of some of the uh selected dudes uh? Maybe we can sort all of the dudes in order of uh total damage done so we uh drop the 1st dude to be the maximal total damage etc. etc. and then somehow swap in to order based around the critical time T to swap higher power lower time dudes earlier as desired... maybe we can at each time step simply take the maximum dude who is uh time critical...
