...
Interesting fascinating task really horrible time blow up from me here due to multiplicity uh missing potentially the amortised cost analysis on worst case input because like only so many dudes even have such a high number of factors and also my code is just not sharp compared with some super fast sub 100ms codes I saw on the platform system later sorting on execution time again has to do with array and + rather than * optimisations and so on and so on worth studying prior to high frequency trading firm rounds to be sure.

Test+Additional Input:

[Worst-Case Runtime] Analysis:
Think the usual approach here is to factor each number and also store how many dudes hit each factor uh so that way on the second pass through we hit the maximum factor which has at least K dudes hitting it.
*/
	...
	for(c=0;c<a;c++){
		...
		for(auto dude:avl)bv[dude]++;
		avvl[c]=avl;
	}
	for(c=0;c<a;c++){
		z=-1;
		for(auto dude:avvl[c])if(bv[dude]>=b and dude>z)z=dude;
		out<<z<<nl;
	}
}
