...
*/
	...
	while(!aq.empty()){
		...
		bv=V(0);
		for(auto d:aset){
			if(b>=(avp[d].first-avp[c].first)*(avp[d].first-avp[c].first)+(avp[d].second-avp[c].second)*(avp[d].second-avp[c].second)){aq.push(d);avb[d]=1;bv.add(d);}
		}
		for(auto dude:bv)aset.erase(dude);
	}
	for(auto dude:avb){
		if(dude)out<<"Yes"<<nl;
		else out<<"No"<<nl;
