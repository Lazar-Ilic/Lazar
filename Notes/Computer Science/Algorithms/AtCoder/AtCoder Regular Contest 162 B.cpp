...
Analysis:
Can be optimised further to be sure.
	...
	while(y>1){
		...
		if(y>1){
			if(av[0]==y){
				...
				u=av[1];
				v=av[2];
				av.erase(av.begin()+1,av.begin()+1+2);
				av.insert(av.begin(),v);
				av.insert(av.begin(),u);
				avp.add({2,y-1});
				u=av[1];
				av.erase(av.begin()+1,av.begin()+1+2);
				av.insert(av.begin()+y-1,y);
				av.insert(av.begin()+y-1,u);
			}
			else{
				// Find index of y and simply shift 1 time.
				z+=1;
				x=1;
				while(av[x]!=y)x++;
				avp.add({x,y-1});
				v=av[x-1]; // To insert v and y after deletion.
				av.erase(av.begin()+x-1,av.begin()+x-1+2);
				av.insert(av.begin()+y-1,y);
				av.insert(av.begin()+y-1,v);
			}
		}
	}
	if(av[0]==0){
		out<<"Yes"<<nl;
		out<<z<<nl;
		for(auto dude:avp)out<<dude.first<<" "<<dude.second<<nl;
	}
	else out<<"No"<<nl;
}
