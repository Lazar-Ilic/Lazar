...
Analysis:
Should probably write up like a custom compare function here uh for a tuple a uh triple or uh whatever uh... based around the inequality on long long multiplication or whatever uh...
*/
	...
	for(b=0;b<a;b++){
		in>>c>>d;
		if(c>0 and d>0){e=gcd(c,d);c/=e;d/=e;avpldi.add({LD(d)/(c+d),b+1});}
		else if(c==0)avpldi.add({1,b+1});
		else avpldi.add({0,b+1});
		// Convenient trick here should work to ensure the correct sorting anti sorting ordering.
	}
	sortify(avpldi);
	for(auto dude:avpldi)out<<dude.second<<" ";
	out<<nl;
}
