...
/*
Analysis:
Classical O[T * sqrt[N]] ought to work here casework on the minimum value and Principle Of Inclusion And Exclusion.

Post Contest Analysis:
		//Case on b being the minimum dude ungh add in 1 for the {b,b,b} I guess ungh.
		al=(al+1)%p;
		// Maximum can then be uh between uh b+1 and uh a/b inclusive. Each possible maximum adds in uh 6 more cases and then the uh 6 for each value inside is like...
		al=(al+L(3)*(a/b-1))%p;
		al=(al+L(6)*(a/b-b)*(b-1))%p;
...
*/
	...
	in>>a;
	al=0;
	for(b=1;b*b<=a;b++)al=(al+1+L(3)*(a/b-1)+L(6)*(a/b-b)*(b-1));
	out<<al%p<<nl;
}
