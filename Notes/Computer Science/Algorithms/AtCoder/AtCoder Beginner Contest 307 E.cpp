...
Analysis:
It should be uh roughly M*[M-1]^[N-1]*[M-1]/M ~ [M-1]^[N-1] uh errrm OKOKOK generating functions and exponentiating the underlying uh polynomial and uh uh uh plugging in and canceling might fail but simple recursion should work here basically uh uh uh 2 states uh are relevant uh the uh original or not the original uh so like how many sequences here uh M starting values then uh... 1 and [M-1] terminating and then recurse Dynamic Program works in O[N] or whatever.
*/
	...
	avl=VL(a,0);
	bvl=VL(a,0);
	bvl[0]=1; // 1 way to have the length 1 dude end in original.
	for(c=1;c<a;c++){
		avl[c]=(bvl[c-1]*d+avl[c-1]*e)%p;
		bvl[c]=avl[c-1];
	}
	out<<avl[a-1]*b%p<<nl;
}
