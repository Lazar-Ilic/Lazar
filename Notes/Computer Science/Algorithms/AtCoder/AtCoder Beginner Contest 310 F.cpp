...
*/
	...
	bvl=VL((1<<9),0);
	avl[0]=1;
	al=0; // Answer to Output
	for(b=0;b<a;b++){
		// Process through update Dynamic Program in to bvl
		bvl=VL((1<<9),0);
		in>>c;
		cl=inverse(c);
		for(d=1;d<=min(9,c);d++){
			for(e=0;e<(1<<9);e++){
				if(e ba (1<<(9-d)))al=(al+avl[e]*cl)%p;
				else{
					f=(1<<(d-1));
					for(g=0;g<9;g++){
						if((e ba (1<<g)) and ((f ba (1<<g))==0))f+=(1<<g);
						if((e ba (1<<g)) and (g+d<9) and ((f ba (1<<(g+d)))==0))f+=(1<<(g+d));
					}
					bvl[f]=(bvl[f]+avl[e]*cl)%p;
				}
			}
		}
		if(c>=10){
			cl=0;
			for(d=0;d<(1<<9);d++)cl=(cl+avl[d])%p;
			al=(al+cl*inverse(c))%p;
		}
		if(c>10){
			cl=inverse(c)*(c-10)%p;
			for(d=0;d<(1<<9);d++)bvl[d]=(bvl[d]+avl[d]*cl)%p;
		}
		avl=bvl;
	}
	out<<al<<nl;
