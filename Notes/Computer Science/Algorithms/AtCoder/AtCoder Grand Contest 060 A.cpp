...
Z az(){
	...
	in>>a>>as;
	avvv=VVV(a,VV(26,V(26,0)));
	if(as[0]!='?' and as[1]!='?'){
		avvv[1][as[1]-'a'][as[0]-'a']=1;
	}
	else if(as[0]=='?' and as[1]!='?'){
		for(c=0;c<26;c++){
			if(c!=as[1]-'a')avvv[1][as[1]-'a'][c]=1;
		}
	}
	else if(as[1]=='?' and as[0]!='?'){
		for(c=0;c<26;c++){
			if(c!=as[0]-'a')avvv[1][c][as[0]-'a']=1;
		}
	}
	else{
		for(c=0;c<26;c++){
	...
	for(c=2;c<a;c++){
		if(as[c]=='?'){
			...
			for(d=0;d<26;d++){
				for(e=0;e<26;e++){
					al=(al+avvv[c-1][d][e])%p;
					av[d]=(av[d]+avvv[c-1][d][e])%p;
					bv[e]=(bv[e]+avvv[c-1][d][e])%p;
				}
			}
			for(d=0;d<26;d++){
				for(e=0;e<26;e++){
					if(d!=e)avvv[c][d][e]=(av[e]-avvv[c-1][e][d]+p)%p;
				}
			}
		}
		else{
			for(d=0;d<26;d++){
				for(e=0;e<26;e++){
					if(as[c]-'a'!=d)avvv[c][as[c]-'a'][d]=(avvv[c][as[c]-'a'][d]+(as[c]-'a'!=d and as[c]-'a'!=e)*avvv[c-1][d][e])%p;
				}
			}
		}
	}
