...
/*
I am fairly confident that some kind of canonical lazy lazy segment tree here will work uh processing through the intervals in reverse order direction and maybe uh index compression frankly with another segment tree to do a range summation to as relevant and a Delta actually a underlying Delta shift vector on the range sum when we map in simply do like a Dynamic Program uh interval with a Delta vector and a partially clever uh segment tree.

Along with some kind of like standard uh intervals Dynamic Programming ugh ugh ugh I took such a massive huge hit here today off of the +50 penalty like a moron catastrophic moron wasting penalty and thus tanking my rating on what should be a trivial 0 penalty uh onsight ABDC type of a round here today obviously trivial.
*/
