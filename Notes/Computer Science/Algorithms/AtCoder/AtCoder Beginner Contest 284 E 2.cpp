...
/*
Perhaps we just continually DFS with a live current set until we keep on hitting some dude so only continue if we do not currently include the dude in the live path and then every single path is guaranteed to be distinct in processing order. OK so clearly this implementation will fail so I will bust out the usual canonical DFS style approach here after smashing down that trivial string hashing task.
*/
	...
	while(!ast.empty() and z<1000000){
		...
		for(auto neighbour:graph[a]){
			if(aset.count(neighbour)==0){
				ast.push(neighbour);
				//aset.insert(neighbour);
			}
		}
		aset.erase();
	}
