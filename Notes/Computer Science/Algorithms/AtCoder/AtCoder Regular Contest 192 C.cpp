...
Maybe we want to find the uh... maximum by uh repeatedly trying a new value in our pair and if it grows the sum larger then we keep it else we mark it as tried. This extends our range in both directions or uh rather will extend it yeah yeah it is logically ensured to end up with uh oh P1 < P2 means we can just bash out from P1 P2 bash out until finishing at which point we know that basically uh we end up with P1 P1 so uh ensure that the dude other from P1 must be an extreme point and then just go through and uh... bash out from there. Try every other dude with that dude to get all of the sums and then sort those pairs and then ensure that the 1 and the 2 are in the right order before output. From prefix sums back to values.
*/
	...
	sortify(avvl);
	for(c=0;c<a;c++){
		if(amii[avvl[c][1]]==1 or amii[avvl[c][1]]==2){
			if(amii[avvl[c][1]]==1){
				// Correct Order
				dl=0;
				out<<"! ";
				for(d=0;d<a;d++)out<<amii[avvl[d][1]]<<" ";
				for(d=0;d<a;d++){
					out<<avvl[d][0]-dl<<" ";
					dl=avvl[d][0];
				}
				...
				dl=0;
				out<<"! ";
				for(d=a-1;d>=0;d--)out<<amii[avvl[d][1]]<<" ";
				bvl=VL(a);
				for(d=0;d<a;d++){
					bvl[d]=avvl[d][0]-bl;
					bl=avvl[d][0];
				}
				for(d=a-1;d>=0;d--)out<<bvl[d]<<" ";
				out<<nl;
				return;
			}
		}
