...
/*
OKOKOK so this fails here and we need to be uh more clever with the uh because we might simply take a uh row and column which do not uh intersect at which point we might want to consider like the top uh rows and uh columns somehow uh... not sure what bounds we get here on what was what what relevant ugh in the case that they do not intersect or uh whatever but certainly there is like ensured to be a uh natural easy way to extract out the first uh we could binary search here for like a threshold value and then uh basically find the top N+1 potential viable row + column sums at which point 1 of those is ensured to not actually induce a uh value at which point OKOKOK then we have reduced it in to those cases basically a uh I mean we can like simply search through uh after determining this uh value I think uh works but like I think just sorted VP works.
*/
	...
	for(b=0;b<a;b++){
		in>>c>>d>>e;
		ampi.insert({{c,d},e});
		if(amil.count(c))amil[c]+=e;
		else amil.insert({c,e});
		if(bmil.count(d))bmil[d]+=e;
		else bmil.insert({d,e});
	}
	al=0;
	for(auto dude:ampi){
		if(amil[(dude.first).first]+bmil[(dude.first).second]-dude.second>al)al=amil[(dude.first).first]+bmil[(dude.first).second]-dude.second;
	}
	out<<al<<nl;
}
