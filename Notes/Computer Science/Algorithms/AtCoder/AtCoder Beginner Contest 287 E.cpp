...
Z az(){
	...
	sort(all(avpsi));
	z=0;
	while(z<sz(avpsi[0].first) and z<sz(avpsi[1].first) and (avpsi[0].first)[z]==(avpsi[1].first)[z])z++;
	av[avpsi[0].second]=z;
	z=0;
	while(z<sz(avpsi[a-1].first) and z<sz(avpsi[a-2].first) and (avpsi[a-1].first)[z]==(avpsi[a-2].first)[z])z++;
	av[avpsi[a-1].second]=z;
	for(b=1;b<a-1;b++){
		y=0;
		while(y<sz(avpsi[b].first) and y<sz(avpsi[b-1].first) and (avpsi[b].first)[y]==(avpsi[b-1].first)[y])y++;
		z=0;
		while(z<sz(avpsi[b].first) and z<sz(avpsi[b+1].first) and (avpsi[b].first)[z]==(avpsi[b+1].first)[z])z++;
		av[avpsi[b].second]=max(y,z);
	}
	for(auto dude:av)out<<dude<<nl;
}
