...
*/
	...
	in>>a>>as;
	bs=as;
	reverse(all(bs)); // as reversed... set indices correctly.
	hashinterval ahi(as);
	hashinterval bhi(bs);
	out<<(ahi.gethashinterval(1,3)==bhi.gethashinterval(1,2))<<nl;
	//H ch=ahi.gethashinterval(1,3);

}

#include <sys/time.h>
I main(){zz();timeval tp;gettimeofday(&tp,0);crand=(I)tp.tv_usec;assert((U)(H(1)*2+1-3)==0);I a=1;while(a--){az();}}