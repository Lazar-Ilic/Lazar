...
Z az(){
	...
	/*
	Simply sum over cases without loss of generality we unlock from Left to Right maximally first prior to stars and barsing upon the remainder. But then ugh the actual stars and bars is affected too uh for sure uh so we need to count up that dude too I think every time we add in a new value or whatever is like a new forced dude I think.
	*/
	...
	uniquify(av);
	av.add(1000000);
	d=0; // Minimum unhit value.
	c=0;
	while(c<sz(av) and d==av[c]){
		d++;
		c++;
	}
	al=choose(b+d-1,d-1);
	e=0; // Number of forced shwoops.
	for(;c<sz(av);c++){
		// Process some value or something. Might be easier to use a set or something to remove duplicate values which do not matter ugh. But I think we can while loop or whatever on an underlying value up up and then up up the process I think maybe.
		while(d<av[c] and e<b){
			e++; // Forced shwoop here to unlock this value.
			while(c<sz(av) and d==av[c]-1){
				// Process through upwards once we shwoop shwoop.
				d++;c++;
			}
			al+=choose(b-e+d,d); // b-e more free and d partitions.
			//out<<b-e+d<<" "<<d<<nl;
			d++; // Will end when d==av[c]
		}
	}
	out<<al%p<<nl;
}
