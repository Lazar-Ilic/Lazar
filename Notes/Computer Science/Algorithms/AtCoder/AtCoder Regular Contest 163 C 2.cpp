...
Analysis:
https://www.ics.uci.edu/~eppstein/numth/egypt/kterm-minden.html

So we can certainly make a uh length 23 with maximum uh... 180... 45 with maximum uh like 1000... 90 with maximum uh... 10000... OKOKOK this is AC with maximum uh 10^9 I think uh... maybe uh uh uh...
...
The actual code to half merge off of this is suboptimal would take a little while to write up ugh ugh ugh.
*/
	...
	avv[4]={2,4,6,12};
	avv[5]={2,4,10,12,15};
	avv[6]={3,4,6,10,12,15};
	avv[7]={3,4,9,10,12,15,18};
	avv[8]={3,5,9,10,12,15,18,20};
	avv[9]={4,5,8,9,10,15,18,20,24};
	avv[10]={5,6,8,9,10,12,15,18,20,24};
	avv[11]={5,6,8,9,10,15,18,20,21,24,28};
	avv[12]={6,7,8,9,10,14,15,18,20,24,28,30};
	for(b=3;b<=12;b++)for(auto dude:avv[b])avset[b].insert(dude);
	for(b=13;b<=500;b++){
		c=avv[b-1][b-2];
		z=1;
		while(z){
			c++;
			for(d=0;d<=b-2 and z;d++){
				if(gcd(L(c-avv[b-1][d]),L(c)*avv[b-1][d])==c-avv[b-1][d] and !avset[b-1].count(L(c)*avv[b-1][d]/(c-avv[b-1][d]))){ // So here I think what we want to instead by trying is like literally just uh uh uh maybe bashing pairs like c d pairs uh with c d both greater maybe uh or uh inside uh... but basically bash so that we generate some low pair to insert and some dude to remove each step uh...
					z=0;
					for(e=0;e<=b-2;e++)if(e!=d){
						avv[b].add(avv[b-1][e]);
						avset[b].insert(avv[b-1][e]);
					}
					avv[b].add(c);
					avset[b].insert(c);
					avv[b].add(L(c)*avv[b-1][d]/(c-avv[b-1][d]));
					avset[b].insert(L(c)*avv[b-1][d]/(c-avv[b-1][d]));
					sortify(avv[b]);
				}
			}
		}
