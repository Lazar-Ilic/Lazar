...
Stuff like this code right here in some Trading settings we could look to want to be uh including more interesting double loops around uh like at c<a we might want to sometimes be checking for like uh c<a and z<=1 occasionally in the mixture depending yadda average runtime concerns yadda expected values yadda I need to focus upon Parallel and Low Latency C genuinely and my labour work and Secure The Bag just halt CodeForces soon at Top 2000 not Top 1000 ugh.

Test + Additional Input:


Analysis:
Fantastic task statement frankly love love love rankings lists are funny genre of structure and order on sets...
*/
	...
	for(c=0;c<a;c++){
		if(avb[c]==0){
			z++;
			if(z>=2)break; // Study the compiled execution here e.g.
			y=c;
		}
	}
	if(z==1)out<<y+1<<nl;
