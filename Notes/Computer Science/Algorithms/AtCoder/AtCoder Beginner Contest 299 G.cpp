...
/*
Umm OK I think I fake solved with respect to an O[n * log^2[n]] algorithm here and uh like actually we can just walk Right until we hit a point where there are only uh basically compute a point to which we need to uh pick 1 dude and uh then so basically compute the final Rightmost value b such that uh the dudes in indices [B,N] contains all of the dudes and uh so from there we need to compute the uh Leftmost index of the minimum dude that appears there and uh then go again from that point Rightwards to like B basically and uh recurse but I think this might be a 2-Pointers like a possibly with a freqarray and like a uh O[n] or O[n * log[n]] maybe just O[n] with checking if that freq hit 0 or whatever walking Left and Right maybe uh a O[3n] algorithm with 2-Pointers walking back and forth Left and Right snagging the minimal possible dude and its Leftmost index repeatedly viability in the sorting on the underlying structure of the permutations.
*/
