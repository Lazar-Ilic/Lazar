...
Analysis:
Ugh I misread the task initially and thought it was a query response task over whether or not a given triple is that way which can be done I think rather simply with a Least Common Ancestor structure. Here complementary counting should work doing casework on the number of triples which are on a path and could be uniquely identified by a maximum vertex at which point there are codes to produce the number of vertices in each subtree and sum those products simply in O[n] as well as to sum that number over all subtrees to for down downdown paths. Uh I am really genuinely just feeling so kind of apathetic and lazy here today to actually write it up and might retire from actually coding up AtCoder rounds too retire retire retire from Competitive Programming once I hit CodeForces 2100 just uh you do the usual copypasta Breadth First Search and then Bottoms Up Code to basically store the size of each subtree and then that summation of products can be done in the usual uh Sum of X[Summation-X] is 2* the desired value uh due to overcounting and then for the sub sub dudes is like the Sum of the Summation so for each dude we can Sum the Sums of each dude in a what is like an Integral of the Summations basically in each dude in the Breadth First Search upsolve to do that complementary count. UniqueVision. I am pretty inspired to compose more flawless C codes and Quit Competitive Programming after pinging down the CodeForces 2100 maybe keep it up on Project Leonhard Euler once in a while.
*/
