...
Z az(){
	...
	//out<<av[sz(av)-1]<<nl;
	// Ugh actually want maximum suffix of as which is a subsequence of bs means just simply O[n] traverse from the Right ends of both strings I think to produce that target value...
	z=0;
	e=a-1;
	f=a-1; // 2 pointers
	while(e>=0 and f>=0){
		while(f>=0 and bs[f]!=as[e]){
			f--;
		}
		if(e>=0 and f>=0 and as[e]==bs[f]){
			z++;
			e--;
			f--;
		}
