...
*/
	in>>al>>a;
	SETP asetp;
	avp={{-2,-1},{-1,-2},{1,2},{2,1},{-1,2},{-2,1},{1,-2},{2,-1},{0,0}};
	for(b=0;b<a;b++){
		in>>c>>d;
		for(auto dude:avp)if(c+dude.first>=1 and c+dude.first<=al and d+dude.second>=1 and d+dude.second<=al)asetp.insert({c+dude.first,d+dude.second});
	}
	out<<al*al-sz(asetp)<<nl;
}
