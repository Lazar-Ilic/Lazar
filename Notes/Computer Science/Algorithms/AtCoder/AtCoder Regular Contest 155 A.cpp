...
Z az(){
	...
	if(z){
		if((al+a)%(2*a)<=a){
			cs=as.substr(0,(al+a)%(2*a));
		}
		else{
			cs=as+bs.substr(0,(al+a)%(2*a)-a);
		}
	}
	else{
		if((al+a)%(2*a)<=a){
			cs=bs.substr(0,(al+a)%(2*a));
		}
		else{
			cs=bs+as.substr(0,(al+a)%(2*a)-a);
		}
	}
	z=1;
	for(b=0;b<sz(cs)/2 and z;b++){
		if(cs[b]!=cs[sz(cs)-1-b])z=0;
	}
	if(z){
		out<<"Yes"<<nl;
	}
