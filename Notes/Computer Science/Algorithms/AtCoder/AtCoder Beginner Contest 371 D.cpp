...
*/
	...
	bvl=VL(a);
	bvl[a-1]=avl[a-1];
	for(b=a-2;b>=0;b--)bvl[b]=bvl[b+1]+avl[b];
	in>>a;
	for(b=0;b<a;b++){
		in>>c>>d;
		if(lowerbound(all(av),c)!=av.end()){
			if(upperbound(all(av),d)!=av.end())out<<bvl[lowerbound(all(av),c)-av.begin()]-bvl[upperbound(all(av),d)-av.begin()]<<nl;
			else out<<bvl[lowerbound(all(av),c)-av.begin()]<<nl;
		}
		else{
			out<<0<<nl;
		}
