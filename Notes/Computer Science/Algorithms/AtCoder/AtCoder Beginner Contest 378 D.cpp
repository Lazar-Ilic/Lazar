...
*/
	...
	for(d=0;d<a;d++){
		for(e=0;e<b;e++){
			if(avv[d][e])aqvp.push({{d,e}});
			while(!aqvp.empty()){
				avp=aqvp.front();
				aqvp.pop();
				if(sz(avp)==c+1)al++;
				else{
					ap=avp[sz(avp)-1]; // Back Element
					if(avp.end()==find(avp.begin(),avp.end(),P{ap.first+1,ap.second}) and ap.first+1<a and avv[ap.first+1][ap.second]){
						avp.push_back({ap.first+1,ap.second});
						aqvp.push(avp);
						avp.pop_back();
					}
					if(avp.end()==find(avp.begin(),avp.end(),P{ap.first-1,ap.second}) and ap.first-1>=0 and avv[ap.first-1][ap.second]){
						avp.push_back({ap.first-1,ap.second});
						aqvp.push(avp);
						avp.pop_back();
					}
					if(avp.end()==find(avp.begin(),avp.end(),P{ap.first,ap.second+1}) and ap.second+1<b and avv[ap.first][ap.second+1]){
						avp.push_back({ap.first,ap.second+1});
						aqvp.push(avp);
						avp.pop_back();
					}
					if(avp.end()==find(avp.begin(),avp.end(),P{ap.first,ap.second-1}) and ap.second-1>=0 and avv[ap.first][ap.second-1]){
						avp.push_back({ap.first,ap.second-1});
						aqvp.push(avp);
						avp.pop_back();
					}
