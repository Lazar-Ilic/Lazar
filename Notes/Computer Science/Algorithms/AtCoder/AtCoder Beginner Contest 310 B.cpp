	...
	for(c=0;c<a;c++){
		for(d=c+1;d<a;d++){
			if(av[c]==av[d]){
				...
				z=0;
				for(e=0;e<b;e++){
					if(avvb[c][e] and !avvb[d][e])y=1;
					if(!avvb[c][e] and avvb[d][e])z=1;
				}
				if(y+z==1){
					out<<"Yes"<<nl;
					return;
				...
				z=1;
				for(e=0;e<b and z;e++)if(avvb[c][e] and !avvb[d][e])z=0;
				if(z){
					out<<"Yes"<<nl;
					return;
				}
				...
				z=1;
				for(e=0;e<b and z;e++)if(avvb[d][e] and !avvb[c][e])z=0;
				if(z){
					out<<"Yes"<<nl;
					return;
				}
