...
Ugh and then it hits me the next morning as I am re reading solutions that one could flip the failed test cases vertically I think and so indeed for proof and logical soundness one indeed ought to go in both directions in the BFS to produce all the states which actually are part of a valid start to finish sequence and then check them. And also it is easier to immediately check the whole boaard for 0 status and then remove the l condition entirely. Well anyways seems like extremely weak test cases writing and shameful from Meta Hacker Cup essentially if this backwards algorithm did work in round because they should have forced ensured the logically valid tight thing I have since added in to my Algorithms notes here in general.
*/