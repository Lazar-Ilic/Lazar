...
Analysis:
Simple Leftmost 1 bit analysis reveals that the Game Theoretically Optimal Strategy for any configuration certainly requires we simply Flip the Leftmost 1 bit repeatedly until the string is completed. It might require more complex analysis or data structures to consider when additional dudes are not simply negated but rather merged backwards in an easier way however that should most likely handle this case reasonably well. Ensuring to have at most 1 of each possible dude so that way in the break down we get like a relatively simple harmonic series is O[16 * MAX_N] is OK.
*/

	in>>a>>as>>b;
	bvb=VB(a+1);
	for(c=1;c<=a;c++)bvb[c]=as[c-1]-'0';
	al=0;
	z=0;
	cvb=VB(a+1);
	for(c=1;c<=a;c++)if(bvb[c]){z++;cvb[c]=1;for(d=c;d<=a;d+=c)bvb[d]=1-bvb[d];}
	avb=VB(a+1);
	for(c=0;c<b;c++){
		in>>d;
		avb[d]=1-avb[d];
