...
Z az(){
	...
	/* For D2 one supposes that a simple frequency counter of multiplicities
	of 1s and 2s is enough to instantly compute if it is possible. Then
	perhaps from there a simple like O[log^2[n]] check for Left Right indices
	thresholds inside of which to shwip shwoop around swapping around 2s and
	1s Left Right around the Z_i index. */
