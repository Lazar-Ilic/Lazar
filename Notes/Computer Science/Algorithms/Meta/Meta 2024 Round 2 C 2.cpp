...
*/
    in>>a>>b>>c;
	avv=VV(a,V(b));
	bvv=VV(a+1,V(b+1)); // For Insertions And Deletions Later
	cvv=VV(a+1,V(b+1)); // For Up Left Sums Later
	dvv=VV(a+1,V(b+1));
	...
	while(l<r){
		...
		for(d=0;d<sz(avvp);d++){
			if(sz(avvp[d])>3000){
				...
				for(e=1;e<=a;e++)for(f=1;f<=b;f++)cvv[e][f]=cvv[e][f-1]+bvv[e][f];
				for(e=1;e<=a;e++)for(f=1;f<=b;f++)cvv[e][f]=cvv[e-1][f]+cvv[e][f];
				for(auto dude:avvp[d])lol-=cvv[min(a,dude.first+m)][min(b,dude.second+m)]+cvv[max((L)0,dude.first-m-1)][max((L)0,dude.second-m-1)]-cvv[max((L)0,dude.first-m-1)][min(b,dude.second+m)]-cvv[min(a,dude.first+m)][max((L)0,dude.second-m-1)]-1;
				for(auto dude:avvp[d])bvv[dude.first+1][dude.second+1]=0;
			}
			else if(sz(avvp[d])>0){
