...
*/
	...
	while(l<r){
		m=(l+r)/2;
		avb=VB(a);
		for(b=0;b<a;b++)if(av[b]<=m or (b>0 and avb[b-1] and abs(av[b]-av[b-1])<=m))avb[b]=1;
		for(b=a-1;b>=0;b--)if(av[b]<=m or (b<a-1 and avb[b+1] and abs(av[b]-av[b+1])<=m))avb[b]=1;
		B ab=1;
		for(b=0;b<a and ab;b++)if(avb[b]==0)ab=0;
		if(ab)r=m;
		else l=m+1;
	}
	out<<l<<nl;
