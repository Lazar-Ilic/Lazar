...
Z az(){
	...
	/* Strict convex hull vertex. At most. K, D < 10^9
	K is pretty low of course in the context of a square.
	Dynamic Program Bottoms Up Left To Right Minimum?
	Strictly Dominating Inequality?
	All Vertices Input Are On The Convex Hull?
	DAG? O[n log n] Works?
	Only need to push maximally.
	Top and Bottom are separate but can be together. 
	Still only push up a K maximally on both Top and Bottom.
	Uh wait that might not be True actually.
	OH wait Maximum OK.
	Uh OK triangle inequality.
	In obtuse gives bounds in this Dynamic Program.
	Should not matter just process left to right.
	And have another side case for the at most 1 acute angle in the
	convex hull on Top and 1 acute angle on Bottom.
	Not really feeling like executing it. */
