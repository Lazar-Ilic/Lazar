...
*/
	...
	in>>al>>bl>>cl;
	if(cl<al and cl<bl)out<<0<<nl;
	else if(bl<=al)out<<2*(cl/bl)-1<<nl;
	else if(cl<al)out<<0<<nl;
	else if(2*al<=bl)out<<cl/al<<nl;
	else out<<max(1+2*(cl-al)/bl,2+2*(cl-2*al)/bl)<<nl;
}
