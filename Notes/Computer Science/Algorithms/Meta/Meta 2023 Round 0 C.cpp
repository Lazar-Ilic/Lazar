...
*/
	...
	in>>a;
	av=V(2*a-1);
	for(b=0;b<2*a-1;b++)in>>av[b];
	if(a==1){out<<1<<nl;return;}
	sort(all(av));
	bv=V(2*a-2); // Can be done differently to save memory.
	for(b=0;b<2*a-2;b++)bv[b]=av[b+1]-av[b];
	m=INT_MAX;
	z=1;
	for(b=1;b<a-1 and z;b++)if(bv[b]!=bv[2*a-2-b])z=0;
	if(z)m=av[2*a-2]+bv[0];
	z=1;
	for(b=0;b<a-2 and z;b++)if(bv[b]!=bv[2*a-2-2-b])z=0;
	if(z and 1<=av[0]-bv[2*a-3])m=av[0]-bv[2*a-3];
	z=1;
	for(b=0;b<a-1 and z;b++){
		if(bv[b]!=bv[2*a-2-1-b]){
			z=0;
			if(bv[b]>bv[2*a-2-1-b]){
				y=av[b]+bv[2*a-2-1-b];
				// Check for correctness.
				x=1;
				bv[b]=bv[b]-bv[2*a-2-1-b];
				for(c=b;x and c<a-2;c++)if(bv[c]!=bv[2*a-2-1-1-c])x=0;
				if(!x)y=INT_MAX;
				if(x or m<INT_MAX){
					out<<min(y,m)<<nl;
					return;
				...
				y=av[2*a-2-1-b+1]-bv[b];
				// Check for correctness.
				x=1;
				bv[2*a-2-1-b]=bv[2*a-2-1-b]-bv[b];
				for(c=b+1;x and c<a-1;c++)if(bv[c]!=bv[2*a-2-1+1-c])x=0;
				if(!x)y=INT_MAX;
				if(x or m<INT_MAX){
					out<<min(y,m)<<nl;
					return;
	...
	if(z){
		y=1;
		for(b=1;b<a-1 and y;b++)if(bv[b]!=bv[0])y=0;
		if(y and 1<=2*av[0]-av[1]){
			out<<2*av[0]-av[1]<<nl;
			return;
		}
		out<<av[a-1]<<nl;
