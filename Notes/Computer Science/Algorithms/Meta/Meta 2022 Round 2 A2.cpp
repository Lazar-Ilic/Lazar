...
Z az(){
	...
	for(c=0;c<b;c++){
		in>>d>>e>>f;
		if(d==1)asg.update(e-1,lolol[f]);
		else{
			e--;f--;
			if((f-e)%2==0 and ausetl.count(asg.query(e,e+(f-e)/2)-asg.query(e+(f-e)/2+1,f)) or ausetl.count(asg.query(e+(f-e)/2,f)-asg.query(e,e+(f-e)/2-1)))z++;
		}
	}
	out<<z<<nl;
}
