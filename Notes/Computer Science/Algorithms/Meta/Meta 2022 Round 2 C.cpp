...
L inversefactorials[9000001];
I inversefactorialsaux[9000001];
...
Z az(){
	...
	av=V(a);
	bv=V(a);
	d=0; // Number of cookies.
	e=0; // Number of cookies <
	f=0; // Number of cookies ==
	g=0; // Number of cookies >
	for(c=0;c<a;c++){
		in>>av[c];
		in>>bv[c];
		d+=av[c];
		if(bv[c]==bv[0])f+=av[c];
		else if(bv[c]<bv[0])e+=av[c];
		else g+=av[c];
	}
	al=(choose(e+f,b+1)-choose(e,b+1)+p)%p;
	bl=choose(d,b+1);
	out<<al*inverse(bl)%p*av[0]%p*inverse(f)%p<<nl;
	// Post Contest Analysis: This can be very easily 1 linerised.
}
