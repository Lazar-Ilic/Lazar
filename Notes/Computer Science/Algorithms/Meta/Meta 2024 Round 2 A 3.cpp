...
And it probably ought to also be noted that on top of using adds instead of multiplications here we could also potentially instead go through and like factorise every single dude in avl if needed rather than ugh uh just looping through for each potential modulus in O[100000000] or whatever could be done faster possibly ungh ready to go in one fell swoop like with a map and a uh uh uh huge uh O[100000000*10^(18/4)] could work or better with some sharper general tricks for more factorising dudes or something depending where whom wants to store which precomputed auxiliary memory vector.
