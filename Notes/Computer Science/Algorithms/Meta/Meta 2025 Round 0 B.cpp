...
*/
	...
	while(!adq.empty()){
		...
		g=i%b;
		h=av[i]+1;
		if(e and av[i-b]>h){av[i-b]=h;adq.push_back(i-b);}
		if(e+1<a and av[i+b]>h){av[i+b]=h;adq.push_back(i+b);}
		if(g and av[i-1]>h){av[i-1]=h;adq.push_back(i-1);}
		if(g+1<b and av[i+1]>h){av[i+1]=h;adq.push_back(i+1);}
	}
	bv=V(n);
	cv=V(n);
	for(e=0;e<a;e++){
		...
		for(g=0;g<b;g++){
			...
			l=g+1;
			r=b-g;
			w=min(min(j,k),min(l,r));
			if(avs[e][g]=='.' and min(w,av[i])>=d)bv[i]=1;
		}
	}
	I answer=0;
	for(i=0;i<n;i++)if(bv[i] and !cv[i]){
		cv[i]=1;
		bdq.push_back(i);
		s=0;
		while(!bdq.empty()){
			...
			e=j/b;
			g=j%b;
			auto push=[&](int k){if(bv[k] and !cv[k]){cv[k]=1;bdq.push_back(k);}};
			if(e)push(j-b);if(e+1<a)push(j+b);
			if(g)push(j-1);if(g+1<b)push(j+1);
		}
