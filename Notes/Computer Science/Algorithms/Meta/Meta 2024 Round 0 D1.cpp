...
Can do this instead with an indicator array of booleans.

Test + Additional Input:


[Worst-Case Runtime] Analysis:
Tanked on D2 rather than even reading D1 today... also got caught up in some Elite Code skimming for C rather than OnSighting off of a right initial read.
*/
	in>>a>>b;
	av=V(a);
	int dist=INT_MAX;
	...
	for(c=0;c<a;c++){
		in>>d;
		av[c]=d;
		if(abs(b-d)<dist or abs(b-d)==dist and d>loc){
			dist=abs(b-d);
			loc=d;
		}
	}
	int ind=0;
	for(c=0;c<a;c++)if(av[c]>=loc)ind++;
	out<<ind<<" "<<dist<<nl;
}
