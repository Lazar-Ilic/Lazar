...
Z az(){
	...
	in>>a>>b>>c;
	// int to long long overflow. LOL misread and "IMPOSSIBLE"!="Impossible"
	maxflow amax(3*a+1);
	maxflow bmax(3*a+1);
	for(d=0;d<b;d++){
		in>>e>>f>>g;
		amax.addedge(e,f+a,g);
		amax.addedge(f,e+a,g);
		amax.addedge(e+a,f+2*a,g);
		amax.addedge(f+a,e+2*a,g);
		amax.addedge(e,f+2*a,g);
		amax.addedge(f,e+2*a,g);
		bmax.addedge(e,f+a,g);
		bmax.addedge(f,e+a,g);
		bmax.addedge(e+a,f+2*a,g);
		bmax.addedge(f+a,e+2*a,g);
		bmax.addedge(e,f+2*a,g);
		bmax.addedge(f,e+2*a,g);
	}
	for(d=0;d<c;d++){
		in>>e>>f;
		maxflow cmax=amax;
		maxflow dmax=bmax;
		out<<" "<<cmax.generate(e,f+2*a)+dmax.generate(e,f+a);
	}
	out<<nl;
}
