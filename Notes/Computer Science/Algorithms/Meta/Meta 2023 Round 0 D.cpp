...
Analysis:
So there are effectively like 2 clones of the graph for each foot and so there is a new graph for the underlying left right at which point it is not like a simple O[n^3] min paths all pairs style of algorithm here is still Time Limit Exceeded of course so... we need to be more clever about minimising the minimum number of edges relating to any odd cycle I think or rather odd paths somehow should be trivialised. Potentially this is one of those bridge cut Strongly Connected Components type of tasks.
*/
