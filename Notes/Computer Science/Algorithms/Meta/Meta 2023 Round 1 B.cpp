...
void printAllUniqueParts(I n){
	...
	while(true){
		...
		if(al<=1000000000){
			I bbb=al;
			if(lolol.count(bbb)==0 or sz(lolol[bbb])>k+1){
				V lololol;
				for(int aaa=0;aaa<=k;aaa++)lololol.add(ppppppppp[aaa]);
				lolol[bbb]=lololol;
			}
		}
		I remains=0;
		while (k>=0 && ppppppppp[k]==1){
			remains+=ppppppppp[k];
			k--;
		}
		if(k<0)return;
		ppppppppp[k]--;
		remains++;
		while(remains>ppppppppp[k]){
			ppppppppp[k+1]=ppppppppp[k];
			remains=remains-ppppppppp[k];
			k++;
		}
		ppppppppp[k+1]=remains;
		k++;
	}
}
