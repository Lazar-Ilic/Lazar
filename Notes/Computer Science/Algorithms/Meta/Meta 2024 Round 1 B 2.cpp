...
Easier to use upperbound here on a simple 1 time construction call for much faster performance.

Test+Additional Input:


[Worst-Case Runtime] Analysis:
3 5 7 11 introduces uh 2 to 2 4 then updates to 2 4 6 8 updates to 2 through 10... 17 extends... and it is a theorem it keeps extending further...
*/
	if(testcase==1){
		I maximum=10000001; // SSRS_
		V spf(maximum+1,-1);
		V primes;
		for(a=2;a<=maximum;a++){
			if(spf[a]==-1){
		...
		lolol=primes;
		ololo=V(0);
		for(a=1;a<sz(lolol);a++)if(lolol[a]-lolol[a-1]==2)ololo.add(lolol[a]);
	}
	in>>a;
	if(a<=4){
		out<<0<<nl;
	...
	out<<1+upperbound(all(ololo),a)-ololo.begin()<<nl;
}
