...
Z az(){
	...
	for(b=0;b<a;b++){
		...
		cv=V(26);
		dv=V(26);
		// Take left half and right half.
		if((d-c)%2==0){
			for(e=0;e<26;e++){
				cv[e]=avv[d][e]-avv[d-(d-c)/2][e];
				dv[e]=avv[c-1+(d-c)/2][e]-avv[c-1][e];
			}
			f=0; // Number of disagreement.
			g=-1; // Indices of disagreement.
			h=-1;
			for(e=0;e<26;e++){
				if(cv[e]!=dv[e]){
					f++;
					...
					if(abs(cv[e]-dv[e])>1)f=3;
				}
			}
			if(f==0)z++;
			else if(f==2 and ((g==as[c-1+(d-c)/2]-'a') or (h==as[c-1+(d-c)/2]-'a')))z++;
		}
	}
	out<<" "<<z<<nl;
}
