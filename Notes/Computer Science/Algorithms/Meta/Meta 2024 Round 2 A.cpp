...
So this is worth writing up on later. I have not benchmarked but my code might be OK with taking in the powers of 10 in to the relevant for loops and directly adding so that all operations involved are additions except for the setting of the bottom of the loops which is a multiplication operation. Another way to do this would be to track like build up from rightmost digit to leftmost and store a vector of long longs with their lead digit in a pair yeah yeah and then similarly like shift up a bump on the underlying digit with the power of 10 in in a uh dude loops in to force only adds there kind of works rather than any multiplication operations involved so something much slicker and less nasty error prone potentially than writing in 100 for loops kind of semi manual with a semi automated checker or even could have used a uh code generating code here to ace the construction of the for loops could have been half slick and so on and so on.
...
*/
	...
	// Generate All Peaks In avl And Insert In To bvvl
	avl=VL(0);
	cvl=VL(17);
	cvl[0]=1;
	for(b=1;b<17;b++)cvl[b]=10*cvl[b-1];

	for(b=1;b<10;b++)for(c=b;c<10;c++){
		bl=0;
		d=0;
		for(e=b;e<=c;e++){
			bl+=e*cvl[d];
