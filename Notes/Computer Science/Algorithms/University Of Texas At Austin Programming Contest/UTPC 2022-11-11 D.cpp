...
V stringmatch(S& t,S& q){ // find indices where q occurs in t
	...
	build(q,pi);
	I k=-1;
	for(I i=0;i<t.length();i++){
		while(k>=-1 and q[k+1]!=t[i])
			k=(k==-1)?-2:pi[k];
		k++;
		if(k==q.length()-1){ //match found
			av.add(i-k);
			k=(k==-1)?-2:pi[k];
		}
	}
...
Z az(){
	...
	in>>bs;
	cv=stringmatch(as,bs);
	if(sz(cv)>0){
		out<<sz(as)<<nl;
		return;
	}
	cv=stringmatch(bs,as);
	if(sz(cv)>0){
		out<<sz(bs)<<nl;
		return;
	}
	...
	m=0;
	for(n=sz(av)-1;n>=0;n--){
		if(av[n]==sz(av)-n or bv[n]==sz(av)-n)m=sz(av)-n;
	}
	out<<sz(as)+sz(bs)-m<<nl;
}
