...
VP graph[200000];// adjacency list of {neighbour, edge weight}
L distances[200000];
I numvertices;
Z minpaths(I source){
...
Z az(){
	...
	sort(all(avp));
	for(b=0;b<a-1;b++){
		// In between b and b+1 solve for 1 point.
		ad=avp[b].first;
		bd=avp[b+1].first;
		for(c=0;c<30;c++){
			cd=(ad+bd)/2;
			...
			ed=0;
			for(d=0;d<=b;d++){
				dd+=D(avp[d].second)/((cd-avp[d].first)*(cd-avp[d].first));
			}
			for(d=b+1;d<a;d++){
				ed+=D(avp[d].second)/((avp[d].first-cd)*(avp[d].first-cd));
			}
			if(dd>ed)ad=cd;
			else bd=cd;
		}
		out<<setprecision(12)<<cd<<nl;
	}
}
