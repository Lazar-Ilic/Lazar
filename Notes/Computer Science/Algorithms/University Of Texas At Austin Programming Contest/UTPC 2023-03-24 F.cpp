...
Z az(){
	...
	/*
	Simple as processing in order uh the maximum rightwards forms an interval and then the next maximum to the Left of that dude Rightward to that dude Etc.
	*/
	...
	sort(all(avp));
	d=a; // Rightmost point.
	for(e=sz(avp)-1;e>=0;e--){
		while(e>=1 and avp[e].first==avp[e-1].first)e--;
		if(avp[e].second<d){
			bv.add(d-avp[e].second);
			d=avp[e].second;
		}
	}
	sort(all(bv));
	z=0;
	for(e=sz(bv)-1;e>=max(0,sz(bv)-b);e--)z+=bv[e];
	out<<z<<nl;
}
