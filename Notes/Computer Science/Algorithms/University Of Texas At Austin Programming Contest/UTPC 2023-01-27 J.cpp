...
Z az(){
	...
	sort(all(avl));
	SG asg(c-1);
	for(d=0;d<c-1;d++)asg.update(d,0);
	for(d=0;d<b;d++){
		in>>e>>f>>g;
		if(e>1){
			asg.update(e-2,asg.query(e-2,e-2)+g);
		}
		if(f<c){
			asg.update(f-1,asg.query(f-1,f-1)-g);
		}
		//out<<asg.query(0,c-2)<<nl;
		out<<(avl.end())-(lowerbound(avl.begin(),avl.end(),asg.query(0,c-2)))<<nl;
	}
}
