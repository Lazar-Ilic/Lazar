...
Z az(){
	...
	in>>a>>b;
	avv=VV(a+1,V(a+1,INT_MAX));
	av=V(a+1,INT_MAX);
	for(c=0;c<b;c++){
		in>>d>>e>>f>>g;
		avv[d][e]=f-g;
	}
	priority_queue<pair<int,int>,vector<pair<int,int>>,greater<pair<int,int>>> apqp;
	apqp.push({0,1});
	while(!apqp.empty()){
		ap=apqp.top();
		apqp.pop();
		if(ap.first<av[ap.second]){
			av[ap.second]=ap.first;
			for(c=1;c<=a;c++){
				if(avv[ap.second][c]!=INT_MAX and av[c]>max(0,ap.first+avv[ap.second][c]))apqp.push({max(0,ap.first+avv[ap.second][c]),c});
			}
		}
	}
	if(av[a]!=INT_MAX)out<<av[a]<<nl;
	else out<<-1<<nl;
}
