...
Z az(){
	...
	avl.add(10000000000000000);
	priority_queue<pair<int,int>,vector<pair<int,int>>,greater<pair<int,int>>> apqp;
	for(c=0;c<b;c++){
		if(avl[lower_bound(avl.begin(),avl.end(),avl[bv[c]-1]+cv[c])-avl.begin()]!=10000000000000000)apqp.push({lower_bound(avl.begin(),avl.end(),avl[bv[c]-1]+cv[c])-avl.begin()-1,bv[c]-1});
	}
	ap={-1,-1};
	z=0;
	while(!apqp.empty()){
