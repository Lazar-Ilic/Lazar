...
I LPS(S as){
	...
	while(i<sz(as)){
		if(as[i]==as[len]){
			...
			if(len!=0)len=av[len-1];
			else{av[i]=len;i++;}
		}
	}
	if(av[sz(av)-1]==0)return sz(av);
	// Consider like the ABCDABABCDABABCDAB case and so on and so on is like uh going to produce here like a 00001212345678 so I think that like the uhh final instance of a uh 1 might indicate the actual loopy component I think ugh.
	else{
		I z=sz(av)-1;
		while(z>=1 and av[z]!=1)z--;
		if(z==0)return sz(av);
		for(I y=0;y<sz(av);y+=z)if((z<=sz(av)-y and as.substr(y,z)!=as.substr(0,z)) or (z>sz(av)-y and sz(av)-y>0 and as.substr(y,sz(av)-y)!=as.substr(0,sz(av)-y)))return sz(av); // Oh uh to be sure this is AC O[n] code right here should work OKOKOK but maybe in a better setting for this task we hash out the whole string thus far somehow to improve efficiency and performance and runtime.
		return max(1,z);
	}
}
