...
*/
	...
	*/
	...
	assc.generate(a);
	av=V(a+1); // Size Of Relevant SSC
	for(c=1;c<=a;c++)av[assc.componentof[c]]++;
	avv=VV(a+1,V(0)); // Graph on SSCs identities.
	al=0; // Maximum Sum Thus Far
	avl=VL(a+1,0); // Maximum Values Into SSCs
	for(auto dude:avp){
	...
	bv=topsort(avv); // Sorted order of processing.
	//outv(av);
	//outv(bv);
	for(c=0;c<sz(bv);c++){
		// Process bv[b] element in SSCs topologically sorted chain.
		d=bv[c]; // Element SSC index.
		avl[d]+=av[d];
		//out<<"avld "<<d<<" "<<avl[d]<<nl;
		if(avl[d]>al)al=avl[d];
		for(auto dude:avv[d])avl[dude]=max(avl[dude],avl[d]);
	}
	out<<al<<nl;
}
