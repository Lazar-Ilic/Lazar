...
Z az(){
	...
	*/
	...
	for(d=0;d<b;d++){
		in>>e>>f;
		bvv[e][f]=0;
		if(e+1<1000)bvv[e+1][f]=0;
		if(e-1>=0)bvv[e-1][f]=0;
		if(f+1<1000)bvv[e][f+1]=0;
		if(e+1<1000 and f+1<1000)bvv[e+1][f+1]=0;
		if(e-1>=0 and f+1<1000)bvv[e-1][f+1]=0;
		if(f-1>=0)bvv[e][f-1]=0;
		if(e+1<1000 and f-1>=0)bvv[e+1][f-1]=0;
		if(e-1>=0 and f-1>=0)bvv[e-1][f-1]=0;
	}
	for(d=0;d<1000;d++){
		avv[d][0]=bvv[d][0];
		for(e=1;e<1000;e++)avv[d][e]=avv[d][e-1]+bvv[d][e];
	}
	for(d=0;d<1000;d++){
		for(e=1;e<1000;e++)avv[e][d]=avv[e-1][d]+avv[e][d];
	}
	for(d=0;d<c;d++){
		in>>e>>f>>g>>h;
		al=avv[g][h];
