...
*/
	...
	/*
	al=0;
	vl=VL(a+1); // 1000000000 overflows here and sparse lazy is not needed. Uh I think that one could intead of sparse lazy use index compression with perhaps some map from integers to long longs and use a lazy rather than a sparse lazy basically. Perhaps an index compression based around uh another easily searchable sorted vector or whatever to process through those queries later.
	...
	*/
	...
	avv=VV(a,V(4));
	for(c=0;c<a;c++){
		in>>avv[c][0]>>avv[c][1]>>avv[c][2]>>avv[c][3];
	}
	av=V(b);
	...
	//outv(av);
	for(c=0;c<a;c++){
		al+=L(avv[c][2]-avv[c][0]+1)*((avv[c][3]-avv[c][1]+1)-(upperbound(av.begin(),av.end(),avv[c][3])-lowerbound(av.begin(),av.end(),avv[c][1])));
	}
	out<<al<<nl;
}
