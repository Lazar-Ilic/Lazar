...
Analysis:
Basically think if we take like for example uh... in 2D uh R2 uh Z2 uh... basically uh uh uh like the point where bi-ai is maximised uh or uh we want like 2 points to be very far from the line y=x basically uh a flip flop on an induced rectangle... need a simple sort and delta kind of maximum approach here should be simple to trawl maybe Google around a little bit ugh... basically uh if we were to uh sort by uh... the sum of the coordinates and the raw difference uh... formula ugh for a rectangle ugh... or just some simple casework inequality on whether or not uh maybe we could honestly consider sorting on the uh... a value even uh... think even just sorting and uh sort sort works here ugh ugh ugh for my Masterhood finally ugh...
*/
	...
	for(c=0;c<a;c++)al+=abs(av[c]-bv[c]);
	for(c=0;c<a;c++){
		avp.add({av[c]-bv[c],av[c]+bv[c]});
	}
	sortify(avp);
	// Think just store like 2 dudes here ap and bp which maximise and minimise some simple metric here thus far for comparison with future dudes in terms of the underlying Deltas works.
	ap=avp[0];
	bp=avp[0];
	bl=0;
	for(c=1;c<a;c++){
		// Process avp[c] accordingly with respect to ap and bp and update ap and bp.
		cp=avp[c];
		e=(ap.first+ap.second)/2;
		f=(ap.second-ap.first)/2;
		g=(bp.first+bp.second)/2;
		h=(bp.second-bp.first)/2;
		i=(cp.first+cp.second)/2;
		j=(cp.second-cp.first)/2;
		if(abs(i-e)+abs(j-f)-(abs(i-j)+abs(e-f))>bl)bl=abs(i-e)+abs(j-f)-(abs(i-j)+abs(e-f));
		if(abs(i-g)+abs(j-h)-(abs(i-j)+abs(g-h))>bl)bl=abs(i-g)+abs(j-h)-(abs(i-j)+abs(g-h));
		if(cp.second+cp.first<ap.second+ap.first)ap=cp;
		if(cp.second-cp.first<bp.second-bp.first)bp=cp;
	}
	out<<al+bl<<nl;
}
