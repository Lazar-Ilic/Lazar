...
*/
	...
	g=e-c;
	if(g%2==1){
		// Can Alice win? Isomorphs in to range covering question.
		if(max(1,d-(g/2+1))<=max(1,f-g/2) and min(b,d+(g/2+1))>=min(b,f+g/2))out<<"Alice"<<nl;
		else out<<"Draw"<<nl;
	}
	else{
		// Can Bob win?
		if(max(1,d-g/2)>=max(1,f-g/2) and min(b,d+g/2)<=min(b,f+g/2))out<<"Bob"<<nl;
		else out<<"Draw"<<nl;
	}
}
