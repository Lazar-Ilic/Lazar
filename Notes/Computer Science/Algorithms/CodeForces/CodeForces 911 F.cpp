...
using L=unsigned long long; // 64 bits = 8 bytes | [-9223372036854775808,9223372036854775807] ~ [-10^19,10^19]
...
Analysis:
Probably the usual like Left Right Right Left Dynamic Programming in conjunction with some kind of a uh analysis of intervals in particular a break down ab initio in to the increasing and decreasing subsequences should help reveal that in fact the answer is like related to the maximum chain length inside of the bounds and some other properties which could be queried in O[log n] for a total of runtime O[q * log[n]] or something on the edge analysis inside the chains breakdown composition inside of each bounds or whatever...
*/

	in>>a>>b;
	av=V(a);
	for(c=0;c<a;c++)in>>av[c];
	// Analyse the initial array in terms of breakdowns in to increasing and decreasing chains and set up auxiliary relevant data structure here to query.
	for(c=0;c<b;c++){
		in>>d>>e;
		d--;e--;
		out<<blah.query()-blahblahblah<<nl;
	}
}
