...
/*
Probably want to be checking for -1 condition here and maybe considering this uh I think we may consider as long as uh there is not like a uh strict strict strict strong majority umm like > (n+1)/2 then we are OK ungh but then from there need to compute how many of each uh initial tuple uh is like uh can cancel uh cancel in greedy ungh from maximum umm mult down and then eventually left with remainder umm? Like if we had uh how many umm maybe umm we can always umm match out if like uh see how many umm those mults too and then singletons or whatever...
*/

	in>>a>>as;
	av=V(26);
	m=0;
	for(b=0;b<a;b++)av[as[b]-'a']++;
	...
	av=V(26);
	for(b=0;b<a/2;b++)if(as[b]==as[a-1-b])av[as[b]-'a']++;
	sort(all(av));
	z=0;
	for(auto dude:av)z+=dude;
	z=(z+1)/2;
	y=z;
	z+=(av[25]>y)*(av[25]-y);
	out<<z<<nl;
