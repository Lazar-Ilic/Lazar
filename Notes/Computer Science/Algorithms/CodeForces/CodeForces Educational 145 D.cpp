...
Z az(){
	...
	/*
	Can simply do casework on joints with which to erase and also umm joints with which to 1 swap and then erase I think.
	*/
	...
	for(e=1;e<sz(as);e++){
		...
		if(as[e]=='0' and as[e-1]=='1')ml=min(ml,cl*(b+c-a-2)+bl);
		else ml=min(ml,cl*(b+c-a));
	}
	out<<ml<<nl;
}
