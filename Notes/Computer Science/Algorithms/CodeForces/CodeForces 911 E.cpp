...
*/
	...
	assc.generate(a-1);
	for(c=0;c<a;c++){
		d=assc.componentof[c];
		amii[d]++;
		bmil[d]+=av[c];
	}
	bvv=VV(a,V(0)); // List of dudes we point to I think.
	for(auto appp:avp){
		if(assc.componentof[appp.first]!=assc.componentof[appp.second]){
			bvv[assc.componentof[appp.first]].add(assc.componentof[appp.second]);
		}
	}
	cv=topsort(bvv);
	m=0;
	al=LLONG_MAX; // Update to minimum of maximum thus far each time.
	VPL avpl;
	avpl=VPL(a);
	for(c=a-1;c>=0;c--){
		d=cv[c];
		if(sz(bvv[d])>0){
			...
			bl=LLONG_MAX;
			for(auto dude:bvv[d]){
				if(avpl[dude].first>e){
					e=avpl[dude].first;
					bl=avpl[dude].second;
				}
				else if(avpl[dude].first==e and avpl[dude].second<bl){
					bl=avpl[dude].second;
				}
			}
		}
		...
		avpl[d]={e+amii[d],bl+bmil[d]};
		if(avpl[d].first>m){
			m=avpl[d].first;
			al=avpl[d].second;
		}
		else if(avpl[d].first==m and avpl[d].second<al)al=avpl[d].second;
	}
	out<<m<<" "<<al<<nl;
}
