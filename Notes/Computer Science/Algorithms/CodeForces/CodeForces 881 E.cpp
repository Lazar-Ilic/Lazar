...
*/
	...
	while(l<r or x){
		...
		bv=V(a);
		cv=V(a+1);
		for(d=0;d<m;d++)bv[av[d]]=1;
		for(d=1;d<=a;d++)cv[d]=cv[d-1]+bv[d-1];
		z=0;
		for(d=0;d<sz(avp) and z==0;d++)if(cv[avp[d].second+1]-cv[avp[d].first]>=(avp[d].second-avp[d].first+3)/2)z=1;
		if(z)r=m;
		else l=m+1;
		if(l==c+1){
			out<<-1<<nl;
