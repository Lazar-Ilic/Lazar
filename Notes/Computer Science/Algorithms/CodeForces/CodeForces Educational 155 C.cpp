...
Analysis:
0000000111111110000001111111111111 for example is worth examining as we must remove dudes if and only if there are >= 2 in the chunk and then O[n] is adequate at which point we can multiply the relevant factorials of chunk lengths and ways to order up those chunks which is like some multinomial... and this can be simplified of course.
*/
	...
	z=0;
	for(a=1;a<sz(as);a++){
		if(as[a]==as[a-1] and a!=sz(as)-1)b++;
		else{
			if(as[a]==as[a-1] and a==sz(as)-1)b++;
			bl=(bl*factorial(b))%p;
			al+=b-1;
			if(b-1>=1)avl.add(b-1);
			b=1;
		}
	}
	out<<al<<" "<<bl*multinomial(al,avl)%p<<nl;
}
