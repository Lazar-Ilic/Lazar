...
*/
	...
	for(e=0;e<b;e++){
		in>>c>>d>>f>>g;
		ap={avp[g].first-(c-avp[f-1].first),avp[g].second-(d-avp[f-1].second)};
		//out<<c<<" "<<d<<" "<<ap.first<<" "<<ap.second<<nl;
		if((ampv.count({c,d}) and ampv[{c,d}][0]<f) or (ampv.count({c,d}) and ampv[{c,d}][sz((ampv[{c,d}]))-1]>=g) or (ampv.count(ap) and (lowerbound((ampv[ap]).begin(),(ampv[ap]).end(),f))!=(ampv[ap]).end() and *(lowerbound((ampv[ap]).begin(),(ampv[ap]).end(),f))<g))out<<"YES"<<nl;
		else out<<"NO"<<nl;
	}
}
