...
*/
	...
	if(a==1)out<<"NO"<<nl;
		...
		al=avl[0]-avl[1];
		asetl.insert(al);
		if(al==0 or asetl.count(al+avl[2]) or al+avl[2]==0){
			out<<"YES"<<nl;
			return;
		}
		for(b=3;b<a;b+=2){
			al+=avl[b-1]-avl[b];
			if(al==0 or asetl.count(al) or (b+1<a and (asetl.count(al+avl[b+1]) or al+avl[b+1]==0))){
				out<<"YES"<<nl;
				return;
			}
			asetl.insert(al);
		...
		al=avl[1]-avl[2];
		asetl.insert(al);
		if(al==0 or (3<a and (asetl.count(al+avl[3]) or al+avl[3]==0))){
			out<<"YES"<<nl;
			return;
		}
		for(b=4;b<a;b+=2){
