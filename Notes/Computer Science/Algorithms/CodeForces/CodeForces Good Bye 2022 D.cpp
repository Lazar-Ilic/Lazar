...
Z az(){
	...
	/* It would seem that actually the way the underlying graph force structure plays out is not that totally trivial. We need to efficiently process through like a directed graph of force structures I think process through maybe one time to set up some digraph of the relevant connected force edges rather than processing through in this naive simplistic O[n^2] immediate obvious way. A usual Depth First Search approach based around those directed edges should almost certainly work where we can like simply toss in all potential didirectional edges for each relevant appearing pair and then process through. */
	in>>a;
	av=V(a);
	bv=V(a);
	cv=V(a+1); // Status on force.
	dv=V(a+1); // Updated status on force.
	ev=V(a+1); // Does it appear?
	z=0; // Count number of forced.
	for(b=0;b<a;b++){
		if(av[b]==bv[b]){
			z++;
			cv[av[b]]=1;
	...
	for(b=1;b<=a;b++){
		dv[b]=max(dv[b],cv[b]); // Updated force.
	}
	y=0; // Count number of free unforced cycles.
	out<<exp(a,z)*exp(2,y)%p<<nl;
}
