...
/*
n pairs averaging sum of 2n+1 ugh means n must be odd obviously as median value of sorted list is average is integral then is from 2n+1-[n-1]/2 to 2n+1+[n-1]/2 is like n=odd case is from n=2m+1 say is like... 3m+3 to 5m+3 or some such thing... try the n=5 case out here real quick...
...
*/
	...
	out<<"Yes"<<nl;
	for(b=1;b<(a+1)/2;b++)out<<b<<" "<<(3*((a-1)/2)+3)-1+b<<nl;
	for(b=(a+1)/2;b<=a;b++)out<<b<<" "<<a+b-(a+1)/2+1<<nl;
}
