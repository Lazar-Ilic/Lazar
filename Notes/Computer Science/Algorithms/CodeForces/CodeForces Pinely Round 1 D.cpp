...
Z az(){
	...
	al=0;
	for(c=1;c<=b;c++){
		// Break down into c chains and options for Left or Right.
		if(c+c-1<=a){
			if(a-b>=c-1 and (c-1>=1 and a-b>=1)){
				al=(al+(exp(3,a-(c+c-1))*choose(b-c+c-1,b-c))%p*choose(a-b-(c-1)+c-2,a-b-(c-1)))%p;
			}
			//out<<al<<nl;
			if(c+c<=a and a-b>=c){
				al=(al+(exp(3,a-(c+c)+1)*choose(b-c+c-1,b-c))%p*choose(a-b-(c)+c-1,a-b-(c)))%p;
				//out<<al<<nl;
				al=(al+(exp(3,a-(c+c))*choose(b-c+c-1,b-c))%p*choose(a-b-(c)+c-1,a-b-(c)))%p;
				//out<<al<<nl;
			}
			if(c+c+1<=a and a-b>=c+1){
				al=(al+(exp(3,a-(c+c))*choose(b-c+c-1,b-c))%p*choose(a-b-(c+1)+c,a-b-(c+1)))%p;
				//out<<al<<nl;
			}
		}
	}
