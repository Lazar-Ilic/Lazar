...
Analysis:
At each step it is greedy optimal to simply take the next appearance of the target character and sort that dude in as needed and delete off any dude lower than him... so simulation with relevant indices of appearance should be more than good enough here.
*/

	in>>a>>b>>as>>bs;
	avv=VV(26,V(0));
	av=V(26);
	for(c=0;c<a;c++)avv[as[c]-'a'].add(c);
	c=0; // Index on as
	for(d=0;d<b;d++){
		if(av[bs[d]-'a']<sz(avv[bs[d]-'a'])){
			for(e=0;e<bs[d]-'a';e++)while(av[e]<sz(avv[e]) and avv[e][av[e]]<avv[bs[d]-'a'][av[bs[d]-'a']])av[e]++;
			av[bs[d]-'a']++;
		}
		else{
			out<<"NO"<<nl;
