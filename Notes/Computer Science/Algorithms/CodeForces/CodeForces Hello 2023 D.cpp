...
/*
I think that the sort of naive approach here works which is potentially to use 2 Segment Trees for like range maximum querying or whatever should immediately work and take this into the like O[n log n] asymptotic. Think that for each razor size we need to find the first element where it is forced ugh and then Just Do It and in fact we could for sure process through initially in like O[n] to compute values which must be shifted and changed and insert all of those into a set and then like maybe simply process through those too in conjunction with the auxiliary dude.
*/
	in>>a;
	av=V(a);
	bv=V(a);
	...
	sort(all(cv)); // Sort the razors sizes and process in order.
	for(c=0;c<a;c++){
		if(bv[c]>av[c]){
			out<<"NO"<<nl;
			return;
			...
			avp.add({bv[c],c}); // Value of needed razor and index.
		}
	}
	sort(all(avp));
	d=0; // Index in cv of razors and minimally use as needed.
	for(c=0;c<sz(avp);){
		ap=avp[c];
		//out<<ap.first<<" "<<ap.second<<nl;
		while(d<b and cv[d]<ap.first)d++;
		if(!(d<b and cv[d]==ap.first)){
			out<<"NO"<<nl;
			return;
		}
		// Process maximal relevant range.
		while(c<sz(avp) and ap.first==avp[c].first and asg.query(ap.second,avp[c].second)<=cv[d])c++;
		d++;
	}
	out<<"YES"<<nl;
}
