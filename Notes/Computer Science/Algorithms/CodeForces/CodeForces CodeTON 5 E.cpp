...
Analysis:
Note firstly that if any 2 of our selected triangles intersect then we may as well Without Loss Of Generality simply select a larger triangle which covers all of those 2 and then Some More. This means that the actual region covered on the hypotenuse projects upwards like a Dyck Path sort of uh... and maybe there is some smart sort of a subset selection procedure to produce minimality for a given point set uh. Dunno if there is some greedy uh expanding uh algorithm uh. Of course in the final configuration there is no incentive to expand further. Would suggest that we may be able to case through from the closest to the furthest from the Hypotenuse and essentially compute the minimum cost needed to expand out our current configuration to cover that point and uh uh uh considering if that minimum cost is uh worth it uh. All at the margin greedy basically uh. And then we would need optimality from there inwards of course. Well uh uh uh like maybe we could uh uh uh somehow uh... instead compute like the uh... minimum cost given that we are going to cover uh some chunk of the hypotenuse... in particular though we can always uh expand uh... as long as it is cheaper to just uh expand uh... means uh... maybe we can uh process from the other direction and somehow uh... say uh...
*/
