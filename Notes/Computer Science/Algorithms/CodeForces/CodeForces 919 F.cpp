...
So the task reduces instantly to something relating to escaping the gridded convex hull of the island at which point we may then go to the wall and loop around and back in along that path so basically it is uh... simple uh... firstly we can propagate out distances from the island and then uh... maybe go outwards and in to propagate the desired answers and return to each query in O[1]... of course considering if and how we need to optimise that propagation inwards... at each ring level I think we can just do it ugh... twice maybe... OKOKOK so we need to propagate uh... OKOKOK so we want to compute the optimal ring at which point it then becomes the uh OKOKOK so we could uh... oh binary search for the ring threshold at which point we then just return uh the ring score or the actual distance whichever was shorter... uh except actually one of their input cases is interesting in that it immediately brings this approach in to questionable woo zones actually as multi layered rings of volcanoes clearly must need to be dealt with actually and uh... different rings can be formed around the inner island at different thresholds...
*/
	...
	while(!avp.empty()){
		for(auto [d,e]:avp){
			if(d+1<a and !avvb[d+1][e]){
				avv[d+1][e]=1+avv[d][e];
				avvb[d+1][e]=1;
				bvp.add({d+1,e});
			}
			if(d-1>=0 and !avvb[d-1][e]){
				avv[d-1][e]=1+avv[d][e];
				avvb[d-1][e]=1;
				bvp.add({d-1,e});
			}
			if(e+1<b and !avvb[d][e+1]){
				avv[d][e+1]=1+avv[d][e];
				avvb[d][e+1]=1;
				bvp.add({d,e+1});
			}
			if(e-1>=0 and !avvb[d][e-1]){
				avv[d][e-1]=1+avv[d][e];
				avvb[d][e-1]=1;
				bvp.add({d,e-1});
			}
	...
	e=a*b;
	while(d<e){
		// Check if there exists a path from the island out to the 
	}
	for(d=0;d<c;d++){
		in>>e>>f;
		e--;f--;
		out<<min(g,avv[e][f])<<nl;
	}					
}
