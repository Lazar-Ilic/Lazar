...
Analysis:
Well the worst case for a given target and power for each dude would be to avoid it optimally in the chain uh and kill of basically all dudes to that one side of it so it is something like uh left right right left dynamic programming per usual one would think here works canonically. That is to say that uh we can compute the minimum value... for each index we may compute the minimum value needed if we start at this index needed to kill all the dudes to the left of here for example... inclusive and right of here exlusive maybe works uh...
*/
	...
	for(b=1;b<a;b++){
		bv[b]=bv[b-1];
		bv[b]=max(bv[b],av[b]);
		bv[b]=max(bv[b],av[b-1]+a-b);
	}
	for(b=a-2;b>=0;b--){
		cv[b]=cv[b+1];
		cv[b]=max(cv[b],av[b+1]+b+1);
	}
	z=INT_MAX;
	for(b=0;b<a;b++)z=min(z,max(bv[b],cv[b]));
	out<<z<<nl;
}
