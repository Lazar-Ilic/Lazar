...
/*
Analysis:
If maximum is not up front then we need to consider suffix followed by reverse from there and then uh prefix is like simple greedy algorithm works. Otherwise it is the 2nd maximum suffix followed by its previous dude and then the uh maximum prefix...

10 9 1 2 3 ... uh yeah yeah yeah just swap the 10 10 and boom always ensured to be forced I think to umm...

10 ... 9 always optimal I think to swap on the 1 dude previous to it in that case.
...
*/
	...
	if(av[0]!=a and av[a-1]!=a){
		for(b=1;b<a;b++){
			if(av[b]==a){
				// Print out and finish loop.
				for(c=b;c<a;c++)out<<av[c]<<" ";
				out<<av[b-1]<<" ";
				c=b-2;
				while(c>=0 and av[c]>=av[0]){
					out<<av[c]<<" ";
					c--;
				}
				for(d=0;d<=c;d++)out<<av[d]<<" ";
				out<<nl;
				return;
			}
		}
