...
Z az(){
	...
	/*
	Breadth First Search from each uh 0 and then uh processed and uh expand rationally by sorting the uh priority queue works here I think.
	*/

	in>>a>>b;
	av=V(a);
	for(c=0;c<a;c++)in>>av[c];
	bv=V(a); // Has this vertex ever been processed yet uh.
	avv=VV(a,V(0));
	for(c=0;c<b;c++){
		in>>d>>e;
		d--;e--;
	...
	for(c=0;c<a;c++){
		if(av[c]==0 and bv[c]==0){
			// Vertex has a 0 and has not yet been processed uh.
			// Breadth First Search from here c.
			cv=V(0);
			priority_queue<P,VP,greater<P>> aaaaq; // Do this later as MINPQ
			int root=c,aaaa=0,bbbb;
			aaaaq.push({0,root});
			processed[root]=1;
			cv.add(root);
			z=0;
			while(!aaaaq.empty() and z>=0){
				aaaa=(aaaaq.top()).second;
				bbbb=(aaaaq.top()).first;
				aaaaq.pop();
				if(bbbb<=z){
					for(auto neighbour:avv[aaaa]){
						if(!processed[neighbour]){
							aaaaq.push({av[neighbour],neighbour});
			...
			for(auto dude:cv)processed[dude]=0;
			if(z==a){
				out<<"YES"<<nl;
				return;
			}
