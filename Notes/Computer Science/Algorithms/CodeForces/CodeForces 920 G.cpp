...
Ugh OK it now becomes apparent that I am in a terribly mediocre malodourous room and also hangry and thus must quit here... could rotate the avs around in to bvs and cvs and dvs here e.g. to not rotate around the actual loop loop structure... simpler than doing that... but the crux I missed onsighting here was that in their 2nd provided visual example we see why we need to logically operate around with respect to maxima and minima in the diagonal and row and column updating scheme so as to not force an O[n^2] blow up if we were to extend otherwise naively... and thus I am disinterested in wrapping up this implementation though it could be repaired here with 30 minutes to go...
*/

	in>>a>>b>>c;
	avv=VV(a,V(b,0)); // Row Sums
	bvv=VV(a,V(b,0)); // Column Sums
	cvv=VV(a,V(b,0)); // Up Right Diagonal Sums
	dvv=VV(a,V(b,0)); // Up Left Diagonal Sums
	avs=VS(a);
	for(d=0;d<a;d++)in>>avs[d];
	for(d=0;d<a;d++)if(avs[d][0]=='#')avv[d][0]=1;
	for(d=0;d<b;d++)if(avs[0][d]=='#')bvv[0][d]=1;
	for(d=0;d<a;d++)if(avs[d][0]=='#')cvv[d][0]=1;
	for(d=1;d<b;d++)if(avs[0][d]=='#')cvv[0][d]=1;
	for(d=0;d<a;d++)if(avs[d][b-1]=='#')dvv[d][b-1]=1;
	for(d=1;d<b;d++)if(avs[0][d]=='#')dvv[0][d]=1;
	for(d=0;d<a;d++)for(e=1;e<b;e++)avv[d][e]=avv[d][e-1]+(avs[d][e]=='#');
	for(d=1;d<a;d++)for(e=0;e<b;e++)bvv[d][e]=bvv[d-1][e]+(avs[d][e]=='#');
	for(d=1;d<a;d++)for(e=1;e<b;e++)cvv[d][e]=cvv[d-1][e-1]+(avs[d][e]=='#');
	for(d=1;d<a;d++)for(e=b-2;e>=0;e--)dvv[d][e]=dvv[d-1][e+1]+(avs[d][e]=='#');
	al=0;
	for(d=0;d<=a-c-1;d++){
		if(d==0){
			// Set up bl
			bl=0;
			for(e=0;e<=c;e++)bl+=avv[e][c-e];
			if(bl>al)al=bl;
			cl=bl;
			for(e=1;e<=b-c-2;e++){
				// Shift dude 1 column right and check for maximum.
				if(e+c+1<b)cl+=-bvv[][];
				else cl+=dvv[][]-
				if(cl>al)al=cl;
			}
			...
			// Update bl
			if(c+1<b)bl+=dvv[d+c][0]-dvv[d-1][c+1]-avv[d-1][c];
			else bl+=dvv[d+c][0]-avv[d-1][c];
			if(bl>al)al=bl;
		}
		cl=bl;
		for(e=1;e<=b-c-2;e++){
			// Shift dude 1 column right and check for maximum.
			if(e+c+1<b)cl+=-(bvv[][]-bvv[][]);
			else cl+=dvv[][]-
			if(cl>al)al=cl;
		}
		cl+=dvv[][]-(bvv[][]-bvv[][]);
