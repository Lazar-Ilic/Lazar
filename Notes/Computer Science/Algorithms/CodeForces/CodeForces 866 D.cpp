...
/*
OK I see so the sum of k_i I see I see I see on reading Input here I see that it is about the maximum after visiting all of the shops is like process the uh maximal chain of visited known at the end from the back if there is or is not like a clear there and uh like if there is no forced clear there I mean see how uh late what can what what to force the maximal desired outcome in our uh setting or what what can or can not go down if there were 2 uncleared prior then boom otherwise monitour the latest possible clearance and then what is what what precisely with respect to clearance and uh Failure to clear on uh parities. Perhaps we can try random colours and rapidly descend in to a good estimator at which point this overall runtime is like say we had uh n dudes uniform on like [0,n] and we try uh log[n] of them is enough to produce a minimum value of <=log[n] say and then on the other n-log[n] of the trials cost at most log[n] which immediately gives us a uh n * log[n] run time if that condition was True.
*/
