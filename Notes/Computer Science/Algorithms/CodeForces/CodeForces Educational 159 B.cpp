...
*/
	...
	in>>al>>bl>>cl>>dl;
	el=(al+6)/7; // Number of tasks.
	if(el/2*(2*dl+cl)>=bl)out<<al-(bl/(2*dl+cl)+((bl)%(2*dl+cl)!=0))<<nl;
	else if(el/2*(2*dl+cl)+(el%2)*(dl+cl)>=bl)out<<al-(el/2+1)<<nl;
	else if(el*dl+cl*al>=bl)out<<al-((bl-el*dl)/cl+((bl-el*dl)%(cl)!=0))<<nl;
}
