...
/*
Suffices to compute for the relevant uh maximal sizes from uh think even a bona fide O[t*sqrt[a+b]] solution will work here so bash up I think to uh like sqrt[a+b] should be uh high enough and then note the uh required number of steps en route upwards is like ceiling of uh divide by uh length/maximum. And take the maximum-1 hit uh. Can prolly uh trinary search here is faster.
*/

	in>>al>>bl;
	ml=LLONG_MAX;
	for(a=1;a<=sqrt(al+bl)+1000;a++)if(a-1+(al+a-1)/a+(bl+a-1)/a<ml)ml=a-1+(al+a-1)/a+(bl+a-1)/a; // 2 probably works here instead of 1000.
	out<<ml<<nl;
}
