...
I forgot C++ does not round down and instead rounds to 0. Oh ugh I did not read the condition that the sum of n does not exceed 100 and so I tried a greedy approximation attempt here Dynamic Program rather than immediately seeing a uh O[n^3] Dynamic Program here which should work which is to case on number of each dude shot thus far for each dude in turn basically simple simple simple DP but ugh I misread the task statement here ugh.

Test + Additional Input:


Analysis:
"Magic" will not save the world! False! Ugh!
*/

	in>>al>>bl;
	L cll,dll,ell,fll,gll,hll;
	cl=0;
	dl=0;
	el=0;
	fl=0;
	...
	for(b=0;b<a;b++){
		in>>c;
		if(cl+max((L)0,(c-dl)/al+(c>dl and c%al!=dl%al))<=fl+max((L)0,(c-gl)/al+(c>gl and c%al!=gl%al))){
			cll=cl+max((L)0,(c-dl)/al+(c>dl and c%al!=dl%al));
			dll=dl+al*(max((L)0,(c-dl)/al+(c>dl and c%al!=dl%al)))-c;
			ell=el+bl*(max((L)0,(c-dl)/al+(c>dl and c%al!=dl%al)));
		}
		else{
			cll=fl+max((L)0,(c-gl)/al+(c>gl and c%al!=gl%al));
			dll=gl+al*(max((L)0,(c-gl)/al+(c>gl and c%al!=gl%al)))-c;
			ell=hl+bl*(max((L)0,(c-gl)/al+(c>gl and c%al!=gl%al)));
		}
		if(cl+max((L)0,(c-el)/bl+(c>el and c%bl!=el%bl))<=fl+max((L)0,(c-hl)/bl+(c>hl and c%bl!=hl%bl))){
			fll=cl+max((L)0,(c-el)/bl+(c>el and c%bl!=el%bl));
			gll=dl+al*(max((L)0,(c-el)/bl+(c>el and c%bl!=el%bl)));
			hll=el+bl*(max((L)0,(c-el)/bl+(c>el and c%bl!=el%bl)))-c;
		}
		else{
			fll=fl+max((L)0,(c-hl)/bl+(c>hl and c%bl!=hl%bl));
			gll=gl+al*(max((L)0,(c-hl)/bl+(c>hl and c%bl!=hl%bl)));
			hll=hl+bl*(max((L)0,(c-hl)/bl+(c>hl and c%bl!=hl%bl)))-c;
		}
		cl=cll;
		dl=dll;
		el=ell;
