...
/*
OK so the multiplicities are Wrong in the actual underlying structural multiplicity based subset subsets inducing bitwise operations Principle Of Inclusion And Exclusion bash which should work here but then I tanked out and wrapped it up using a simple Bottoms Up Dynamic Programming approach which is also ACcepted in round... unfortunately. We would like to be ACing onsighting the correct Right Game Theoretically Optimal uh super duper simple like uh O[64^2] or O[64^3] like here if possible for uh massive O[n] like basically realised we want a uh O[n] read Input in to multiplicity like prefer strongly uh O[n+64^2] over O[128n] like in the bash.
*/
	...
	avv=VV(7,V(0));
	avv[0]={1,-1,5,-70,1330,-18620,93100};
	avv[1]={1,-2,24,-456,6384,31920};
	avv[2]={1,-3,45,-630,3150};
	avv[3]={1,-4,10,-20};
	avv[4]={1,-5,15};
	avv[5]={1,-6};
	avv[6]={1};
	...
	for(c=b;c<=6;c++){
		...
		for(d=0;d<64;d++){
			if(subsetsize(d)==c){
				z=0;
				for(e=0;e<64;e++)if((d ba e)==d)z+=av[e];
				bl=(bl+exp(2,z)-1)%p;
			}
		}
		out<<al<<nl;
		al=(al+bl*avv[b][c-b])%p;
	}
	out<<al<<nl<<nl;
}
