...
*/
	...
	avp=VP(a);
	OSET<I> aoset;
	for(b=0;b<a;b++)in>>avp[b].first>>avp[b].second;
	sortify(avp);
	al=0;
	for(b=a-1;b>=0;b--){
		aoset.insert(avp[b].second);
		al+=aoset.orderofkey(*aoset.lower_bound(avp[b].second))-aoset.orderofkey(*aoset.lower_bound(avp[b].first));
	}
	out<<al<<nl;
}
