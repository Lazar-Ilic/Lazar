...
Analysis:
Simply consider dependencies I think uh to unlock is like a uh to unlock each dude requires a uh certain order is like uh we need to find the uh maximal antichain basically of dependencies like... is finding the maximum chain in the uh reverse order I think uh... is like uh to each edge uh compute the uh index in the list and uh simply compute the iteration by uh direct uh basically uh uh uh we know that uh like uh OKOKOK each dude uh goes out on either the round of its parent or uh say we start off round 1 unlocking the node root and then uh note that from that point on each dude either unlocks in the round of its parent or that round+1 so like uh just invert the relevant uh edges and map uh Pairs in to Ints for indices and comparison should work here.
*/
...
while(!aq.empty()){
	...
	for(auto neighbour:avv[aaa]){
		if(!processed[neighbour]){
			...
			if(ampi[{min(aaa,neighbour),max(aaa,neighbour)}]<ampi[{min(aaa,parents[aaa]),max(aaa,parents[aaa])}])av[neighbour]=av[aaa]+1;
			else av[neighbour]=av[aaa];
		}
	}
}
