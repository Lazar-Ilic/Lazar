...
Z az(){
	...
	f=INT_MAX;
	for(c=0;c<=a/2;c++){
		// Process lengths 1 and 2 then 3 and 4 and so on and so on.
		if(bv[c]>e)e=bv[c];
		if(bv[c]<f)f=bv[c];
		// Compute number of relevant lengths off of indices 0 to a-1
		// 2*c+1 and 2*c+2 lengths.
		if(2*c+1>=e-f+1 and 2*c+1<=a)al+=min(a-1-2*c-1+1,f)-max(0,e-2*c-1+1)+1;
		if(2*c+2>=e-f+1 and 2*c+2<=a)al+=min(a-1-2*c-2+1,f)-max(0,e-2*c-2+1)+1;
	}
	out<<al<<nl;
}
