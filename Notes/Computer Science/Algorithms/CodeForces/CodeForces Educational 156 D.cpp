...
Analysis:
Well it certainly ought to be noted that the chain of > from Right To Left must indicate n n-1 n-2 ... by definition and that the chain of < from Right To Left must also similarly indicate 1 2 3 ... by definition. So the first character in the input must be 1 of the 2 and in any case let us try an additional like example...
<>????
Would be like:
?17???? Has 24 uh possible uh dudes but uh... actually 120 uh 5! uh...
<>???<>???
Is like:
? 2 10 ? ? ? 1 11 ? ? ?
Interesting toy example to consider responding to queries here because it seems like the number of choices in multiplication from Left To Right is like related to the number of forced dudes to the Right and is otherwise free I would think...
7 * 1 * 1 * 6 * ...
Is just sum of factorials I think uh...
1>5>6 oh this forces like the first dude is opposite sign of the uh other dude in a kind of handwaved way I think...
41?5?6
But it could also be like uh...
312546 or something ugh...
OKOKOK I am too hungry and rage quit this round think 2 pointers technique here is enough noting the semi-force on the maximum thus far with or without that combinatorial code should be good enough strong enough here for like a fast ABCD might be in the clear for Masterhood today but I am too slow and want a bona fide Top 50 ABCDE Educational Round nearly ensured as my 60th and final official CodeForces round soon... "Stay Hunger"- Steven Paul Jobs, Andro Lu Bai, et al.
*/

	in>>a>>b>>as;
	c=0;
	for(auto dude:as)if(dude=='?')c++;
	if(as[0]=='?')out<<0<<nl;
	else out<<factorial(c)<<nl;
	for(d=0;d<b;d++){
		in>>e>>ac;
		e--;
		if(ac=='?' and as[e]!='?'){
