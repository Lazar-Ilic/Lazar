...
Z az(){
	...
	c=0;
	d=0; // Find middle or middle 2 elements.
	if(a%2==1){
		// Compute c middle element index.
		for(e=0;e<a;e++){
			if(av[e]==(a+1)/2)c=e;
		}
		d=c; // Middle 2 indices starting shifters.
	}
	else{
		// Compute c and d middle element index.
		for(e=0;e<a;e++){
			if(av[e]==a/2)c=e;
			else if(av[e]==a/2+1)d=e;
		}
	}
	if(d<c){
		out<<a/2<<nl;
