...
*/
	...
	for(b=0;b<a;b++){
		in>>av[b];
		avp.add({av[b],b}); // Value and index.
	}
	sort(all(avp));
	z=1;
	for(auto dude:avp){
		z=1-z;
		if(z==0){
			if(aset.count(dude.first)){
				bv[dude.second]=dude.first;
				aset.erase(dude.first);
				cv[dude.second]=*(bset.begin());
				bset.erase(bset.begin());
				if(cv[dude.second]>dude.first){
					out<<"NO"<<nl;
					return;
				}
			}
			...
			if(bset.count(dude.first)){
				cv[dude.second]=dude.first;
				bset.erase(dude.first);
				bv[dude.second]=*(aset.begin());
				aset.erase(aset.begin());
				if(bv[dude.second]>dude.first){
					out<<"NO"<<nl;
					return;
				}
			}
