...
Analysis:
OKOKOK 3 minutes in the round and it immediately does split in to like bottom row next bottom yadda up and splits and adds can blow up exponentially I think uh but want maximum length chains across yadda yadda yadda... can we add uh uh uh...

https://usaco.guide/problems/cses-1073-towers/solution

Think it is like isomorphic with greedying after counting the number of horizontal blocks of each length in [1,N] frankly is like from the top to the bottom is like uh start out with some live rectangles and uh uh uh... instead of processing row by row we uh uh uh process rectangle by rectangle that is to say uh... so each tower down can have uh at most impact on uh like 2 rectangles uh... means O[n] rectangles and then it is O[1] to add h to the relevant dude uh.
*/
