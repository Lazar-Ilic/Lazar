...
Z az(){
	...
	in>>a>>b>>c;
	al=0;
	d=min(abs(1-b),min(abs(1-c),min(abs(a-b),abs(a-c))));
	if(a%2==0)al=(d%p)*choose(a,a/2)%p;
	else al=(d%p)*2*choose(a,a/2)%p;
	for(e=d+1;e<=d+1+a;e++){
		f=b+e;
		if(f<1)f=1;
		g=c-e;
		if(g>=a)g=a;
		if(f-1+a-g>a)continue;
		if(a%2==0){
			if(a/2-(f-1)>=0 and a-(f-1)-(a-g)>=a/2-(f-1))al=(al+choose(a-(f-1)-(a-g),a/2-(f-1)))%p;
		}
		else{
			if(a/2-(f-1)>=0 and a-(f-1)-(a-g)>=a/2-(f-1))al=(al+choose(a-(f-1)-(a-g),a/2-(f-1)))%p;
			if(a/2+1-(f-1)>=0 and a-(f-1)-(a-g)>=a/2+1-(f-1))al=(al+choose(a-(f-1)-(a-g),a/2+1-(f-1)))%p;
		}
	}
	out<<al<<nl;
}
