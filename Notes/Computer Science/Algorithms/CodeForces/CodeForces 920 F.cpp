...
*/
	...
	c=(int)sqrt(a)+1;
	avvl=VVL(c+1,VL(a,0)); // Prefix Sums
	bvvl=VVL(c+1,VL(a,0)); // Triangularised Prefix Sums
	for(d=1;d<=c;d++){
		for(e=0;e<a;e++){
			if(e-d>=0){
				avvl[d][e]=avvl[d][e-d]+avl[e];
				bvvl[d][e]=bvvl[d][e-d]+avl[e]*(1+e/d);
			}
			else{
				avvl[d][e]=avl[e];
				bvvl[d][e]=avl[e];
	...
	for(d=0;d<b;d++){
		...
		if(f>c){
			...
			if(e-f>=0)out<<bvvl[f][e+f*(g-1)]-bvvl[f][e-f]-(avvl[f][e+f*(g-1)]-avvl[f][e-f])*((e-f)/f+1)<<" ";
			else out<<bvvl[f][e+f*(g-1)]<<" ";
		}
	}
	out<<nl;
}
