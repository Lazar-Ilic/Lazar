...
/*
Can compute uh with doubles should probably work umm up to an epsilon or whatever is tight enough maybe and then go again for the actual precision rationally ummm... split is umm between 0 and 1 to between umm whatever is like a-1 potential umm values umm. Think like umm... 1 2 3 4 is like ummm... geometricish or something umm... geoemtric decay umm... unclear umm precision umm... geometric decay here is like ummm... on the underlying max is like ummm... Python might handle this to umm full precision umm... not really ummm... delta is like ummm on the underlying still ummm... OKOKOK no AC here umm because obviously like umm greedy is not AC here and n log n would be AC but the underlying selection algorithm is still like umm a uh umm strictly umm monotone maximiser on the underlying ummm... think I am probably not AC for the round here.
*/
	...
	bvd[0]=av[0];
	for(b=1;b<a;b++){
		bvd[b]=(bvd[b-1]+av[b])/2.0;
	}
	cvd[a-1]=av[a-1];
	for(b=a-2;b>=0;b--){
		cvd[b]=(cvd[b+1]+av[b])/2.0;
	}
	ad=0;
	for(b=0;b<a-1;b++)if(cvd[b+1]-bvd[b]>ad)ad=cvd[b+1]-bvd[b];
	out<<ad<<nl;
}
