...
Analysis:
Seems like a simple enough sort of a lazy segment tree task where we could go through constructing the entire tree and saving the time stamps and then go through again doing all of the range additions on the whole tree and save the values to subtract off at the end at those relevant saved timestamps using a simple lazy segment tree with range additions on the underlying Depth First Search In tree e.g. the flattened one.
*/
