...
Z az(){
	...
	sort(all(avs));
	for(c=0;c<a;c++){
		// Compute maximum prefix match.
		bs=bvs[c];
		z=0;
		y=0;
		//out<<bs<<nl;
		if(lowerbound(avs.begin(),avs.end(),bs)!=avs.end()){
			cs=*(lowerbound(avs.begin(),avs.end(),bs));
			z=0;
			while(z<b and cs[z]==bs[z])z++;
		}
		if(lowerbound(avs.begin(),avs.end(),bs)!=avs.begin()){
			cs=*(lowerbound(avs.begin(),avs.end(),bs)-1);
			y=0;
			while(y<b and cs[y]==bs[y])y++;
		}
		out<<max(y,z)<<" ";
	}
	out<<nl;
