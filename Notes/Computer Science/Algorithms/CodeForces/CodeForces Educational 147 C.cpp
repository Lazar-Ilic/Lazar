...
/*
Casework on like ungh minimum uh maximum over each letter I think umm should work here ungh.
*/

	in>>as;
	m=INT_MAX;
	for(b=0;b<26;b++){
		// Compute the uh maximum ungh for this character ungh.
		n=0; // Maximum
		f=-1; // Previous occurence. End supposition is uh at sz(as) as well of course and um take delta-1 of course.
		for(c=0;c<sz(as);c++){
			if(as[c]-'a'==b){
				n=max(n,c-f-1);
				f=c;
