...
Z az(){
	...
	*/
	...
	cvl=VL(a,0); // Multiplicities.
	evl=VL(a+1,0); // Super additions.
	dvl=VL(a+1,0);
	...
	for(b=0;b<a;b++){
		...
		asglz.addto(b,(lowerbound(dvl.begin(),dvl.end(),avl[b]+dvl[b])-dvl.begin())-1,1);
		evl[lowerbound(dvl.begin(),dvl.end(),avl[b]+dvl[b])-dvl.begin()-1]+=avl[b]-(*(lowerbound(dvl.begin(),dvl.end(),avl[b]+dvl[b])-1)-dvl[b]);
	}
