...
Z az(){
	...
	sort(all(avp)); // Weakest power and then iterate through.
	sort(all(bvp));
	bv=V(a);
	bv[a-1]=bvp[a-1].second;
	for(c=a-2;c>=0;c--){
		bv[c]=min(bv[c+1],bvp[c].second);
	}
	// Damage is b at first.
	c=0;
	while(b>0){
		// Reduce by b
		c+=b; // Total attack done thus far.
		//out<<c<<nl;
		// Find first dude still alive e.g. > c.
		if(lowerbound(bvp.begin(),bvp.end(),P{c+1,0})==bvp.end()){
			out<<"YES"<<nl;
			return;
		}
		else{
			b-=bv[lowerbound(bvp.begin(),bvp.end(),P{c+1,0})-bvp.begin()]; // Desired minimum power of dudes Still Alive.
		}
	}
	out<<"NO"<<nl;
}
