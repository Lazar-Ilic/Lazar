...
Z az(){
	...
	*/
	...
	in>>a>>b>>al;
	//ad=INT_MAX; // Should work here.
	l=1;
	r=1000000000;
	while(l<r){
		// Dividing the first side in to m part. Leaves a side length of a/m of course means that b/(a/m) is floor(b*m/a)*m >= al.
		m=(l+r)/2;
		if(L(m)*floor(D(b)*m/a)>=al){
			r=m;
		}
		else{
			l=m+1;
