...
Analysis:
Compute the minimum of the radius needed for A to hit P or the maximum of A hitting B and B hitting P.
*/

	//in>>a;
	in>>ad>>bd>>cd>>dd>>ed>>fd;
	out<<min(sqrt((cd-ad)*(cd-ad)+(dd-bd)*(dd-bd)),max(sqrt((ed-ad)*(ed-ad)+(fd-bd)*(fd-bd)),sqrt((ed-cd)*(ed-cd)+(fd-dd)*(fd-dd))/2))<<nl;
}
