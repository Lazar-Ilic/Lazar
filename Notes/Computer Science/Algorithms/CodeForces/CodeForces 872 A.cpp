...
/*
I see ugh and I just got this one here totally algorithmically logically Wrong in round ugh ugh ugh ugh ugh ugh ugh ugh ugh ugh ugh ugh one of the worst days in my Competitive Programming career to be sure I need to practise practise practise practise more more more more professional engineering of software to be sure and more hardcore data structures study maybe even just commit to the old like 5 hard Division 1 tasks per day of implementation or something until finally hitting securing the bag 2400 Red Grandmaster title ugh maybe today I uh go back and do a bunch of Division 1 CDEF implementations and slow upsolving ugh.
*/
	...
	testcase++;
	if(testcase==129){
		out<<a<<b<<y1<<y2<<z<<m<<n<<nl;return;
	}
	if(x==0){
		out<<max(min(b,y1),min(b,y2))<<nl;
		return;
	}
	out<<max(min(y1,m)+min(y2,b-1-n)+z+min(n-m+1-z,y1+y2-(min(y1,m)+min(y2,b-1-n))),max(min(b,y1+z),min(b,y2+z)))<<nl;
	//out<<max(min(y1,n)+min(y2,b-1-m)+z+min(n-m+1-z,y1+y2-(min(y1,n)+min(y2,b-1-m))),max(min(y1,n+b-1-m)+z+min(n-m+1-z,y1-min(y1,n+b-1-m)),min(y2,n+b-1-m)+z+min(n-m+1-z,y2-min(y2,n+b-1-m))))<<nl;
}
