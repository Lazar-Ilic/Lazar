...
/*
Say the uh MEX was uh like 3 and we want to make it uh 4 then we need to somehow remove all instances of the number uh 4 and also toss in a 3 and also leave a 0 1 2 I think so like take the minimum cover of 4 or if 0 instances of 4 we are in the clear and uh as long as there is 1 free dude uh not the uh 1 1 1 1 0 case e.g. try it out.
*/
	...
	c=0;
	y=0;
	while(c<=a and bv[c]>0){
		if(bv[c]>1)y=1;
		c++;
	} // End on c==MEX
	if(c==0 or (y and bv[c+1]==0)){
		out<<"YES"<<nl;
		return;
	}
	if(y==0 and c==a){
	...
	// Else we must try covering uh that entire interval.
	d=0; // Leftmost instance of uh c+1.
	e=a-1; // Rightmost instance of uh c+1.
	avb=VB(c+1); // Does dude appear in prior.
	while(d<a and av[d]!=c+1){
		if(av[d]<=c)avb[av[d]]=1;
		d++;
	}
	while(e>=0 and av[e]!=c+1){
		if(av[e]<=c)avb[av[e]]=1;
		e--;
	}
	y=1;
	for(f=0;f<=c-1 and y;f++)if(avb[f]==0)y=0;
	if(y)out<<"YES"<<nl;
	else out<<"NO"<<nl;
}
