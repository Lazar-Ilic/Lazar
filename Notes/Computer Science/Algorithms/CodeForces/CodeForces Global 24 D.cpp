...
Z az(){
	...
	for(b=0;b<a/2+(a%2);b++){
		for(c=0;c<=max(0,b-1);c++){
			if(b!=0)al=(al+choose(max(0,b-1),c)*(b+1-((a)%2))%p*av[a-3-c])%p;
			else al=(al+(1-(a%2))*av[a-2])%p;
		}
		// Think we can do like factorial range sums.
		// And maybe like power of 2 range sums as well.
		// Off of prefix sums.
		// Bona fide O[n] time or something.
	}
	out<<al*a%p<<nl;
}
