...
Analysis:
So I think we uh need to sort the relevant queries in terms of the implied minima so like for each uh mountain we need to find for each query uh basically the minimum relevant mountain height in this chain goes through at that height and so we can sort queries and as we process through add in mountains tick by tick height level by height level. I was in a meeting of course tanking out here due to work obligations and meetings and things is why I tanked out here in this round. So in particular we add in an edge if it is when we go through we add it in if it was to some dude greater or equal uh to us and uh then edges process in in order is OK can also add in edges themself to be sorted and processed through in the Disjoint Set Union data structure canonical example usage case here.
*/
	...
	e=0;
	for(f=0;f<c;f++){
		while(e<a and avp[e].first<=bvp[f].first){
			g=avp[e].second;
			for(auto neighbour:avv[g])if(av[neighbour]<=bvp[f].first)adsu.unite(neighbour,g);
			e++;
		}
		if(adsu.sameset(bvv[bvp[f].second][0],bvv[bvp[f].second][1]))avb[bvp[f].second]=1;
	}
	for(d=0;d<c;d++){
		if(avb[d])out<<"YES"<<nl;
		else out<<"NO"<<nl;
