...
STR MCMF{//Stanford
	...
	PL maxflow(I source,I sink){
		...
		// Can insert a bar here to set up the parities to be correct.
		while (path(source),seen[sink]){
			L fl=inf;
			for(I p,r,x=sink;tie(p,r)=parent[x],x!=source;x=p){
				fl=min(fl,r?capacity[p][x]-flow[p][x]:flow[x][p]);
				// And then like ensure parities here are correct modulo 2.
			}
			// Ensuring that fl = 0 [mod 2]
			totalflow+=fl;
			for(I p,r,x=sink;tie(p,r)=parent[x],x!=source;x=p){
				if(r){
					flow[p][x]+=fl;
...
Z az(){
	...
	// Think we just need to modify it half cleverly.
