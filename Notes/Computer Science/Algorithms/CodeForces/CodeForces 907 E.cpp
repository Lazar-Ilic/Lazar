...
Analysis:
Seems as though one may simply go through and immediately greedily 0 out any dude who has GCD 1 with both of its neighbours which are > 1 so those each reduce the count by 2 and then after that we may remove 1 for each bad dude which contains a value > 1 by 0ing out the other dude at which point we then might be left with reducing n-1 for each n chain of 1s sorting from lengthiest chains downwards...
*/
