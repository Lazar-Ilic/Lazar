...
Z az(){
	...
	/* I do not think that this task is so trivial. Somehow we might want to try to use a Linearity Of Expectation type approach here. We might need to store partial probabilities modulo p of there existing a fly at each vertex as well as expected value modulo p of the number of flies on each side of each vertex uh uhhhm downstream or upstream of say an arbitrary root node 1. Then for each edge in the dude I think we process through and then process through again at the end like the contribution of each edge in the graph towards the final sum has to do with the probability that 1 of the 2 randomly selected dudes is upstream and 1 is downstream or we need the usual Dynamic Program 2 for the case where the 2 dudes are downstream on different downstream paths subnodes. */
	in>>a;
}
