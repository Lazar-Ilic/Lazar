...
Z az(){
	...
	*/
	...
	in>>a>>cl>>dl;
	av=V(a);
	bv=V(a+1); // Did dude appear 1 time?
	for(b=0;b<a;b++){
		in>>av[b];
		if(av[b]<=a)bv[av[b]]=1;
	}
	//sort(all(av));
	// Bash casework compute the cost of generating a uh 1 dude 2 dude... based upon the uh number of dudes going to be deleted and uh inserted is based around like a uh 1 pointer approach might work to uh generate the uh maximal index which works uh I think uh. Start at uh -1.
	ml=cl*a+dl; // Maximum cost.
	r=0; // Number of dudes to not trash uh.
	sort(all(av));
	uniquify(av);
	for(auto dude:av){
		r++;
		if((dl)*(dude-r)+(cl)*(a-r)<ml)ml=(dl)*(dude-r)+(cl)*(a-r);
	}
	out<<ml<<nl;
}
