...
Z az(){
	...
	sort(all(av));
	// Process through in order simply O[n] from here if ugh like == obviously we need to add 1 element to the new chain ugh == to the previous element else if ugh we actually have to track the multiplicity of current live chains that is to say the number of == in the previous block and iterate ugh.
	z=0;
	b=0; // In live block.
	c=0;
	d=0; // In previous live block.
	while(c<a){
		b=1;
		if(c==0 or av[c]!=av[c-1]+1){
			z+=d;
			d=0;
			y=0;
		}
		...
		while(c<a-1 and av[c]==av[c+1]){
			b++;
			c++;
		}
		c++;
		if(y){
			// Properly processed with respect to whatever.
			if(d>b)z+=(d-b);
		}
		d=b;
	}
