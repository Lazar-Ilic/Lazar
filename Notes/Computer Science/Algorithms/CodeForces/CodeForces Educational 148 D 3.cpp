...
*/
	...
	for(c=0;c<b;c++){
		in>>d;
		e=(d-a+1)/2; // 2 shwoops off uh. Leaves uh d-2*e.
		bminpq=aminpq;
		for(f=d;f>2*e;f--){
			bminpq.push(bminpq.top()+f);
			bminpq.pop();
		}
		PQ lolpq;
		while(!bminpq.empty()){
			lolpq.push(bminpq.top());
			bminpq.pop();
		}
		for(f=2*e;f>0;f--){
			lolpq.push(lolpq.top()-1);
			lolpq.pop();
		}
		m=INT_MAX;
		while(!lolpq.empty()){
