...
Z az(){
	...
	ULL aull,bull,cull,dull,eull,full;
	in>>a>>aull;
	bull=aull%p; // Total number of unambiguous chains.
	eull=aull%p; // Total number of chains
	full=aull%p;
	cull=aull%p; // Number of live unambiguous chains.
	dull=aull; // Number of live unambiguous choices remaining.
	for(b=2;b<=a;b++){
		// Process through these chains.
		if(dull>0 and isprime(b)){
			dull=dull/b;
		}
		cull=(cull*(dull%p))%p;
		bull=(bull+cull)%p;
