...
Z az(){
	...
	/*
	Misread the task statement ugh so we have to pick the 1st friend from the Left and second friend from the Right e.g. we need to select a binary string minimising that difference ugh. So I think that we ugh need to like sort maybe maybe sort and then consider moving from Right to Left that is to say that if we sort then we pick all of the dudes to the Right of some point we ugh ugh just do it.
	*/
	...
	m=0;
	z=INT_MAX;
	for(b=sz(avp)-1;b>=1;b--){
		// Actually took Left on the dudes Left of here.
		m=max(m,-avp[b].second);
		z=min(z,abs(avp[b-1].first-m));
	}
	out<<z<<nl;
}
