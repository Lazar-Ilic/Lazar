...
Z az(){
	...
	// Seems to me as if this rather simple task should isomorph somehow into a question on whether or not there is a dude in an adjacent vertex or on a straight up path terminating in a bonus square to the finish guy where we can start on a non-bonus square but still just like a straight up Breadth First Search should work here after checking the immediate neighbours of the target final square I think. So yeah would strike me like a simple Breadth First Search from the ugh Final one and maybe indices of starting tokens in a hash set.
	in>>a>>b;
	in>>c>>d;
	for(e=0;e<c;e++){
		in>>f;
