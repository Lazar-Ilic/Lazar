...
/*
Is there like a fairly strong approximation algorithm for streaming with like O[1] or rather O[polylog n bits thus far] of memory usage for storing the curve of partial sums thus far in order to estimate?
*/
	...
	d=0;
	for(b=0;b<a and d<c/2;b++){
		if(avb[b]==0)d++;
	}
	out<<max(1,b)<<nl;
	/*
	av=V(a/2+1); // First relevant index with number of 2s.
	av[0]=1;
	b=0; // Number of 2s.
	for(c=0;c<a;c++){
		in>>d;
