...
Analysis:
OnSight note that it suffices to generate a Dynamic Program O[nk] or O[mk] algorithm here. One supposes that perhaps O[2*n*k] can be done following an O[m * log[m]] sort on the intervals. Or perhaps even O[2*n*k^2] at which point we could then do a casework on values and how many intervals are currently live or some such thing in a -= framework of analysis setting. However, I am feeling somewhat lethargic here today and am statistically locked out of actually opting in to this round so I am content to read the editorial tutorial later.
*/
