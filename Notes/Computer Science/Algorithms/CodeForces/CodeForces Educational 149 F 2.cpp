...
/*
Analysis:
Seems like we could binary search for the threshold value and then for each iteration simply move from Left to Right and Right to Left uh computing the minimal sum of uh OKOKOK we need the uh maximal chain with sum <= so uh I guess we can uh store the values in like a uh set and uh keep the index of the rightmost dude uh... and uh store that pointer somehow uh... or uh pointer to uh maximal chain length and uh... OSET... oh I guess I need to solve this task in bona fide O[n] per test case somehow... maybe a quick O[n log n] based around sorting a vector of pairs...
...
*/
	...
	while(al<bl){
		cl=(al+bl)/2;
		//out<<cl<<nl;
		bv=V(a+1); // Maximal length chain <= cl sum to Left of break point.
		cv=V(a+1); // Maximal length chain <=cl sum to Right of break point.
		OSET<P> aoset,boset;
		aoset.insert({0,1});
		dl=0; // Sum of dudes to Left of point.
		f=0; // Index uh OK to examine.
		for(d=1;d<=a;d++){
			// Going to insert uh av[d-1] here and then uh update.
			if(av[d-1]<(*aoset.findbyorder(f)).first){
				dl+=av[d-1];
				dl-=(*aoset.findbyorder(f)).first;
			}
			aoset.insert({av[d-1],d});
			while(f+1<=d and dl+(*aoset.findbyorder(f+1)).first<=cl){
				dl+=(*aoset.findbyorder(f+1)).first;
				f++;
			}
			bv[d]=f;
		}
		...
		for(d=a-1;d>=0;d--){
			// Going to insert uh av[d-1] here and then uh update.
			if(av[d]<(*boset.findbyorder(f)).first){
				dl+=av[d];
				dl-=(*boset.findbyorder(f)).first;
			}
			boset.insert({av[d],d});
			while(f+1<=(a-d) and dl+(*boset.findbyorder(f+1)).first<=cl){
				dl+=(*boset.findbyorder(f+1)).first;
				f++;
			}
			cv[d]=f;
		}
		z=0;
		for(d=0;d<=a;d++)if(bv[d]+cv[d]>=b)z=1;
		if(z)bl=cl;
		else{al=cl+1;cl++;}
	}
	out<<cl<<nl;
