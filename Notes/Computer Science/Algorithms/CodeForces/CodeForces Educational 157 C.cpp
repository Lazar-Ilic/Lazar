...
Analysis:
Multiplicity on prefix and suffix sums of each length pair should work here to loop through at the end basically uh... or uh even better we can loop through same length pairs and also uh... basically for each dude have a 1 matrix counting the dudes of each length with each sum and another for the needed dudes basically to pair off correctly uh...
*/
	...
	for(b=0;b<a;b++){
		...
		reverse(all(av));
		d=0;
		for(auto dude:av)d+=dude;
		avv[sz(av)][d]++;
		e=d;
		f=0;
		for(g=sz(av)-1;g>sz(av)/2;g--){
			e-=av[g];
			f+=av[g];
			if(e-f>0)bvv[(g)-(sz(av)-g)][e-f]++;
		}
		e=d;
		f=0;
		for(g=0;g<sz(av)/2;g++){
			e-=av[g];
			f+=av[g];
			if(e-f>0)bvv[(sz(av)-1-g)-(g+1)][e-f]++;
		}
	}
	al=0;
	for(b=0;b<6;b++){
		for(c=0;c<46;c++){
			al+=L(avv[b][c])*bvv[b][c];
		}
	}
	for(b=0;b<6;b++)for(c=0;c<46;c++)al+=L(avv[b][c])*avv[b][c];
	out<<al<<nl;
}
