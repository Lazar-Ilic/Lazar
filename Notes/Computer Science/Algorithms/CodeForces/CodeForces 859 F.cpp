...
Z az(){
	...
	/*
	Naive simulation should work here with 4 simple uh VVBs to represent the actual state on the underlying and the next movement ugh directionally that is like uh if we have been in this position before terminate with the -1 output else simply transition into the next relevant state in a simple loop etc. etc.
	*/
	...
	z=0;
	/*
	if(c==0 and (g==0 or g==3))z--;
	else if(c==a-1 and (g==1 or g==2))z--;
	else if(d==0 and (g==0 or g==1))z--;
	else if(d==b-1 and (g==2 or g==3))z--;
	*/
	while(avvvb[c][d][g]==0){
		//out<<c<<" "<<d<<" "<<g<<" "<<z<<nl;
		if(c==e and d==f){
		...
		avvvb[c][d][g]=1;
		// Now compute the next state in terms of ABG Asian Baby Girl notation.
		if(g==0){
			if(c>0 and d>0){
				c--;
				d--;
