...
Z az(){
	...
	VD avl,bvl,cvl,dvl,evl,fvl,gvl,hvl,ivl,jvl,kvl,lvl,mvl;
	...
	*/
	...
	il=0;
	jl=0;
	// Casework through umm unlucky based upon the remainder of umm like umm say it was equivalent to umm like 2a=i for i in 0 to k-1 or whatever and then case out the dudes excluded there.
	dl=0; // Number of unlucky.
	for(d=0;d<b;d++){
		// Add number of unlucky with uh sum of d modulo b and uh not hitting any of the requisite dudes.
		avl=VD(b,1);
		bvl=VD(1,1); // Dude to uh take any time the uh modulo 2 is umm odd I think.
		z=0;
		for(e=0;e<b;e++){
			if((2*e)%b==d){
				avl[e]=0; // Excluded.
		...
		// Exponentiate uh avl and uh take it modulo bl I guess umm.
		cl=al; // Uh exponent.
		while(cl>0){
			// If odd uh multiply and mod out.
			if(cl%2==1){
			cvl=convolution(avl,bvl);
			for(f=b;f<=2*b-2 and f<sz(cvl);f++)cvl[f-b]=(cvl[f-b]+cvl[f])-L(L(cvl[f-b]+cvl[f])/p)*L(p);
			cvl.resize(b);
			bvl=cvl;
			}
			dvl=avl;
			cvl=convolution(avl,dvl);
			for(f=b;f<=2*b-2 and f<sz(cvl);f++)cvl[f-b]=(cvl[f-b]+cvl[f])-L(L(cvl[f-b]+cvl[f])/p)*L(p);
			cvl.resize(b);
			avl=cvl;
			cl/=2;
