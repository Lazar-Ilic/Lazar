...
Z az(){
	...
	/*
	Hmm interesting task I guess one can shorten the path by checking in such Input cases about easier ways to optimally shwoop shwoop Leftwards ugh by computing in those cases that distance going out and back and back I suppose. Ugh here we need like a range minimum query structure I would suppose on the underlying ugh ugh ugh how interesting we would need a range minimum query upon this I think. Testing the c g e range of course min of min and max of max and then computing the minimum in that range.
	*/
	...
	in>>c>>d>>e>>f;
	if(c<=e){
		for(g=c+1;g<=e;g++)d=min(d,av[g-1]+1);
	}
	else{
		for(g=c-1;g>=e;g--)d=min(d,av[g-1]+1);
	}
	...
	if(d>f){
		RMQ armq(av);
		for(g=1;g<=a;g++)m=min(m,abs(c-g)+abs(e-g)+abs(f-(armq.query(min(c,min(e,g))-1,max(c,max(e,g))-1)+1)));
	}
	out<<m<<nl;
}
