...
Analysis:
Oh no! Zoinkers boinkers doinkers Nasty is mixing up the Alchemical potions emotions again yikeserino zoinkers Disc Jockeying SLVR in to JWLS Gold VIII!

So in any case I claim that proof of correctness is like uh think about uh...

11111011111
paired with
11111111111
is better than paired with
11111100000

Actually because we can flip whatever basically we want the uh minimum value of the pairwise uh bitwise XOR basically because we can nail in 2^k-1-XOR I think.
*/
	...
	sortify(avp);
	m=INT_MAX;
	for(c=0;c<a-1;c++)if((avp[c].first bx avp[c+1].first)<m){
		m=(avp[c].first bx avp[c+1].first);
		n=avp[c].second;
		o=avp[c+1].second;
		q=0;
		for(f=0;f<b;f++)if((avp[c].first ba (1<<f))==0 and (avp[c+1].first ba (1<<f))==0)q+=(1<<f);
	}
	out<<min(n,o)<<" "<<max(n,o)<<" "<<q<<nl;
}
