...
Analysis:
Well uh the immediate logical flow here is to consider uh like + 1 dude or -1 dude or - 2 dudes maybe uh... would think that perhaps uh... maximum flow in to all cases uh... is like uh... not clear to me uh uh uh... the single uh the + 1 the -1 the - 2 cases uh... the +2 cases uh...
...
11110 uh... gets 1 flow uh...
10110 uh... gets 1 flow uh...
10010 uh... gets 2 flow uh...

At which point we are uh done I think uh... think think think we can just sort out in that way that route maybe uh... not sure not sure not sure uh... well uh if there is a connection I think it is done so can check for the inf first and then logic out I think uh... even like the most uh naive uh... n^3 algorithm might work OK here uh uh uh.
*/
	...
	al=0;
	y=1;
	for(c=a-1;c>=1 and y;c--){
		m=INT_MAX; // Maximum additional flow we can take at this level.
		n=-1;
		o=-1;
		for(auto dude:avv){
			if(avb[dude[0]]!=avb[dude[1]] and dude[2]<m){
				m=dude[2];
				n=dude[0];
				o=dude[1];
			}
		}
		// Subtract off all of that flow from all affected dudes in the avv matrix and then remove a relevant dude with 0 flow left uh... each time just remove 1 dude uh... the dude who was at uh avb[dude]==1 or whatever...
		//out<<"m: "<<m<<" "<<avb[0]<<" "<<avb[9]<<nl;
		for(e=0;e<sz(avv);e++){ // Ugh losing rating because auto does not work like that with respect to the underlying memory pointers ugh.
			if(avb[avv[e][0]]!=avb[avv[e][1]]){
				avv[e][2]-=m;
			}
		}
		//outvv(avv);
		...
		if(n==0 or o==0){
			...
			avvb.add(avb);
			if(avb[n])avb[n]=0;
			else if(avb[o])avb[o]=0;
		}
	}
	out<<al<<" "<<sz(dv)<<nl;
	for(c=1;c<=sz(dv);c++){
		for(auto dude:avvb[c-1])out<<dude;
		out<<" ";
		out<<dv[c-1]<<nl;
	}
}
