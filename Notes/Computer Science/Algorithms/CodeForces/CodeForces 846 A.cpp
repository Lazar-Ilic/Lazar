...
Z az(){
	...
	in>>a;
	bv=V(0); // First 3 odd indices.
	cv=V(0); // First 2 even indices.
	av=V(a);
	for(b=1;b<=a;b++){
		in>>c;
		if(c%2==1 and sz(bv)<3)bv.add(b);
		else if(c%2==0 and sz(cv)<2)cv.add(b);
	}
	if(sz(bv)==3){
		out<<"YES"<<nl;
		out<<bv[0]<<" "<<bv[1]<<" "<<bv[2]<<nl;
		return;
	}
	else if(sz(bv)>=1 and sz(cv)==2){
		out<<"YES"<<nl;
		out<<bv[0]<<" "<<cv[0]<<" "<<cv[1]<<nl;
		return;
	}
	else{
		out<<"NO"<<nl;
