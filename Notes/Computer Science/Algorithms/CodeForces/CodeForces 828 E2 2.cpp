...
Z az(){
	...
	ULL aull,bull,cull,dull,eull,full,gull,hull;
	VULL avull;
	in>>aull>>bull>>cull>>dull;
	eull=aull*bull;
	full=eull;
	while(full<aull*bull)full+=eull;
	while(full<=cull*dull){
		avull=factor(full);
		for(a=0;a<(1<<sz(avull));a++){
			gull=1;
			hull=1;
			for(b=0;b<sz(avull);b++){
				if(a ba (1<<b))gull*=avull[b];
				else hull*=avull[b];
			}
			if(gull>aull and gull<=cull and hull>bull and hull<=dull){
				out<<gull<<" "<<hull<<nl;
				return;
			}
		}
