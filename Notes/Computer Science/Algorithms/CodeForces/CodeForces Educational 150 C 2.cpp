...
Analysis:
Can go through and compute the best value and then compute like the best modifier value with frequency array. And checking offset on our dude and dudes to the uh left uh.
*/

	in>>as;
	avl=VL(5,0); // Freq to Left
	bvl=VL(5,0); // Freq to Right
	cvl={1,10,100,1000,10000};
	ml=0;
	for(c=0;c<sz(as)-1;c++){
		if(as[c]=='A')avl[0]++;
		else if(as[c]=='B')avl[1]++;
		else if(as[c]=='C')avl[2]++;
		else if(as[c]=='D')avl[3]++;
		else if(as[c]=='E')avl[4]++;
	}
	al=0;
	for(c=sz(as)-1;c>=0;c--){
		// Compute al to be unmodified uh.
		if(c<sz(as)-1){
			if(as[c]=='A')avl[0]--;
			else if(as[c]=='B')avl[1]--;
			else if(as[c]=='C')avl[2]--;
		...
		z=1;
		for(d=as[c]-'A'+1;d<=4 and z==1;d++)if(bvl[d]>0)z=-1;
		al+=cvl[as[c]-'A']*z;
		// Now compute the Delta uh for each possible change uh. Impacts uh this value of course times uh z and uh the Left dudes uh... only relevant if we are uh uh uh like a unique max I think like a uh... we need to be greater than all dudes to our Right and uh also uh... this only impacts to the Left dudes between us and the next dude of our size or greater uh... well it impacts dudes sandwiched between greater than the other dudes and uh... can traverse from this point Right to Left I think uh... OKOKOKOK well we can just bash I think actually uh bash bash bash then uh 5 dudes is like uh... this can be simplified further uh and sped up dramatically uh.
		if(aset.count(c)==0){
			for(d=0;d<=4;d++){
				bl=cvl[as[c]-'A']*(-z);
				z=1;
				for(e=d+1;d<=4 and z==1;d++)if(bvl[d]>0)z=-1;
				bl+=cvl[d]*z;
				ml=max(ml,bl);
			}
		}
		if(as[c]=='A')bvl[0]++;
		else if(as[c]=='B')bvl[1]++;
		else if(as[c]=='C')bvl[2]++;
		else if(as[c]=='D')bvl[3]++;
		else if(as[c]=='E')bvl[4]++;
	}
	al=al+ml;
	// Now evaluate the values for uh the other uh just 25 strings uh...
	out<<al+ml<<nl;
}
