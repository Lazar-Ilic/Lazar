...
/*
Well as long as the uh sell the tree oh OK so sell the tree means increase the uh cost so as long as we need uh like k>c so that adding 1 to that dude uh increases that dude uh and also then like oh OKOKOKOK no I think that uh walking directly down to the closest end of a diameter is like optimal. Ugh canonical uh DFS DFS leads to uh 2 central or 1 central dude at which a uh final DFS I think should work there uh I think I think I think uh.
*/
...
while(!aq.empty()){
	...
	for(auto neighbour:graph[a]){
		if(!processed[neighbour]){
			...
			distances1[neighbour]=distances1[a]+1;
		}
	}
}
...
m=0;
n=0;
for(b=0;b<a;b++)if(distances1[b]>m){m=distances1[b];n=b;}
...
// Unfortunate here today I mean like this is a really really really trivial task and I uh not sure I ever will be needing like distances1 2 3 4 in a uh nother call from another function seems super wise to have maximal uh diameter diameter diameter code here to deduce the 1 or 2 point diameter median mid points set here and also the full set of the complete diameter end uh vertices there either uh like uh OK any vertex which could be the end of a uh so-called diameter so especially in that uh 2 vertex set case the uh complete set of a dudes on the Left hand side and b dudes on the Right hand side fully form that precise vertex set of pairs of diameters is good AC here find the nearest one to the initial root vertex off of distances1
}
