...
So if one is doing this uh in round one ought to ensure to remove the in>>a statement at the bottom if running all test input cases in 1 batch rather than separately in a separate external loop call like maybe some ecnerwala Andrew He scraper would do automatically.
*/
	...
	//cvv=VV(a,V(0)); // Can store as uh vector like [cost,majority values...] ugh and ensure to uh store the values uh offset by the uh not including th current node's uh value I think uh. Ugh that should uh probably work if I can implement it up correctly ugh ugh ugh.
	VPLSET cvpls;
	cvpls=VPLSET(a);
	for(b=0;b<a-1;b++){
		in>>c>>d;
		...
		for(auto dude:bvv[b]){
			if(sz(children[dude])==0){
				...
				for(auto child:children[dude]){
					if(child!=n and y){
						for(auto dude2:cvpls[child].second){
							if((cvpls[n].second).count(dude2 bx (av[n] bx av[child])) or (lolset).count(dude2 bx av[child]))y=0;
							else lolset.insert(dude2 bx av[child]);
						}
					}
				}
				if(y){
					swap(cvpls[n],cvpls[dude]);
					av[dude]=(av[dude] bx av[n]);
					for(auto child:children[dude]){
						if(child!=n){
							for(auto dude2:cvpls[child].second)(cvpls[dude].second).insert(dude2 bx (av[n] bx av[child]));
					...
					for(auto child:children[dude]){
						...
						for(auto dude2:cvpls[child].second)majority[dude2 bx av[child]]++;
						SET lolset;
					...
					for(auto lol:majority)if(lol.second==maxxx)(cvpls[dude].second).insert(lol.first);
				}
			}
		}
		if(b==0){
			for(auto dude:cvpls[0].second)if((dude bx av[0])==0){
