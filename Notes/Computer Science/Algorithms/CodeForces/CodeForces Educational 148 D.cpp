...
/*
OKOKOK contemplate this task and come back for a more terse legible implementation here ugh really kind of All We Do is Fail some days... yesterday was one of the lower moments of my Competitive Programming career thus far but Still We Rise, Still We Rise and Still We envision that bright bright Future of $10000000 Grandmaster Red 2400 so there! Tomorrow I am hopping in the mixture on a fun Division 1 round should be fun fun fun hoping to onsight ABC boom Top 200 vroom!
...
*/
	...
	bv=V(a+1);
	bv[0]=av[0];
	for(c=1;c<=a-1;c++)bv[c]=min(bv[c-1]+1,av[c]);
	cv=V(a); // a-1
	dv=V(a);
	n=INT_MAX; // Minimum from front. Will be shifted.
	for(c=0;c<a-1;c++){
		cv[c]=av[c]+(a-c);
		n=min(n,cv[c]);
	}
	cv[a-1]=av[a-1]; // Minimum at rear.
	m=INT_MAX;
	for(c=0;c<a;c++){
		dv[c]=av[c]+(a-c);
		m=min(m,dv[c]);
	...
	bv[a]=m;
	cl=0; // Total sum of dudes in cv.
	dl=0; // Total sum of dudes in dv.
	for(c=0;c<a;c++)cl+=cv[c];
	for(c=0;c<a;c++)dl+=dv[c];
	for(c=0;c<b;c++){
		in>>d;
		if(d<=a)out<<bv[d]<<" ";
		else{
			...
			if(d-2*e==a){
				out<<min(L(m)+d-a,(dl+L(a)*(d-a)-e)/a)<<" ";
				//if(e<=dl)out<<L(bv[a])+d-a<<" ";
				//else out<<bv[a]-(1+(e-dl-1)/(a))+d-a<<" ";
			}
			else if(d-2*e==a-1){
				out<<min(L(cv[a-1]),min(L(n)+d-a,(cl+L(a-1)*(d-a)-e)/a))<<" ";
				//if(e<=cl)out<<min(L(cv[a-1]),L(bv[a-1])+d-a+1)<<" ";
				//else out<<bv[a-1]-(1+(e-cl-1)/(a))+d-a+1<<" ";
			}
		}
	}
	out<<nl;
