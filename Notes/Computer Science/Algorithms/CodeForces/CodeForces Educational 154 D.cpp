...
Analysis:
Could potentially go from Left To Right and Right To Left here computing the minimum number of operations needed to sort strictly increasing and Negative or uh strictly increasing and Positive on both sides uh uh uh and then take the minimum on a Dynamic Programming summation. OKOKOK so think that means if we were greater just break down in to decreasing chains uh from the Left and so on and so on in to the number of decreasing chains and so on and so on.
*/

	in>>a;
	cv=V(a);
	for(b=0;b<a;b++)in>>cv[b];
	av=V(a+1);
	bv=V(a+1);
	av[1]=1;
	for(b=2;b<=a;b++){
		av[b]=av[b-1];
		if(cv[b-1]>=cv[b-2])av[b]++;
	}
	bv[a-1]=0;
	for(b=a-2;b>=0;b--){
		bv[b]=bv[b+1];
