...
Misread it and tanked for 70 minutes in round upon it... TT :( Big Sad... would have been Rank 14 and 2200 Master rating ungh cringe... OK then after regrading it would have been around Rank ~150 if not full ABCDEF flexture for ~2100 Master... interesting that I could potentially manipulate the Educational structure to Jump to ~2090 and then ~2250 with 1 ABCDEF round for Top 15.

Test + Additional Input:


Analysis:
Ugh Love Is Rage Love Is Rage I would have Loved to get the 1200 solved D today... but 0 Cigar I guess Another Day ugh and cous' Maxwell wrote that this is possible... <3
*/
	...
	z=av[0];
	bv=V(31);
	for(d=0;d<31;d++)if(av[0] ba (1<<d))bv[d]++;
	for(b=1;b<a;b++){
		z=(z bx av[b]);
		for(d=0;d<31;d++)if(z ba (1<<d))bv[d]++;
	}
	e=0;
	for(d=0;d<31;d++)if(bv[d]>(a+1)/2)e=(e bo (1<<d));
	out<<e<<" ";
	for(b=0;b<a;b++){
		e=(e bx av[b]);
		out<<e<<" ";
