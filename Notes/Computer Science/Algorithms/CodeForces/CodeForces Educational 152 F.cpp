...
Analysis:
Seems like just a sort and binary search threshold where we uh try the max of our 2 dudes first uh and then uh if that works out clears the uh >= threshold boom boom boom preserve lower value and uh that works I think after sorting just binary search in like O[n log n] or whatever here O[n * 30].
*/
	...
	while(l<r){
		...
		z=1;
		for(e=1;e<a and z;e++){
			if((c bx av[e])>=m){c=av[e];avb[e]=maxx;}
			else if(d==-1 or (d bx av[e])>=m){
				d=c;
				c=av[e];
				avb[e]=minn;
				maxx=1-maxx;
