...
*/
	...
	in>>a>>b;
	avvb=VVB(a,VB(a,0));
	for(c=0;c<a;c++)for(d=0;d<a;d++){
		in>>e;
		if(e)avvb[c][d]=1;
	}
	z=0;
	for(c=0;c<a/2;c++){
		for(d=0;d<a;d++)if(avvb[c][d]!=avvb[a-1-c][a-1-d])z++;
	}
	if(a%2==1){
		for(d=0;d<a/2;d++)if(avvb[a/2][d]!=avvb[a/2][a-1-d])z++;
	}
	if(z<=b and (a%2==1 or z%2==b%2))out<<"YES"<<nl;
	else out<<"NO"<<nl;
}
