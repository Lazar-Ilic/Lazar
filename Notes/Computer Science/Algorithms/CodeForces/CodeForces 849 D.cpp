...
*/
	...
	bv=V(a+1);
	for(c=1;c<=a;c++){
		aset.insert(as[c-1]-'a');
		av[c]=sz(aset);
	}
	aset.clear();
	for(c=1;c<=a;c++){
		aset.insert(as[a-c]-'a');
		bv[c]=sz(aset);
	}
	m=0;
	for(c=0;c<=a;c++){
		if(av[c]+bv[a-c]>m)m=av[c]+bv[a-c];
	}
	out<<m<<nl;
...
/*
	...
	b=0;
	c=a-1;
	while(b<c and ((as[b]=='0' and as[c]=='1') or (as[b]=='1' and as[c]=='0'))){
		b++;
		c--;
	}
	out<<c-b+1<<nl;
...
/*
	in>>a;
	// Can potentially halt earlier once confirmed not going to hit [1,1]
	b=0;
	c=0;
	in>>as;
	for(d=0;d<a;d++){
