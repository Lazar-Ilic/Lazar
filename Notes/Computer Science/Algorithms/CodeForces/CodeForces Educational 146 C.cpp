...
/*
So the uh relevant costs are uh like the uh we just strictly sort up the multiplicities and the uh costs and boom.
*/
	...
	bl=b;
	cl=c;
	for(d=sz(avp)-1;d>=0;d--){
		// Process this uh dude.
		e=avp[d].second;
		if(bl<cl or (bl<=cl and b<c)){
			bv.add(e);
			bl+=b;
		}
		else{
