...
*/
	...
	cv[a-2]=1;
	for(b=2;b<a;b++){
		if(av[b-1]-av[b-2]>av[b]-av[b-1])bv[b]=bv[b-1]+1;
		else bv[b]=bv[b-1]+av[b]-av[b-1];
	}
	for(b=a-3;b>=0;b--){
		if(av[b+2]-av[b+1]>av[b+1]-av[b])cv[b]=cv[b+1]+1;
		else cv[b]=cv[b+1]+av[b+1]-av[b];
	}
	in>>b;
	for(c=0;c<b;c++){
		in>>d>>e;
		d--;e--;
		if(e>d)out<<bv[e]-bv[d]<<nl;
		else out<<cv[e]-cv[d]<<nl;
	}
}
