...
Analysis:
Convert in to the iteration and the position in that iteration using inversion over triangular sum or whatever and then compute it by noting that the prefix will end up monotone increasing up to a point and the number of deletions has to do with the chain thus far which can be represented as a stack essentially works. And we can even do this in loop I think to not have to invert and so on and so on. And once the indicator is done though and we are at a monotone increasing point however with all dudes added in from the right we then just keep on removing the final dude from the Rightmost position and so on and so on.
*/

	in>>as>>al;
	bl=sz(as);
	cl=sz(as);
	c=0; // Index to add in to stack.
	ast.push(-1);
	ast.push(-1);
	while(1){
		if(bl>=al){
			// Output answer here based around the current stack and the remaining dudes to add in from the Right left in the loop for the final string.
			bs="";
			while(sz(ast)>2){
				bs.add(C('a'+ast.top()));
				ast.pop();
			...
			reverse(all(bs));
			for(e=c;e<sz(as);e++)bs.add(as[e]);
			out<<bs[sz(bs)-1-(bl-al)];
			return;
		}
		d=ast.top();
		ast.pop();
		if(d<ast.top()){
			...
			ast.push(d);
			while(c<sz(as) and as[c]-'a'>=ast.top()){
				ast.push(as[c]-'a');
				c++;
			}
			if(c==sz(as)){
