...
Z az(){
	...
	/* OK so umm like obviously let us state some initial trivial mathematical observations. So uh this immediately isomorphs into some combinatorial expression where each relevant bit has to be represented >= 1 time and from there it is down to basically solving a counting task related to those subsets of powers of 2 and the multiplicities over all the dudes of the even and odd multiplicities but we certainly only need to pay attention if there is like some special odd odd thing happening and I dunno if there is some Dynamic Programming or Combo.cpp joint way to do this in an asymptotic like O[60^3 * log[p]] or what what exactly precisely because I am not really onsighting this one in round I think per se. */
}
