...
*/
	...
	for(b=0;b<a;b++){
		if(av[b]==2){
			...
			av[b]=-1;
			y=1;
			for(c=b+1;c<a and y;c++){
				if(av[c]==0 or av[c]==-1)y=0;
				av[c]=-1;
			}
			y=1;
			for(c=b-1;c>=0 and y;c--){
				if(av[c]==0 or av[c]==-1)y=0;
				av[c]=-1;
			}
		}
	...
	for(b=0;b<a;b++){
		if(av[b]==1){
			...
			y=1;
			for(c=b+1;c<a and y;c++){
				if((av[c]==0 or av[c]==-1) and x){y=0;av[c]=-1;}
				else if(av[c]==0 or av[c]==-1)y=0;
				else av[c]=-1;
			}
		}
	}
	for(b=0;b<a;b++)if(av[b]==0)z++;
	out<<z<<nl;
}
