...
Analysis:
Seems simple enough like greedy works after sorting but when one gets around to tracking the dudes used thus far and we go over in the cyclicity it actually still requires some attention to details in how the shift shift works with like ~5 different underlying pointers... and I hopped in this mixture 20 minutes in to the round after feeling like I half onsighted this Task D for the Masterhood but was a little bit too slow on the keyboard here today to make it pop off confidently and would prefer to chillax wait around for a better opportunity to pounce...
*/
	...
	for(c=0;c<a;c++){
		avp.add({av[c],c+1}); // Multiplicity and couleur.
	}
	sortify(avp); // Later just store minimum used and always cycle through from left to right with a checker based around the minimum used dude in this cycle and the actual multiplicity when we have any down swing loop back over to the left and refresh the leftmost dude used in cycle.
	bv=V(b);
	cv=V(b);
	for(d=0;d<b;d++)in>>bv[d];
	for(d=0;d<b;d++)in>>cv[d];
	avv=VV(d,V(0));
	...
	reverse(all(lol));
	z=1;
	y=0; // Leftmost dude used in avp to be modified down... when current dude ends of course it is OK to update y uh to the current Rightmost unused value if it satisfies otherwise shwoop over to the left again.
	for(auto dude:lol){
		avv[lol.second]=V((lol.first).second);
		e=(lol.first).second;
		f=lol.second;
		x=0; // Index to update...
		w=0; // Index being used...
		while(x<e and y<a and (y==0 or avp[y].second==avp[y-1].second+1)){
			avv[f][x]=avp[w].second;
		}
		if(z==0){
			out<<-1<<nl;
			return;
