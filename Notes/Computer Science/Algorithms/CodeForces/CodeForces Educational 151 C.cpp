...
Faster and Faster and Faster and Faster. Umm perhaps does a good compiler here actually convert the underlying VB directly in to integers to compare in 1 comparison operation?
...
*/
	...
	in>>bs>>cs;
	b=0;
	avb=VB(1+cs[0]-bs[0],0);
	y=1+cs[0]-bs[0];
	z=0;
	for(c=0;c<sz(as) and b<a;c++){
		if(as[c]>=bs[b] and as[c]<=cs[b] and !avb[as[c]-bs[b]]){avb[as[c]-bs[b]]=1;z++;}
		if(z==y){
			b++;
			if(b<a){
				avb=VB(1+cs[b]-bs[b],0);
				y=1+cs[b]-bs[b];
				z=0;
			}
		}
