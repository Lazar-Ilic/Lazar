...
I could consider using a+5 instead of a everywhere in declarations of vectors but like... seems better to just focus in and dial in on super sharp tight air tight memory usage everywhere and try harder to optimise low latency C when possible using C arrays rather than vectors and focusing on runtime too.

Test + Additional Input:


Analysis:
Is there a simple way to modify this one in a simple way in order to preserve and capture all of the relevant information so that we may simply observe the Least Common Ancestor as an auxiliary and compute from there? Or will we need some sort of uh canonical Heavy Light Decomposition code here? I think it is provable that the former is not adequate here merely observing the values at the Least Common Ancestor and these 2 nodes should not be adequate here so I think we might at the very least need to add more relevant pertinent auxiliary information or execute range sub tree sub range sub path udpating here somehow and I am not sure I am seeing it in round here today ugh ugh ugh.
*/
	...
	for(b=0;b<a;b++){
		...
		if(ac=='+'){
			...
			e--;
			avv[d][2]=avv[e][2]+f;
			avv[d][0]=max(avv[e][0],avv[d][2]);
			avv[d][1]=min(avv[e][1],avv[d][2]);
			avv[d][3]=max(avv[e][3],avv[d][2]-avv[d][1]);
			avv[d][4]=min(avv[e][4],avv[d][2]-avv[d][0]);
		}
		else{
			in>>e>>f>>g;
			e--;f--;
			if(g>=avv[f][4] and g<=avv[f][3])out<<"YES"<<nl;
			else out<<"NO"<<nl;
		}
	}
}
