...
*/
	...
	//cvv=VV(a,V(0)); // Can store as uh vector like [cost,majority values...]
	VPLS cvpls;
	for(b=0;b<a-1;b++){
		in>>c>>d;
		c--;d--;
		...
		for(auto dude:bvv[b]){
			if(sz(children[dude])==0){
				...
				for(auto child:children[dude]){
					al+=cvv[child][0]+1;
					for(c=1;c<sz(cvv[child]);c++)majority[cvv[child][c]]++;
					cvv[child]=V(0);
				}
				I maxxx=0;
				for(auto lol:majority)if(lol.second>maxxx)maxxx=lol.second;
				cvv[dude].add(al-maxxx);
				for(auto lol:majority)if(lol.second==maxxx)cvv[dude].add(lol.first bx av[dude]);
			}
		}
		if(b==0){
