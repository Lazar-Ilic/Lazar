...
Analysis:
If we casework from Right to Left on the maximum value we see that each dude extends Rightwards until hitting some dude higher but what is relevant then is how for each Right step there uh how many dudes to the Left for each step work uh which means the minimum to the Left must be in the line uh... and so we just need the index where some dude appears uh what minimum we need to obtain to the Left is O[n] I think uh any dude like Left of here captures uh so 1 to the Left and then whatever is the next smallest to the Right of 1 yadda extends left yadda so each value uh is like how far Left we need to be uniquely index identifies the Left capture so this is O[n]. Umm my time management here is kind of dreck 5 minutes upsolve uh uh uh could bounce in or not uh monitour here for ABCDE maybe wait wait wait. To O[n] here is like uh something maybe about uh OKOKOK I think most likely probably out on this round uh uh uh think I should be ABCDEF this round uh but not too sharp here upon this morning ugh.
*/
