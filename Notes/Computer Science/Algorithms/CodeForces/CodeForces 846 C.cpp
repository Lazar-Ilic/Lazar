...
Z az(){
	...
	av=V(a);
	bv=V(b);
	cv=V(0); // Multiplicities of preferences.
	for(c=0;c<a;c++)in>>av[c];
	for(c=0;c<b;c++)in>>bv[c];
	sort(all(av)); // Sort the preferences.
	for(c=0;c<a;c++){
		z=1;
		while(c<a-1 and av[c+1]==av[c]){
			c++;
			z++;
		}
		c++;
	...
	sort(all(cv));
	// Then maybe compute the multiplicities.
	sort(all(bv)); // Sort the capacities.
	// Ugh probably just like a trivial little simple Maximum Flow instance somehow thresholded to restrict each table to flowing into at most 1 dude somehow. Ugh is this not known as like Maximum Bipartite Matching in the literature?
}
