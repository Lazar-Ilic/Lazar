...
Z az(){
	...
	b=1;
	while(b<a)b*=2;
	b*=2; // Smallest Power Of 2 Over Native Size
	av=V(b,0); // Count of multiplicity of raw prefix string XORs.
	av[0]=1;
	c=0;
	al=L(a)*L(a+1)/2; // Answer thus far.
	for(d=0;d<a;d++){
		in>>e;
		c^=e;
		for(f=0;f*f<b;f++)al-=av[(f*f)^c];
		av[c]++;
	}
	out<<al<<nl;
}
