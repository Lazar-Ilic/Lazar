...
Analysis:
https://stackoverflow.com/questions/42752079/max-coverage-disjoint-intervals

I think we can uh pass through and uh for each point basically uh consider uh uh uh basically like uh how to Dynamically Program if this dude was the uh Left point on a later interval and uh the Right point and uh take the maximum over the uh possible Left points from here would be like uh somehow the value is uh like uh... in the mixture now as uh uh uh like uh... basically min I mean we can invert in to the uh minimum case and uh consider how many points we do not grab is like uh... literally just the minimum out on the Left of here completely uh... and then uh case upwards uh...
*/

	in>>a;
	av=V(a);
	bv=V(a,-1); // Minimum value uncaptured to Left of dudes of colour.
	cv=V(a,-1); // Minimum for this index being a Right pointer uh or uh null I guess.
	m=0; // Maximum thus far to Output.
	in>>c;
	c--;
	av[0]=c;
	bv[c]=0;
	...
	for(b=1;b<a;b++){
		...
		c--;
		av[b]=c;
		cv[b]=min(b+1,cv[b-1]+1);
		if(bv[c]!=-1){
			cv[b]=min(cv[b],bv[c]);
			bv[c]=min(bv[c],cv[b-1]);
		}
		else{
			bv[c]=cv[b-1];
		}
