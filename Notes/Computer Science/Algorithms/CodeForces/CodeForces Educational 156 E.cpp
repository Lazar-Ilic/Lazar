...
Analysis:
I ought to take some notes here so it is an interesting 20 dimensional convex hull I think or rather convex shape upon which to try and optimise the parameters uh in reasonable polylogarithmic time queries unless I am misunderstanding something uh... where once we select the numbers for each dude we just greedy pick min dudes clearing the threshold value for that dude uh... not sure we could simply uh select an optimal parameter for each dude based around a clearing here uh... so that semi-naive is like O[[log[n]]^m] is still far too slow of course here.
*/
