...
Analysis:
Hoping this is the round ugh I would love to win my Masterhood today in the Cambridge 'hood finally retire from Competitive Programming Red And Gold Master Tiger fraternite. So let us see if I can onsight a tricky Dynamic Programming or Data Structures Task E here and nail in the ABCDE round finally ugh Top 50 is enough I should think for Master ugh ugh ugh maybe today is the day 2023-08-31... maybe I can just uh try bashing here for a low n case to find out how many dudes terminate in like a permutation ugh... for each length ugh somehow Dynamic Program uh... think maybe this is like a uh Markov Chain actually uh uh uh the Probability of each uh maximal chain leading in to a position is interesting and uh has to do with transitions and matrices uh... maybe I see if I can ace this one and then come back to the start uh... start at 0 and then each transition is OK uh uh uh well formulated and uh then the EV emerges at each state and the EV adds up and so on and so on across states P of terminating there and transitioning back in to the 0 state at the next dude adds up transitively. Take raw number works here uh in the Dynamic Program. Got that Dynamic Program onsight uh 9 minutes in.
*/
	...
	for(c=2;c<=a;c++){
		e=0;
		for(d=b;d>=1;d--){
			avv[c][d]=(e+L(b-d+1)*avv[c-1][d-1])%p;
			e=(e+avv[c-1][d-1])%p;
		}
		al=(al+avv[c][b]*exp(b,a-c))%p;
		avv[c][1]=(avv[c][1]+avv[c-1][b]*L(b))%p;
		//avv[c][b]=0;
		//outv(avv[c]);
	}
	out<<al%p<<nl;
