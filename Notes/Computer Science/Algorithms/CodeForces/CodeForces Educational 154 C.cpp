...
Kayser Soze mooudder fudgers Yikes zoinkers boinkers tight there at the end whoo whoops mooudder fudgers that was close Yikes hopefully I do not get hacked.

Test + Additional Input:


Analysis:
Store the length of the current string and the length of the longest forced valid prefix in order and any time we see a 0 we must ensure they are not equal otherwise they should be equal I think. And 0 obviously forces off backwards min call.
*/

	in>>as;
	a=0; // Longest forced valid prefix.
	b=0; // Length
	z=0;
	y=1000000000;
	for(auto dude:as){
