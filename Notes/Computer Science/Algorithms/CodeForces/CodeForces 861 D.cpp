...
Z az(){
	...
	/*
	Extremely trivial. Ought to have been an onsight in round to be sure but I did not even attempt to read the task in round. Doh!
	*/
	...
	al=L(b/2)*(a-b+1);
	bl=0;
	l=0;r=-1; // Copied from czhang2718
	for(i=0;i<=a-1;i++){
		int llll=max(i-b+1,b-1-i);
		int rrrr=min(2*a-1-b-i,i-1);
		if(llll<=rrrr){
			while(l>llll){l--;cnt[av[l]][l%2]++;}
			while(l<llll){cnt[av[l]][l%2]--;l++;}
			while(r>rrrr){cnt[av[r]][r%2]--;r--;}
			while(r<rrrr){r++;cnt[av[r]][r%2]++;}
			bl+=cnt[av[i]][i%2];
		}
	}
	out<<al-bl<<nl;
