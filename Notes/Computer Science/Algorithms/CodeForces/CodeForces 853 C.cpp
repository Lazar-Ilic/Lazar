...
Z az(){
	...
	/*
	Oh OK so like we might need to simply again do the complimentary counting argumentation so that works.
	*/

	in>>a>>b;
	al=L(a+a)*(b+1)*b/2;
	av=V(a);
	bv=V(a+b+1,-1); // Last flipped index value inclusive.
	cv=V(a+b+1); // Total to subtract off at end.
	for(c=0;c<a;c++){
		in>>av[c];
		bv[av[c]]=0; // Initiate at index 0.
	}
	for(c=1;c<=b;c++){
		in>>d>>e;
		d--;
	...
	for(c=0;c<a+b+1;c++)if(bv[c]!=-1)cv[c]+=b+1-bv[c];
	for(c=0;c<a+b+1;c++)al-=L(cv[c])*(cv[c]-1)/2;
	out<<al<<nl;
}
