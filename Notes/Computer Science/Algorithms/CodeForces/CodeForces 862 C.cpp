...
*/
	...
	for(c=0;c<b;c++){
		in>>al>>bl>>cl;
		if(bl*bl-4*al*cl>=0 and ()*()<=0){
			out<<"No"<<nl;
		}
		else{
			// I think just checking the range of uh potential possible tangent like computing the uh pair of points where the uh derivative actually passes through 0 can work to find the interval of potential plausible ranges which work.
		}
	}
}
