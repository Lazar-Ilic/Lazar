...
/*
Consider if they are both uh matched already we need to like compute the uh maximal uh increasing chain I uh think I mean non-decreasing chain that sorts to itself essentially uh compute the uh maximal non-decreasing chain in uh is it not just like uh finding the Left and Right deviation values and then the maximal non-decreasing chain in the target which works there? Is ensured to uh work I think uh.
*/
	...
	e=-1;
	c=0;
	while(c<a and av[c]==bv[c])c++;
	if(c<a){
		d=c;
		c=a-1;
		while(c>0 and av[c]==bv[c])c--;
		e=c;
		// Umm OKOKOK so nonnegative IFF care ungh.
	}
	if(d==-1){
		// Just compute longest non-decreasing in bv.
		m=1; // Maximum chain thus far ungh.
		d=0;
		e=0; // Left and Right de facto.
		for(c=0;c<a;c++){
			f=c; // Chain up the c-value I think um.
			while(c+1<a and bv[c+1]>=bv[c])c++;
			if(c-f+1>m){
				m=c-f+1;
				d=f;
				e=c;
		...
		m=INT_MAX;
		for(c=e;c>=d;c--){
			if(bv[c]<m)m=bv[c]; // Stores minimum.
		}
		while(d-1>=0 and bv[d-1]<=m){ // Will terminate at proper d value I think.
			m=bv[d-1];
			d--;
		}
		m=0;
		for(c=d;c<=e;c++){
			if(bv[c]>m)m=bv[c]; // Stores maximum.
		}
		while(e+1<a and bv[e+1]>=m){
			m=bv[e+1];
			e++;
		}
		out<<d+1<<" "<<e+1<<nl;
