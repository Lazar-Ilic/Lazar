%%writefile a.cpp
// "I was born with the strength of the Sun" - レーザ [篠田 賢治]
...
// Time: O[N * log [N]]
//const L mod=(119<<23)+1,root=62;
const L mod=(L(12)<<28)+1,root=5; // = 998244353
// WolframAlpha: primitive root of (12*2^28+1)
// For p < 2^30 there is also e.g. 5 << 25, 7 << 26, 479 << 21
// and 483 << 21 (same root). The last two are > 10^9.
L modpower(L a,L b){
	L ans=1;for(;b;a=a*a%mod,b/=2)if(b&1)ans=ans*a%mod;
	return ans;
}
Z ntt(VL &a){//chilli
...
*/
	...
	/*
	avl=VL(1048576,1755647);
	bvl=VL(1048576,1755647);
	cvl=convolution(avl,bvl);
	out<<mod<<nl;
	out<<cvl[999999]<<nl; // Confirmed Right ACcepted 276262510
	*/

	avl=VL(1048576,1000000000);
	bvl=VL(1048576,1000000000);
	cvl=convolution(avl,bvl);
	out<<mod<<nl;
	out<<cvl[999999]<<nl; // OKOKOK this is Wrong but promising steps forward in the right direction perhaps with a modified version on an underlying uh 80-bit or 96-bit or 128-bit or Karatsuba style here could be enough to AC or run for 100 days in Python...
}
