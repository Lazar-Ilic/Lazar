...
/*
Example of faster summing summation of the maximum 500000 values from a pseudorandomly generated vector of integers of size 10000000 with heapify in I believe that this Standard Template Library stl make_heap heapify function here is O[n] call and uh well in worst case this is then followed by like uh 500000 O[log n] calls is still overall uh worst case O[n] whereas the latter could be uh like 10000000 calls at uh O[log 500000] or something like that uh uh uh and here is faster uh.
*/
	...
	av=V(10000000);
	for(b=0;b<10000000;b++)av[b]=10000*(rand()%10000)+(rand()%10000);
	bv=av;
	//time(&starttime);
	starttime=clock();
	gettimeofday(&start1,NULL);
	al=0;
	make_heap(av.begin(),av.end());
	for(b=0;b<500000;b++){
		al+=av.front();
		pop_heap(av.begin(),av.end());
		av.pop_back();
	}
	out<<al<<nl;
	//time(&endtime);
	...
	out<<setprecision(5)<<endtime-starttime<<nl;
    out<<setprecision(6)<<((end1.tv_sec-start1.tv_sec)*1e6+(end1.tv_usec-start1.tv_usec))*1e-6<<nl;
	//time(&starttime);
	starttime=clock();
	gettimeofday(&start2,NULL);
	al=0;
	for(b=0;b<500000;b++)aminpq.push(bv[b]);
	for(b=500000;b<10000000;b++){
		if(bv[b]>aminpq.top()){
			aminpq.pop();
			aminpq.push(bv[b]);
		}
	...
	out<<setprecision(5)<<endtime-starttime<<nl;
	out<<setprecision(6)<<((end2.tv_sec-start2.tv_sec)*1e6+(end2.tv_usec-start2.tv_usec))*1e-6<<nl;
}
