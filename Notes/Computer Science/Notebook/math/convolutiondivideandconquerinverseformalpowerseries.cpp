...
"
- Wasalu Muhammad Jaco, Drogas [Losers To] 光 Light レーザー LASERs
...
*/
	...
	int A[3];
	int T;
	for (; ~scanf("%d%d%d%d", &A[0], &A[1], &A[2], &T); ) {
		sort(A, A + 3);
		Mint ans = 0;
		if (A[0] == A[2]) {
			if (T == 0) {
			...
			const int T0 = (A[2] - A[0]) / 2;
			const int X0 = (A[0] + A[2]) / 2;
			if (T0 <= T) {
				const int n = T - T0;
				Poly es(n + 1), fs(n + 1), gs(n + 1);
				for (int i = 0; i <= n; ++i) {
					es[i] = 1;
					fs[i] = 1;
					gs[i] = 1;
					for (int h = 0; h < 3; ++h) {
						es[i] *= invFac[i];
						fs[i] *= invFac[(T0 + (X0 - A[h])) / 2 + i];
						gs[i] *= invFac[(T0 - (X0 - A[h])) / 2 + i];
					}
				}
				es = convolve(es, es);
				es.resize(n + 1);
				Poly hs = convolve(fs, gs);
				hs.resize(n + 1);
				for (int i = 0; i <= n; ++i) {
					for (int h = 0; h < 3; ++h) {
						es[i] *= fac[i];
