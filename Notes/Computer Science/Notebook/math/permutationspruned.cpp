...
*/
	...
	// Example of partially optimised pruned search through permutations lexicographically. This is for USAMTS Year 35 Round 2 Task 1. Could potentially be optimised further.
	av={20,21,22,23,24,25,27,28,30,32,33,35};
	avv={{},{0},{1},{0},{0,1,3},{1,2,4},{2,5},{3,4},{4,5,7},{5,6,8},{7,8},{8,9,10}};
	avvb=VVB(12,VB(12));
	bvvb=VVB(12,VB(12));
	bvvb[2][1]=bvvb[3][0]=bvvb[4][0]=bvvb[4][3]=bvvb[5][1]=bvvb[5][2]=bvvb[5][4]=bvvb[6][5]=bvvb[7][3]=bvvb[7][4]=bvvb[8][7]=bvvb[9][6]=bvvb[9][8]=bvvb[11][8]=bvvb[11][9]=1;
	for(a=0;a<12;a++)for(b=a+1;b<12;b++)if(gcd(av[a],av[b])==1){avvb[a][b]=avvb[b][a]=1;}
	bv=V(0);
	for(a=0;a<12;a++)aset.insert(a);
	z=-1;
	while(1){
		// Insert in lexicographic permutation order.
		if(aset.upperbound(z)!=aset.end()){
			bv.add(*(aset.upperbound(z)));
			aset.erase(aset.upperbound(z));
			z=-1;
		}
		else{
			while(sz(bv)>0 and aset.upperbound(z)==aset.end()){
				z=bv[sz(bv)-1];
				aset.insert(bv[sz(bv)-1]);
				bv.pop_back();
			}
			if(aset.upperbound(z)!=aset.end()){
				bv.add(*(aset.upperbound(z)));
		...
		// Test this permutation prefix by checking only the new value. If failed then change the z to this back value and remove via bv.pop_back(). If valid and complete, then output [and return].
		y=1;
		for(a=0;a<sz(avv[sz(bv)-1]) and y;a++){
			if(bvvb[sz(bv)-1][avv[sz(bv)-1][a]]!=avvb[bv[sz(bv)-1]][bv[avv[sz(bv)-1][a]]]){
				y=0;
				z=bv[sz(bv)-1];
				aset.insert(z);
				bv.pop_back();
		...
		if(y and sz(bv)==12){
			for(auto ind:bv)out<<av[ind]<<" ";
			out<<nl; // 28 22 27 23 33 35 24 20 21 25 30 32
			return;
		}
	}
}
