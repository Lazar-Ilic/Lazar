...
// Optimized PMF calculation using running product
static long double pmf(int k, long double lam) {
    if (k < 0 || lam < 0) return 0.0;
    if (k == 0) return exp(-lam);
    if (k > 170) return 0.0;  // Avoid overflow in factorial
    
    long double result = exp(-lam);
    for (int i = 1; i <= k; ++i) {
        result *= (lam / i);
    }
    return result;
...
void build_tables_for_a(int a, RouteTables& tab) {
    ...
    for (int c = MIN_CAP; c <= MAX_CAP; ++c) {
        tab.ec1[c - MIN_CAP] = static_cast<long double>(c + TOLL * lambda);
    }

    std::vector<long double> pb(NUM_CAPS, 0.0);
    std::vector<long double> ecb(NUM_CAPS, 0.0);
    std::vector<long double> eca(NUM_CAPS, 0.0);
    
    for (int c_idx = 0; c_idx < NUM_CAPS; ++c_idx) {
        int c = c_idx + MIN_CAP;
        long double sp = 0.0, sb = 0.0, sa = 0.0, pp = 0.0, pa = 0.0;
        
        for (int k = 0; k <= 40; ++k) {
            long double p = pmf(k, lambda);
            if (p < 1e-15) break;
            
            long double cost = c + TOLL * k;
            if (cost <= 30) {  
                pp += p;
                sb += p * cost;
            } else {
        ...
        pb[c_idx] = pp;
        ecb[c_idx] = (pp > 1e-15) ? (sb / pp) : static_cast<long double>(c);
        eca[c_idx] = (pa > 1e-15) ? (sa / pa) : static_cast<long double>(c + TOLL);
    }
    ...
    for (int s = 0; s < N_H_STRAT; ++s) {
        ...
        for (int cR_idx = 0; cR_idx < NUM_CAPS; ++cR_idx) {
            for (int cU_idx = 0; cU_idx < NUM_CAPS; ++cU_idx) {
                long double pbR = pb[cR_idx], paR = 1.0 - pbR;
                long double pbU = pb[cU_idx], paU = 1.0 - pbU;
                ...
                tab.pR[cR_idx][cU_idx][s] = static_cast<long double>(pr);
                
                if (pr > 1e-15) {
                    long double nR = w10 * ecb[cR_idx] + 0.5 * w11 * ecb[cR_idx] + 0.5 * w00 * eca[cR_idx];
                    tab.ccR[cR_idx][cU_idx][s] = static_cast<long double>(nR / pr);
                } else {
                ...
                if (pu > 1e-15) {
                    long double nU = w01 * ecb[cU_idx] + 0.5 * w11 * ecb[cU_idx] + 0.5 * w00 * eca[cU_idx];
                    tab.ccU[cR_idx][cU_idx][s] = static_cast<long double>(nU / pu);
                } else {
...
int main() {
    ...
    printf("Precomputing physics tables for a in [0, 300]... \n");
    fflush(stdout);
