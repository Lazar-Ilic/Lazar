...
Z az(){
	...
	sieve(1000000);
	for(a=0;a<sz(primes);a++){
		if(primes[a]>=100000 and primes[a]<1000000){
			aset.clear();
			b=primes[a];
			while(b!=0){
				aset.insert(b%10);
	...
	I maxscore,minscore,maxexample,minexample;
	maxscore=-1;
	minscore=INT_MAX;
	for(a=10000000;a<30000000;a++){
		if(a%100000==0)out<<a<<nl;
		aset.clear();
		cv=V(10);
		b=a;
		while(b!=0){
		...
		if(sz(aset)==8){
			...
			for(c=0;c<sz(av);c++){
				...
				while(d!=0 and e){
					if(!cv[d%10])e=0;
					f+=min(abs(bv[d%10]-bv[g]),8-abs(bv[d%10]-bv[g]));
					g=d%10;
					d/=10;
				}
				if(e){
	...
	out<<maxexample<<nl<<minexample<<nl;
}
