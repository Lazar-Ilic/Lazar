...
const L MAX_PR=(long long)4e9+1;
std::vector<std::bitset<MAX_PR>> wrapper(1);
auto & isprime = wrapper[0];
V sieve(L lim){//Håkan Terelius
...
*/
	...
	sieve(MAX_PR-1);
	x=0;
	for(al=0;al<MAX_PR-10000000;al++){
		z=1;
		cl=al;
		for(b=0;b<2024 and z;b++){
			cl+=b;
			if(isprime[cl])z=0;
			else if(b==x){
				out<<x<<" "<<al<<nl;
	...
	*/
	y=1;
	bitset<1010000000> lol;
	for(al=0;;al+=1000000000){
		out<<al<<nl;
		lol.reset();
		for(bl=0;bl<1010000000;bl++){
			if(isprime(al+bl)){
				cl=bl;
				z=0;
				for(;cl>=0 and z<2024;){
