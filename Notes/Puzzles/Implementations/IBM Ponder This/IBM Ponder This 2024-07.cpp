...
*/
	avv=VV(25,V(0));
	for(a=0;a<(1<<16);a++){
		if(subsetsize(a)==8 and auset.count(a)==0 and buset.count(a)==0){
			// Compute the rotations of a and insert.
			b=0;c=0;d=0;e=0;f=0;g=0;h=0;
			for(i=0;i<4;i++)for(j=0;j<4;j++)if(a ba (1<<(4*i+j)))b+=(1<<(4*(3-j)+(i)));
			for(i=0;i<4;i++)for(j=0;j<4;j++)if(a ba (1<<(4*i+j)))c+=(1<<(4*(3-i)+(3-j)));
			for(i=0;i<4;i++)for(j=0;j<4;j++)if(a ba (1<<(4*i+j)))d+=(1<<(4*(j)+(3-i)));
			for(i=0;i<4;i++)for(j=0;j<4;j++)if(a ba (1<<(4*i+j)))e+=(1<<(4*(i)+(3-j)));
			for(i=0;i<4;i++)for(j=0;j<4;j++)if(a ba (1<<(4*i+j)))f+=(1<<(4*(3-j)+(3-i)));
			for(i=0;i<4;i++)for(j=0;j<4;j++)if(a ba (1<<(4*i+j)))g+=(1<<(4*(3-i)+(j)));
			for(i=0;i<4;i++)for(j=0;j<4;j++)if(a ba (1<<(4*i+j)))h+=(1<<(4*(j)+(i)));
			z=1;
			for(i=0;i<4 and z;i++)for(j=0;j<2 and z;j++)if((((bool)(a ba (1<<(4*i+j))))==((bool)(a ba (1<<(4*i+j+1)))) and ((bool)(a ba (1<<(4*i+j+1))))==((bool)(a ba (1<<(4*i+j+2))))) or (((bool)(a ba (1<<(4*j+i))))==((bool)(a ba (1<<(4*(j+1)+i)))) and ((bool)(a ba (1<<(4*(j+1)+i))))==((bool)(a ba (1<<(4*(j+2)+i))))))z=0;
			if(z){
				auset.insert(a);
				auset.insert(b);
				auset.insert(c);
				...
				auset.insert(h);
				b=0;
				for(c=0;c<4;c++)for(d=0;d<3;d++){
					b+=(int)(bool)(((bool)(a ba (1<<(4*c+d))))==((bool)(a ba (1<<(4*c+d+1)))))+(int)(bool)(((bool)(a ba (1<<(4*d+c))))==((bool)(a ba (1<<(4*(d+1)+c)))));
				}
				avv[b].add(a);
			}
			else{
	...
	for(a=0;a<25;a++)out<<a<<" "<<sz(avv[a])<<nl;
	for(a=0;a<=6;a++){
		out<<nl<<a<<nl<<nl;
		for(auto dude:avv[a]){
			for(b=0;b<4;b++){
				for(c=0;c<4;c++)out<<(int)(bool)(dude ba (1<<(4*b+c)));
				out<<nl;
			}
			out<<nl;
		}
	}
	avs={"1010100101001010","1101011010101101","0100100101010010","1010101010110101","0101010101001010","1010101010110101","1011010101010101","0100101010101010","1011010101010101","0100101010101011","1001010010010100","0110101101101001","1001010010010110","0110101101101011","0101010010110100","1010101101001010"};
	z=0;
	for(a=0;a<16;a++)for(b=0;b<15;b++)z+=(int)(bool)(avs[a][b]==avs[a][b+1])+(int)(bool)(avs[b][a]==avs[b+1][a]);
	out<<z<<nl;
	for(a=0;a<16;a++)out<<avs[a]<<nl;
	out<<nl;
	for(m=0;m<4;m++)for(n=0;n<4;n++){
		a=0;
		for(k=0;k<4;k++)for(l=0;l<4;l++)if(avs[4*m+k][4*n+l]=='1')a+=(1<<(4*k+l));
		b=0;c=0;d=0;e=0;f=0;g=0;h=0;
		for(i=0;i<4;i++)for(j=0;j<4;j++)if(a ba (1<<(4*i+j)))b+=(1<<(4*(3-j)+(i)));
		for(i=0;i<4;i++)for(j=0;j<4;j++)if(a ba (1<<(4*i+j)))c+=(1<<(4*(3-i)+(3-j)));
		for(i=0;i<4;i++)for(j=0;j<4;j++)if(a ba (1<<(4*i+j)))d+=(1<<(4*(j)+(3-i)));
		...
		for(i=0;i<4;i++)for(j=0;j<4;j++)if(a ba (1<<(4*i+j)))g+=(1<<(4*(3-i)+(j)));
		for(i=0;i<4;i++)for(j=0;j<4;j++)if(a ba (1<<(4*i+j)))h+=(1<<(4*(j)+(i)));
		if(buset.count(a)>0 or cuset.count(a)>0){out<<"Invalid Solution"<<nl;return;}
		cuset.insert(a);
		cuset.insert(b);
		cuset.insert(c);
		cuset.insert(d);
