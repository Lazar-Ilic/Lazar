...
Analysis:
Rather than calling sqrt O[1000000] times here in a setting like this one or in some setting if we were scanning over the region between 2 close circles one could instead use 2 pointers which are updated each cycle which would then bring that component of the runtime analysis down from O[n * log[blah]] to O[n] which is actually very convenient if the circles are very close especially this is super relevant. And in general with underlying monotone functions.
*/
	...
	while(1){
		...
		avb=VB(jl-il,0);
		for(al=0;al<gl;al++){
			dl=(L(sqrt(4*il-3*al*al))-al)/2-2;
			el=(L(sqrt(4*jl-3*al*al))-al)/2+2;
			fl=al*al+al*dl+dl*dl;
			if(fl>il and fl<jl)avb[fl-il]=1;
			for(bl=dl+1;bl<el;bl++){
				fl+=al+bl+bl-1;
				if(fl>il and fl<jl)avb[fl-il]=1;
			}
		}
		for(al=gl+1;al<hl-1;al++){
			bl=0;
			for(cl=al*al+1;cl<(al+1)*(al+1);cl++)if(avb[cl-il])bl++;
			if(bl==kl){
				out<<al<<nl;
				return;
			}
