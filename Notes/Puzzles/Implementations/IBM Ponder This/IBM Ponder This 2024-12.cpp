...
*/
	av=V(10); // Number Of Dudes Thus Far Having Digit
	bv=V(10); // Maxima
	for(a=1;a<1000000000;a++){
		if(a%10000000==0)out<<a<<nl;
		b=a;
		avb=VB(10,0);
		while(b>0){
			avb[b%10]=1;
		...
		for(b=1;b<10;b++)if(avb[b])av[b]++;
		for(b=1;b<10;b++)if(2*av[b]==a)bv[b]=a;
	}
	outv(bv); // https://oeis.org/A346685
}
