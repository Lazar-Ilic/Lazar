...
*/
	...
	VB lol(26000000000,0);
	for(a=2;a<26000000000;a+=2)lol[a]=1;
	b=0;
	c=0;
	al=0;
	QL aql;
	for(a=3;a<26000000000;a++){
		if(lol[a]==0){
			al+=c;
			for(d=a;d<26000000000;d+=a)lol[d]=1;
			if(b<1000000000){
				c++;
				b++;
				aql.push(a+2*1000000000);
				if(b==1000000000)out<<a<<nl;
			}
		}
		if(sz(aql) and aql.front()==a){
			aql.pop();
			c--;
		}
	}
